...
describe('CandleValidator', () => {
  it('should validate a correct candle', () => {
    ...
    const res = CandleValidator.validate(candle);
    expect(res.isValid).toBe(true);
    expect(res.errors).toHaveLength(0);
  });

  it('should reject candles with High < Open or High < Close', () => {
    const invalidHighCandle: ICandle = {
      timestamp: new Date(),
      open: 24800,
  ...
  it('should reject negative prices and negative volume', () => {
    ...
    const res = CandleValidator.validate(negativeCandle);
    expect(res.isValid).toBe(false);
  ...
  it('should normalize, sort chronologically, and deduplicate a candle series', () => {
    ...
    const result = CandleValidator.normalizeAndCleanSeries(rawCandles);
  ...
  it('should convert timeframes to milliseconds correctly', () => {
    expect(CandleValidator.timeframeToMs(Timeframe.M1)).toBe(60000);
    expect(CandleValidator.timeframeToMs(Timeframe.M5)).toBe(300000);
    expect(CandleValidator.timeframeToMs(Timeframe.M15)).toBe(900000);
    expect(CandleValidator.timeframeToMs(Timeframe.H1)).toBe(3600000);
    expect(CandleValidator.timeframeToMs(Timeframe.H4)).toBe(14400000);
    expect(CandleValidator.timeframeToMs(Timeframe.D1)).toBe(86400000);
  });
});
