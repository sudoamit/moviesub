...
const execution_price_resolver_1 = require("../execution-price-resolver");
...
describe('ExecutionPriceResolver', () => {
    it('should resolve fresh live tick with LIVE_TICK source tag', async () => {
        ...
        const result = await execution_price_resolver_1.ExecutionPriceResolver.resolveExecutionPrice({
            symbol: 'BTCUSDT',
            tradingMode: enums_1.TradingMode.PAPER,
        ...
        expect(result.source).toBe(enums_1.ExecutionPriceSource.LIVE_TICK);
    ...
    it('should reject execution in PAPER mode if live tick is missing even if a candle is available (zero candle fallback)', async () => {
        await expect(execution_price_resolver_1.ExecutionPriceResolver.resolveExecutionPrice({
            symbol: 'NIFTY',
            tradingMode: enums_1.TradingMode.PAPER,
            maxMarketDataAgeSeconds: 5,
            liveTick: null,
        ...
        })).rejects.toThrow(errors_1.MarketDataUnavailableError);
    });
    it('should throw StaleMarketDataError if market tick exceeds max age threshold in PAPER mode', async () => {
        ...
        })).rejects.toThrow(errors_1.StaleMarketDataError);
    });
    ...
    it('should allow historical candle in BACKTEST mode with BACKTEST_CANDLE source tag', async () => {
        const result = await execution_price_resolver_1.ExecutionPriceResolver.resolveExecutionPrice({
            symbol: 'NIFTY',
            tradingMode: enums_1.TradingMode.BACKTEST,
            getLatestCandle: async () => ({
        ...
        expect(result.source).toBe(enums_1.ExecutionPriceSource.BACKTEST_CANDLE);
    });
    describe('calculateSlippage', () => {
        it('BUY: fill price >= reference price (reference: 100, slippage: 10 bps -> fill: 100.10)', () => {
            const buySlip = execution_price_resolver_1.ExecutionPriceResolver.calculateSlippage(100, 'BUY', 50, 10);
        ...
        it('SELL: fill price <= reference price (reference: 100, slippage: 10 bps -> fill: 99.90)', () => {
            const sellSlip = execution_price_resolver_1.ExecutionPriceResolver.calculateSlippage(100, 'SELL', 50, 10);
        ...
        it('Zero slippage: fill price equals reference price', () => {
            const zeroSlip = execution_price_resolver_1.ExecutionPriceResolver.calculateSlippage(100, 'BUY', 0);
        ...
        it('Maximum configured slippage: slippage never exceeds maxSlippageBps', () => {
            const maxSlip = execution_price_resolver_1.ExecutionPriceResolver.calculateSlippage(100, 'BUY', 20, 100);
            expect(maxSlip.slippageBps).toBe(20);
            expect(maxSlip.fillPrice).toBe(100.2);
        });
    });
});
//# sourceMappingURL=execution-price-resolver.test.js.map