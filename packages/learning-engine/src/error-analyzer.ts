import { FailureMode, IErrorReport, IFailureModeStats, TradingExperience } from './types';

export class ErrorAnalyzer {
  /**
   * Analyzes an array of completed trading experiences to compile a detailed Error Report.
   */
  public static analyze(experiences: TradingExperience[]): IErrorReport {
    ...
    const failureStats: IFailureModeStats[] = [];
    ...
    for (const [mode, bucket] of modeBuckets.entries()) {
      ...
      const timeframeDist: Record<string, number> = {};
      ...
      for (const e of exps) {
        ...
        const tf = (e.marketState?.candles?.[0]?.timeframe as string) || '15m';
    ...
    for (const driver of topLossDrivers) {
      if (driver.failureMode === 'HTF_CONFLICT' && driver.count >= 5) {
        recommendations.push(
          `Eliminate trades conflicting with Higher Timeframe trend. ${driver.count} trades generated ${driver.averageR}R expectancy.`,
        );
      } else if (driver.failureMode === 'VOLATILITY_MISREAD' && driver.count >= 5) {
        recommendations.push(
          `Restrict entries during HIGH_VOLATILITY shocks or scale position sizing down by 50%.`,
        );
      } else if (driver.failureMode === 'STOP_TOO_TIGHT' && driver.count >= 5) {
        recommendations.push(
          `Widen minimum structural stop buffer. Tight stops caused ${driver.count} early premature invalidations.`,
        );
      } else if (driver.failureMode === 'TARGET_TOO_FAR' && driver.count >= 5) {
        recommendations.push(
          `Introduce partial scaling at TP1 (1.5R) before trailing to breakeven to lock in accrued favorable excursion.`,
        );
      } else if (driver.lossContributionPct > 20) {
        recommendations.push(
          `Failure mode ${driver.failureMode} is responsible for ${driver.lossContributionPct}% of total losses. Prioritize generating a candidate filter.`,
        );
      }
    }
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        'Failure distribution is well-dispersed without concentrated systematic errors.',
      );
    }
   ...
   */
  public static analyzeHighConfidenceLosses(experiences: TradingExperience[]): {
    count: number;
    frequencyPct: number;
    averageLossR: number;
    ...
    const highConfLosses = highConf.filter(
      (e) => e.outcome?.status === 'LOSS' || (e.outcome?.pnlR || 0) < 0,
    );
   ...
   */
  public static computeConditionalProbabilities(experiences: TradingExperience[]): {
    byRegime: Record<string, { winRate: number; count: number; expectancyR: number }>;
    byVolatility: Record<string, { winRate: number; count: number; expectancyR: number }>;
    bySession: Record<string, { winRate: number; count: number; expectancyR: number }>;
    bySetup: Record<string, { winRate: number; count: number; expectancyR: number }>;
  } {
    const calcGroup = (keyExtractor: (e: TradingExperience) => string) => {
      const groups: Record<string, TradingExperience[]> = {};
      for (const e of experiences) {
        const k = keyExtractor(e) || 'UNKNOWN';
        if (!groups[k]) groups[k] = [];
        groups[k].push(e);
      }
      const res: Record<string, { winRate: number; count: number; expectancyR: number }> = {};
      for (const [k, exps] of Object.entries(groups)) {
        const wins = exps.filter(
          (e) => e.outcome?.status === 'WIN' || (e.outcome?.pnlR || 0) > 0,
        ).length;
