...
export class DriftDetector {
  ...
  public static evaluateDrift(
    ...
    if (recentExp < baselineExpectancyR * 0.4) {
      ...
      details.push(
        `Performance Drift: Recent expectancy (${recentExp.toFixed(2)}R) dropped >60% below baseline (${baselineExpectancyR}R).`,
      );
    }

    // 2. Execution Drift Check
    const slippageCount = recentExperiences.filter((e) =>
      e.failureReasons.includes('SLIPPAGE'),
    ).length;
    if (slippageCount / count > 0.15) {
      executionDrift = true;
      details.push(
        `Execution Drift: Excessive slippage detected on ${((slippageCount / count) * 100).toFixed(1)}% of trades.`,
      );
    }

    // 3. Regime Drift Check
    const highVolCount = recentExperiences.filter(
      (e) => e.marketContext.regime === 'HIGH_VOLATILITY',
    ).length;
    if (highVolCount / count > 0.4) {
      regimeDrift = true;
      details.push(
        `Regime Drift: Unusual concentration in HIGH_VOLATILITY regime (${((highVolCount / count) * 100).toFixed(1)}% of trades).`,
      );
    }

    // 4. Prediction Drift Check
    const meanProb =
      recentExperiences.reduce((sum, e) => sum + (e.prediction.probabilityWin || 0.5), 0) / count;
    if (meanProb < 0.4 || meanProb > 0.85) {
      predictionDrift = true;
      details.push(
        `Prediction Drift: Model confidence shifted abnormally to mean P(Win) = ${(meanProb * 100).toFixed(1)}%.`,
      );
    }

    const hasDrift =
      featureDrift || predictionDrift || performanceDrift || regimeDrift || executionDrift;

    let recommendation: 'CONTINUE' | 'MONITOR_CLOSELY' | 'FREEZE_PROMOTIONS' | 'ROLLBACK' =
      'CONTINUE';
