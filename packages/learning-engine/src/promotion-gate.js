...
class PromotionGate {
     ...
     */
    static evaluateCandidate(candidate, criteria = this.DEFAULT_CRITERIA) {
        const reasons = [];
        const rejectionDetails = [];
        let score = 100;
        // 1. Sample Size Check
        if (candidate.evidence.sampleSize < criteria.minHistoricalTrades) {
            rejectionDetails.push(`Insufficient sample size: ${candidate.evidence.sampleSize} trades < required ${criteria.minHistoricalTrades}.`);
            score -= 30;
        }
        else {
            reasons.push(`Historical sample size validated (${candidate.evidence.sampleSize} observations).`);
        }
        // 2. Out-of-Sample Expectancy Delta Check
        const oosExp = candidate.validationMetrics?.outOfSampleExpectancy ??
        ...
        if (delta < criteria.minOutOfSampleExpectancyDelta) {
            rejectionDetails.push(`Out-of-sample expectancy improvement (+${delta.toFixed(2)}R) did not meet minimum threshold (+${criteria.minOutOfSampleExpectancyDelta}R).`);
            score -= 30;
        }
        else {
            reasons.push(`Demonstrated statistically robust out-of-sample improvement (+${delta.toFixed(2)}R).`);
        }
        ...
        if (ruinProb > criteria.maxMonteCarloRuinProbability) {
            rejectionDetails.push(`Monte Carlo probability of ruin (${(ruinProb * 100).toFixed(1)}%) exceeds safety limit (${(criteria.maxMonteCarloRuinProbability * 100).toFixed(1)}%).`);
            score -= 25;
        }
        else {
        ...
        if (criteria.requireTransactionCostSurvival &&
            ...
            reasons.push(`Maintains positive expectancy under aggressive transaction fee and slippage stress.`);
        }
        // 5. Shadow Trading Validation (Mandatory for promotion)
        if (!candidate.shadowMetrics) {
            rejectionDetails.push(`Insufficient shadow evidence: shadowMetrics is missing. Candidate cannot be promoted without shadow evidence.`);
            score -= 40;
        }
        else if (candidate.shadowMetrics.shadowTradeCount < criteria.minShadowTrades) {
            rejectionDetails.push(`Insufficient shadow trade volume (${candidate.shadowMetrics.shadowTradeCount} trades < required ${criteria.minShadowTrades}).`);
            score -= 40;
        }
        else if (candidate.shadowMetrics.shadowExpectancy < expBefore) {
            rejectionDetails.push(`Shadow trading expectancy (${candidate.shadowMetrics.shadowExpectancy}R) underperformed baseline (${expBefore}R).`);
            score -= 35;
        }
        else {
            reasons.push(`Live shadow trading performance confirmed (+${candidate.shadowMetrics.shadowExpectancy}R across ${candidate.shadowMetrics.shadowTradeCount} trades).`);
        }
        // 6. Explicit Auto Promotion Approval Check
        if (!criteria.allowAutoPromotion) {
            rejectionDetails.push(`Auto promotion disabled (allowAutoPromotion is false). Candidate strategy held in SHADOW / VALIDATED state.`);
            score -= 50;
        }
     ...
     */
    static calculateCanaryAllocation(candidate, liveCanaryTradesCount, canaryExpectancyR, baselineExpectancyR) {
