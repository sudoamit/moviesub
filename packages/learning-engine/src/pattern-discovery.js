...
class PatternDiscoveryEngine {
     ...
     */
    static discover(experiences, options = {}) {
        ...
        const predicateDefs = [
            {
                name: 'HTF_ALIGNED',
                test: (e) => !e.failureReasons.includes('HTF_CONFLICT') &&
                    e.marketState?.multiHorizon?.alignment === 'ALIGNED',
            },
            {
                name: 'LIQUIDITY_SWEPT',
                test: (e) => e.marketState?.smc?.liquiditySweeps?.length > 0 ||
                    (e.marketState?.quant?.smcQuant?.liquiditySweepDepth || 0) > 0,
            },
            {
                name: 'TRENDING_REGIME',
                test: (e) => e.marketContext.regime === 'BULLISH_TREND' || e.marketContext.regime === 'BEARISH_TREND',
            },
            {
                name: 'HIGH_VOLATILITY',
                test: (e) => e.marketContext.regime === 'HIGH_VOLATILITY' ||
                    e.marketContext.volatilityRegime === 'HIGH',
            },
            {
                name: 'HIGH_RVOL',
                test: (e) => (e.marketState?.quant?.momentum?.relativeVolume || 1.0) >= 1.3,
            },
            {
                name: 'DISPLACEMENT_CONFIRMED',
                test: (e) => (e.marketState?.quant?.smcQuant?.bosStrength || 0) >= 70 ||
                    (e.marketState?.quant?.smcQuant?.chochStrength || 0) >= 70,
            },
            {
                name: 'OPTIMAL_DEALING_RANGE',
                test: (e) => !e.failureReasons.includes('BAD_PREMIUM_DISCOUNT'),
            },
        ];
        ...
        candidateConditions.push({
            ...
            test: (e) => predicateDefs[0].test(e) && predicateDefs[1].test(e) && predicateDefs[4].test(e),
        });
        candidateConditions.push({
            name: 'HTF_ALIGNED + TRENDING_REGIME + DISPLACEMENT_CONFIRMED',
            test: (e) => predicateDefs[0].test(e) && predicateDefs[2].test(e) && predicateDefs[5].test(e),
        });
        ...
        for (const cand of candidateConditions) {
            ...
            const robustnessScore = Math.min(100, Math.round(Math.min(1.0, sampleSize / 100) * 40 + Math.min(1.0, tStat / 3.0) * 60));
...
//# sourceMappingURL=pattern-discovery.js.map