...
exports.WalkForwardValidator = void 0;
...
class WalkForwardValidator {
     ...
     */
    static validate(candidate, experiences, options = {}) {
        ...
        const sorted = [...experiences].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
        ...
        for (let f = 0; f < numFolds; f++) {
            ...
            if (trainSlice.length === 0 || testRaw.length === 0)
                continue;
            ...
            const foldArtifact = Object.freeze({
                foldIndex: f + 1,
            ...
            const isEval = candidate_evaluator_1.CandidateEvaluator.evaluate(foldCandidate, trainSlice);
            // 2. Evaluate frozen retrained candidate on validation fold
            const valEval = candidate_evaluator_1.CandidateEvaluator.evaluate(foldCandidate, valSlice);
            // 3. Evaluate frozen retrained candidate out-of-sample on OOS fold
            const oosEval = candidate_evaluator_1.CandidateEvaluator.evaluate(foldCandidate, testSlice);
            ...
            const winRate = testSlice.length > 0 ? Number(((wins / testSlice.length) * 100).toFixed(1)) : 50;
        ...
        const isRobust = meanOOS > 0.05 && folds.filter((f) => f.passed).length >= Math.ceil(effectiveFolds * 0.65);
        return {
            folds,
...
//# sourceMappingURL=walk-forward-validator.js.map