...
export type TradeOutcomeClassification = 'GOOD_TRADE_WIN' | 'GOOD_TRADE_LOSS' | 'BAD_TRADE_WIN' | 'BAD_TRADE_LOSS' | 'EXECUTION_FAILURE' | 'DATA_FAILURE' | 'MODEL_FAILURE' | 'STRATEGY_FAILURE';
export type FailureMode = 'WRONG_DIRECTION' | 'FALSE_BOS' | 'FALSE_CHOCH' | 'LIQUIDITY_MISREAD' | 'WEAK_DISPLACEMENT' | 'FAILED_ORDER_BLOCK' | 'FAILED_FVG' | 'BAD_PREMIUM_DISCOUNT' | 'HTF_CONFLICT' | 'REGIME_MISCLASSIFICATION' | 'VOLATILITY_MISREAD' | 'LOW_VOLUME' | 'LATE_ENTRY' | 'EARLY_ENTRY' | 'STOP_TOO_TIGHT' | 'TARGET_TOO_FAR' | 'BAD_RR' | 'ML_FALSE_POSITIVE' | 'ML_FALSE_NEGATIVE' | 'LLM_FALSE_POSITIVE' | 'LLM_FALSE_NEGATIVE' | 'SLIPPAGE' | 'EXECUTION_ERROR' | 'DATA_ERROR' | 'STRATEGY_FAILURE' | 'MODEL_FAILURE' | 'TIMEOUT_CHOP';
...
export type StrategyCandidateType = 'FILTER' | 'THRESHOLD' | 'FEATURE' | 'MODEL' | 'REGIME' | 'VOLATILITY' | 'EXIT' | 'POSITION_SIZE';
export type CandidateStatus = 'GENERATED' | 'BACKTESTING' | 'VALIDATED' | 'SHADOW' | 'PROMOTED' | 'REJECTED' | 'ROLLED_BACK';
...
export interface PromotionCriteria {
    ...
    minOutOfSampleExpectancyDelta: number;
    maxAllowedDrawdownIncreasePct: number;
    maxMonteCarloRuinProbability: number;
