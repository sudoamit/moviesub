...
export class LLMResearchAssistant {
  ...
  public static buildResearchPrompt(data: {
    ...
    const topLosses = data.errorReport.topLossDrivers
      .map(
        (d) =>
          `- ${d.failureMode}: ${d.count} trades, contribution: ${d.lossContributionPct}%, avg R: ${d.averageR}`,
      )
      .join('\n');
...
`.trim();
   ...
   */
  public static parseLLMHypotheses(
    llmResponse: string,
    baselineExpectancy: number,
  ): Partial<ResearchHypothesis>[] {
    const parsed: Partial<ResearchHypothesis>[] = [];

    // Fallback parser or structural extraction
    if (
      llmResponse.toLowerCase().includes('countertrend') ||
    ...
    if (
      llmResponse.toLowerCase().includes('displacement') ||
      llmResponse.toLowerCase().includes('liquidity')
    ) {
      parsed.push({
        title:
          'LLM Hypothesized: Require institutional displacement threshold on Order Block entries',
        source: 'LLM_ASSISTANT',
