...
export class ExperimentManager {
  ...
  public static calculatePerformanceMetrics(
    ...
    const grossLoss = Math.abs(
      losses.reduce((acc, t) => acc + (t.pnl < 0 ? t.pnl : t.pnlR * 1000), 0),
    );
    ...
    const downsideVariance =
      rValues.filter((r) => r < 0).reduce((a, b) => a + Math.pow(b, 2), 0) /
    ...
    const calmarRatio =
      maxDrawdownPercent > 0 ? Number(((expectancy * 100) / maxDrawdownPercent).toFixed(2)) : 5.0;
   ...
   */
  public static runSlippageStress(
    trades: { pnlR: number; pnl: number; isWin: boolean }[],
  ): SlippageStressResult[] {
   ...
   */
  public static runMissedTradeStress(
    trades: { pnlR: number; pnl: number; isWin: boolean }[],
  ): MissedTradeStressResult[] {
    const results: MissedTradeStressResult[] = [];
   ...
   */
  public static calculateComplexityPenalty(
    rulesCount: number,
    ...
    if (totalComponents <= 4) return 0.0;
  ...
  public static async executeExperiment(
    ...
    const candidateStrategyVersion =
      options.candidateStrategyVersion || `v2.1-exp-${Date.now().toString(36)}`;
    ...
    const experimentHash = ExperimentRegistry.computeExperimentHash({
    ...
    const cachedExp = ExperimentRegistry.findExperimentByHash(experimentHash);
    if (cachedExp) {
      return cachedExp;
    }

    // 3. Simulate Baseline Trades
    const baselineSim = BacktestSimulator.runSimulation({
      symbol: instrument,
      timeframe,
    ...
    const baselineMetrics = this.calculatePerformanceMetrics(
      baselineSim.trades.map((t) => ({
    ...
    const candidateSim = BacktestSimulator.runSimulation({
      symbol: instrument,
      timeframe,
      candles: testCandles.length > 0 ? testCandles : candles,
    ...
    const slippageSurvivalScore =
      slippageStressScenarios.filter((s) => s.isProfitable).length / slippageStressScenarios.length;
    const missedTradeSurvivalScore =
      missedTradeScenarios.filter((s) => s.isProfitable).length / missedTradeScenarios.length;

    const robustnessMetrics: RobustnessMetrics = {
      walkForwardDegradationPct: Math.max(
        0,
        Number(
          (
            ((baselineMetrics.expectancy - candidateMetrics.expectancy) /
    ...
    const passedOutOfSample =
      candidateMetrics.expectancy > baselineMetrics.expectancy &&
      candidateMetrics.profitFactor >= 1.3;
    const passedHoldout =
      robustnessMetrics.monteCarloRuinProbability <= 0.02 && slippageSurvivalScore >= 0.66;

    const rejectionReasons: string[] = [];
    if (!passedOutOfSample)
      rejectionReasons.push(
        'Candidate did not outperform baseline expectancy on out-of-sample data.',
      );
    if (robustnessMetrics.monteCarloRuinProbability > 0.02)
      rejectionReasons.push(
        `Monte Carlo ruin probability (${robustnessMetrics.monteCarloRuinProbability}) exceeded 2% safety threshold.`,
      );
    if (slippageSurvivalScore < 0.66)
      rejectionReasons.push('Candidate collapsed under 2x/3x slippage stress testing.');

    let status: ExperimentStatus = 'PASSED';
    ...
    ExperimentRegistry.registerExperiment(experiment);
    return experiment;
  }
}
