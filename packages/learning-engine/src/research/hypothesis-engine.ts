...
export class HypothesisEngine {
   ...
   */
  public static validateHypothesisStructure(hypothesis: ResearchHypothesis): boolean {
    ...
    if (!hypothesis.population) {
      throw new Error(
        'Hypothesis must specify a target population (e.g. "NIFTY 15m", "BTCUSDT 15m").',
      );
    }
    if (hypothesis.sampleSize < 5) {
      throw new Error(
        'Hypothesis must have a minimum empirical sample size of at least 5 observations.',
      );
    }
   ...
   */
  public static mineHypothesesFromExperiences(
    experiences: TradingExperience[],
  ): ResearchHypothesis[] {
    const hypotheses: ResearchHypothesis[] = [];
    if (!experiences || experiences.length === 0) return hypotheses;

    const totalTrades = experiences.length;
    const overallExpectancy =
      experiences.reduce((acc, e) => acc + (e.outcome?.pnlR || 0), 0) / totalTrades;

    // 1. HTF Conflict Analysis
    const htfConflicted = experiences.filter(
      (e) =>
    ...
    if (htfConflicted.length >= 8) {
      const htfExpectancy =
        htfConflicted.reduce((acc, e) => acc + (e.outcome?.pnlR || 0), 0) / htfConflicted.length;
      if (htfExpectancy < 0) {
        const delta = Math.abs(htfExpectancy - overallExpectancy);
        hypotheses.push({
          id: `hyp_htf_conflict_${Date.now()}`,
          title:
            'Reject setups where execution direction conflicts with Higher Timeframe (HTF) market bias',
    ...
    const highVolCountertrend = experiences.filter(
      (e) =>
        (e.marketContext?.volatilityRegime === 'HIGH_VOLATILITY' ||
          e.marketContext?.volatilityRegime === 'EXPANDING') &&
        e.failureReasons?.includes('VOLATILITY_MISREAD'),
    );
    if (highVolCountertrend.length >= 6) {
      const volExpectancy =
        highVolCountertrend.reduce((acc, e) => acc + (e.outcome?.pnlR || 0), 0) /
        highVolCountertrend.length;
      hypotheses.push({
        id: `hyp_high_vol_countertrend_${Date.now()}`,
        title:
          'Filter out countertrend mean-reversion entries during extreme expanding volatility regimes',
        source: 'REGIME_STATS',
        condition:
          'IF volatility_regime == HIGH_VOLATILITY AND trade_type == COUNTERTREND THEN suppress signal',
        population: 'ALL_INSTRUMENTS',
    ...
    const weakDisplacementLosses = experiences.filter(
      (e) =>
        e.failureReasons?.includes('WEAK_DISPLACEMENT') ||
    ...
    const highConfLosses = experiences.filter(
      (e) =>
        (e.prediction?.probabilityWin || 0) >= 0.75 &&
    ...
    if (highConfLosses.length >= 5) {
      hypotheses.push({
        ...
        title:
          'Recalibrate Platt Scaler and tighten entry gating when ML prediction exceeds 0.80 during consolidation regimes',
        source: 'ML_ERROR',
        condition:
          'IF ML_probability >= 0.80 AND regime == RANGE THEN apply Platt scaling dampener',
        population: 'ALL_INSTRUMENTS',
