...
exports.ScorecardEngine = void 0;
...
class ScorecardEngine {
    ...
    static generateSelfImprovementScorecard(data) {
        const hypotheses = research_memory_1.ResearchMemory.listHypotheses();
        const experiments = experiment_registry_1.ExperimentRegistry.listExperiments();
        ...
        const candidatesRejected = experiments.filter((e) => e.status === 'REJECTED' || e.status === 'FAILED').length;
        // Find best candidate
        const passedExperiments = experiments.filter((e) => e.passedOutOfSample);
        const bestExp = passedExperiments.sort((a, b) => b.candidateMetrics.expectancy - a.candidateMetrics.expectancy)[0];
        const expectedImprovementDeltaR = bestExp
            ? Number((bestExp.candidateMetrics.expectancy - bestExp.baselineMetrics.expectancy).toFixed(3))
            : 0.0;
        let systemState = 'IMPROVING';
     ...
     */
    static generateLearningScorecard() {
...
exports.ScorecardEngine = ScorecardEngine;
