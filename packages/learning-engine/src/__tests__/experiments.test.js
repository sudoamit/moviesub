...
describe('DatasetManager & Quantitative Experiment System', () => {
    ...
    it('should create an immutable hashed dataset and partition into chronological Train, Validation, and OOS splits', () => {
        const manager = new dataset_manager_1.DatasetManager();
    ...
    it('should run a reproducible quantitative experiment with cost stress testing and Monte Carlo simulation', () => {
        ...
        const exp = experiments_1.ExperimentRunner.runExperiment('NIFTY_SMC_Base', splits, { minScore: 70 }, 12345);
        ...
        const expManager = new experiments_1.QuantExperimentManager();
    ...
    it('should compare candidate and baseline experiments statistically', () => {
        ...
        const dataset = manager.createDataset('NIFTY', '15m', mockSamples);
        const splits = manager.splitDataset(dataset.metadata.datasetId);
        const baseline = experiments_1.ExperimentRunner.runExperiment('Baseline', splits, { minScore: 60 });
        const candidate = experiments_1.ExperimentRunner.runExperiment('Candidate_Optimized', splits, {
            minScore: 75,
        });
        const comparison = experiments_1.ExperimentComparator.compare(baseline, candidate);
