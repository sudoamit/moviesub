...
describe('Learning Engine Correctness & Self-Improvement Regression Suite (Phases 1 - 29)', () => {
    ...
    test('Test 1: Point-in-time feature isolation — future candles do not affect features at decision T', () => {
        ...
        const valResult = index_1.PointInTimeValidator.validatePointInTimeExperience(expA);
    ...
    test('Test 3: Temporal Dataset Builder enforces max(train.ts) < min(val.ts) < min(oos.ts)', () => {
        const ds = new index_1.DatasetManager();
    ...
    test('Test 4: OOS data modifications cannot influence training fold parameters', () => {
        const scaler1 = new index_1.TemporalFeatureScaler();
        const trainData = [
        ...
        const scaler2 = new index_1.TemporalFeatureScaler();
    ...
    test('Test 5: Candidate with parameter change actually modifies simulated execution results', () => {
        ...
        const result = index_1.CandidateEvaluator.evaluate(baseCandidate, experiences);
    ...
    test('Test 6: Candidate evaluation reports actual simulated expectancy, not synthetic formulas', () => {
        ...
        const result = index_1.CandidateEvaluator.evaluate(candidate, experiences);
        // Measured expectancy: (-1.0 * 0.5 - 0.05) = -0.55R per trade
        expect(result.candidateExpectancy).toBe(-0.55);
    ...
    test('Test 7: Two candidates with different trade sequences produce distinct Monte Carlo results', () => {
        ...
        const mcA = index_1.MonteCarloEngine.simulate(candARMultiples, { seed: 100, ruinThresholdDrawdownR: 2.0 });
        const mcB = index_1.MonteCarloEngine.simulate(candBRMultiples, { seed: 100, ruinThresholdDrawdownR: 2.0 });
    ...
    test('Test 8: PromotionGate blocks candidate with missing or insufficient shadow trade evidence', () => {
        ...
        const res1 = index_1.PromotionGate.evaluateCandidate(candidateNoShadow);
        expect(res1.approved).toBe(false);
        expect(candidateNoShadow.status).toBe('REJECTED');
        const candidateLowShadow = {
            ...candidateNoShadow,
            status: 'SHADOW',
            shadowMetrics: {
        ...
        const res2 = index_1.PromotionGate.evaluateCandidate(candidateLowShadow);
    ...
    test('Test 9: Model and Strategy registries promote without mutating frozen objects', () => {
        const initialStrat = index_1.StrategyRegistry.getActiveStrategy();
        ...
        index_1.StrategyRegistry.registerStrategy(newStrat);
        index_1.StrategyRegistry.promoteStrategy(newStrat.strategyVersion);
        const activeStrat = index_1.StrategyRegistry.getActiveStrategy();
    ...
    test('Test 10: RollbackManager restores previous stable version dynamically', () => {
        ...
        const recentExperiences = Array.from({ length: 20 }, (_, i) => ({
            id: `exp_deg_${i}`,
            tradeId: `t_deg_${i}`,
            timestamp: new Date(1700000000000 + i * 60000),
            instrument: { symbol: 'BTCUSDT', assetType: 'CRYPTO' },
        ...
        const rbRes = index_1.RollbackManager.checkAndExecuteRollback(recentExperiences, 15);
        expect(rbRes.shouldRollback).toBe(true);
        expect(index_1.StrategyRegistry.getActiveStrategy()?.status).toBe('ACTIVE');
    });
    ...
    test('Test 11: PointInTimeValidator rejects experiences where future label starts before decision', () => {
        ...
        const valRes = index_1.PointInTimeValidator.validatePointInTimeExperience(invalidExp);
    ...
    test('Test 12: Extreme OOS values do not mutate pre-fitted training scaler statistics', () => {
        const scaler = new index_1.TemporalFeatureScaler();
        const trainData = [
    ...
    test('Test 13: Walk-forward validator processes folds with distinct training windows', () => {
        ...
        const wfRes = index_1.WalkForwardValidator.validate(cand, experiences, { numFolds: 3 });
    ...
    test('Test 14: Seeded PRNG produces 100% deterministic Monte Carlo results for identical inputs', () => {
        ...
        const res1 = index_1.MonteCarloEngine.simulate(rMultiples, { seed: 12345, iterations: 500 });
        const res2 = index_1.MonteCarloEngine.simulate(rMultiples, { seed: 12345, iterations: 500 });
    ...
    test('Test 15: CandidateGenerator output contains baseline expectancy before actual evaluation', () => {
        ...
        const candidates = index_1.CandidateGenerator.generateCandidates(inputs);
    ...
    test('Test 17: Datasets with identical timestamps but different feature values produce different dataHashes', () => {
        const ds1 = new index_1.DatasetManager();
        const samplesA = [
        ...
        const ds2 = new index_1.DatasetManager();
        const samplesB = [
    ...
    test('Test 19: PromotionGate rejects auto-promotion when allowAutoPromotion is false', () => {
        ...
        const resNoAuto = index_1.PromotionGate.evaluateCandidate(candidate, {
            ...index_1.PromotionGate.DEFAULT_CRITERIA,
    ...
    test('Test 20: WalkForwardValidator fits candidate parameters per fold, producing fold-specific artifacts', () => {
        ...
        const wfRes = index_1.WalkForwardValidator.validate(baseCand, experiences, { numFolds: 3 });
        expect(wfRes.folds.length).toBeGreaterThan(0);
    ...
    test('Test 21: MonteCarloEngine throws INSUFFICIENT_CANDIDATE_EXECUTION_RESULTS on empty R-multiples', () => {
        expect(() => {
            index_1.MonteCarloEngine.simulate([]);
...
//# sourceMappingURL=learning-engine-correctness.test.js.map