...
export class VolatilityPerformanceAnalyzer {
  /**
   * Tracks volatility forecasting accuracy and strategy performance across volatility regimes.
   */
  public static analyze(experiences: TradingExperience[]): IVolatilityBucketStats[] {
    const buckets: Map<string, TradingExperience[]> = new Map();

    for (const exp of experiences) {
      const b =
        exp.marketContext.volatilityRegime ||
    ...
    const results: IVolatilityBucketStats[] = [];
    ...
    for (const [bucket, exps] of buckets.entries()) {
      ...
      const rmse =
        validVolCount > 0 ? Number(Math.sqrt(sumSqDiff / validVolCount).toFixed(4)) : 0.0035;
