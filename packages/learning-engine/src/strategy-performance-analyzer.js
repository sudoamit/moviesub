...
class StrategyPerformanceAnalyzer {
    ...
    static analyze(experiences) {
        ...
        const totalHoldingSeconds = experiences.reduce((sum, e) => sum + e.outcome.holdingTimeSeconds, 0);
        const averageHoldingTimeMinutes = Math.round(totalHoldingSeconds / totalTrades / 60);
        const goodLosses = experiences.filter((e) => e.outcomeClassification === 'GOOD_TRADE_LOSS').length;
        const goodTradeLossRatio = lossCount > 0 ? Number(((goodLosses / lossCount) * 100).toFixed(1)) : 0;
...
//# sourceMappingURL=strategy-performance-analyzer.js.map