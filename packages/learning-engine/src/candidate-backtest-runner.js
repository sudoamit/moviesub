...
exports.CandidateBacktestRunner = void 0;
...
class CandidateBacktestRunner {
     ...
     */
    static createExecutionConfig(candidate) {
        const change = candidate.change || {};
        const minMtfScore = change.parameter === 'minMtfScore'
            ? (typeof change.value === 'number' ? change.value : change.fittedValue)
            : change.minScore;
        const stopLossAtrMultiplier = change.parameter === 'stopLossAtrMultiplier' ? change.value : 1.0;
        const sizingMultiplier = change.parameter === 'sizingMultiplier'
            ? change.value
            : change.sizingMultiplier || 1.0;
        const highVolatilitySizingMultiplier = change.highVolatilitySizingMultiplier ||
            (change.parameter === 'highVolatilitySizingMultiplier' ? change.value : undefined);
        const minProbability = change.minProbability ||
            (change.parameter === 'minProbability' ? change.value : undefined);
        const filterRegime = change.filterRegime || (change.parameter === 'filterRegime' ? change.value : undefined);
        const conditionRules = change.conditionRules || [];
     ...
     */
    static runCandidateBacktest(candidate, experiences) {
        const config = this.createExecutionConfig(candidate);
        const execSim = new backtesting_1.ExecutionSimulator(backtesting_1.FillModel.OHLC_PATH, backtesting_1.SameCandleAmbiguityMode.OHLC_PATH, { submissionLatencyMs: 15, processingLatencyMs: 5 }, `cand_${candidate.id}`);
        ...
        for (let idx = 0; idx < experiences.length; idx++) {
            ...
            if ((exp.marketContext?.regime === 'HIGH_VOLATILITY' || exp.marketContext?.volatilityRegime === 'HIGH') &&
            ...
            const candles = exp.candlesDuringTrade || [];
            ...
            if (candles && candles.length > 0 && stopDist > 0) {
                ...
                if (fillEntry.length > 0) {
                    ...
                    executedTrades.push({
                        id: `tr_${exp.id}`,
                        direction: isLong ? shared_1.Direction.BULLISH : shared_1.Direction.BEARISH,
                        ...
                        pnl,
                        pnlRMultiple: pnlR,
                        exitReason: isWin ? shared_1.SignalState.TP1_HIT : stoppedOut ? shared_1.SignalState.SL_HIT : shared_1.SignalState.EXPIRED,
                ...
                executedTrades.push({
                    ...
                    pnl,
                    pnlRMultiple: pnlR,
                    exitReason: exp.outcome?.status === 'WIN' ? shared_1.SignalState.TP1_HIT : shared_1.SignalState.SL_HIT,
                    signalTimestamp: new Date(exp.decisionTimestamp || entryTime),
                    orderCreatedAt: new Date(entryTime),
                    orderSubmittedAt: new Date(entryTime),
                    entryFillTimestamp: new Date(entryTime),
...
exports.CandidateBacktestRunner = CandidateBacktestRunner;
//# sourceMappingURL=candidate-backtest-runner.js.map