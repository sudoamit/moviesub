...
const trade_lifecycle_manager_1 = require("../trade-lifecycle-manager");
...
describe('TradeLifecycleManager', () => {
    ...
    it('should activate a pending signal when price enters entry zone', () => {
        ...
        const update = trade_lifecycle_manager_1.TradeLifecycleManager.evaluateTick(signal, tickCandle);
        expect(update.newState).toBe(shared_1.SignalState.ACTIVE);
        expect(update.isClosed).toBe(false);
    });
    it('should trigger SL_HIT when price drops below stop loss', () => {
        const activeSignal = { ...signal, state: shared_1.SignalState.ACTIVE };
        const tickCandle = {
            timestamp: new Date(),
            open: 24950,
            high: 24960,
        ...
        const update = trade_lifecycle_manager_1.TradeLifecycleManager.evaluateTick(activeSignal, tickCandle);
        expect(update.newState).toBe(shared_1.SignalState.SL_HIT);
    ...
    it('should trigger TP1_HIT when price reaches Target 1', () => {
        ...
        const update = trade_lifecycle_manager_1.TradeLifecycleManager.evaluateTick(activeSignal, tickCandle);
        expect(update.newState).toBe(shared_1.SignalState.TP1_HIT);
    ...
    describe('PositionLot Multi-Tier Lifecycle & Partial Scale-Out', () => {
        it('should create an immutable PositionLot with initial execution event', () => {
            const lot = trade_lifecycle_manager_1.TradeLifecycleManager.createPositionLot(signal, 25000, 100, 1756972800000);
        ...
        it('should scale out 30% at TP1 and move stop loss to breakeven', () => {
            ...
            const res = trade_lifecycle_manager_1.TradeLifecycleManager.evaluateLotTick(lot, tp1Candle, trade_lifecycle_manager_1.DEFAULT_PARTIAL_EXIT_POLICY);
        ...
        it('should track MAE and MFE correctly across ticks', () => {
            ...
            trade_lifecycle_manager_1.TradeLifecycleManager.evaluateLotTick(lot, adverseCandle);
            expect(lot.mae).toBe(60);
            expect(lot.mfe).toBe(80);
        });
    });
});
//# sourceMappingURL=trade-lifecycle-manager.test.js.map