...
describe('PortfolioRiskManager', () => {
    it('should allow valid new position within total risk and capacity limits', () => {
        ...
        const status = portfolio_risk_manager_1.PortfolioRiskManager.validateNewPosition(100000, openPositions, proposed, 'EQUITY');
    ...
    it('should reject new position when total open risk exceeds maximum limit', () => {
        ...
        const status = portfolio_risk_manager_1.PortfolioRiskManager.validateNewPosition(100000, openPositions, proposed, 'CRYPTO', { maxOpenRiskPercent: 6.0 });
        expect(status.isAllowed).toBe(false);
        expect(status.rejectionReason).toContain('breach portfolio max open risk limit');
    });
});
//# sourceMappingURL=portfolio-risk-manager.test.js.map