...
exports.TradeLifecycleManager = exports.DEFAULT_PARTIAL_EXIT_POLICY = void 0;
const shared_1 = require("@quant/shared");
exports.DEFAULT_PARTIAL_EXIT_POLICY = {
...
class TradeLifecycleManager {
     ...
     */
    static validatePartialExitPolicy(policy) {
        if (!policy) {
     ...
     */
    static createPositionLot(signal, executionPrice, quantity, executionTime, orderId, entryFee = 0, entrySlippage = 0) {
        ...
        const tp1 = signal.takeProfits?.tp1 ??
            executionPrice +
                (signal.direction === shared_1.Direction.BULLISH ? riskDistance * 1.5 : -riskDistance * 1.5);
        const tp2 = signal.takeProfits?.tp2 ??
            executionPrice +
                (signal.direction === shared_1.Direction.BULLISH ? riskDistance * 2.5 : -riskDistance * 2.5);
        const tp3 = signal.takeProfits?.tp3 ??
            executionPrice +
                (signal.direction === shared_1.Direction.BULLISH ? riskDistance * 4.0 : -riskDistance * 4.0);
        const entryEvent = {
        ...
        const signalTimestamp = signal.timestamp instanceof Date
            ? signal.timestamp.getTime()
            : typeof signal.timestamp === 'number'
                ? signal.timestamp
                : executionTime;
        const entrySnapshot = Object.freeze({
            ...
            orderId,
            side: signal.direction === shared_1.Direction.BULLISH ? 'BUY' : 'SELL',
     ...
     */
    static evaluateLotTick(lot, candle, policy = exports.DEFAULT_PARTIAL_EXIT_POLICY, candleTimestamp = candle.timestamp instanceof Date
        ? candle.timestamp.getTime()
        : typeof candle.timestamp === 'number'
            ? candle.timestamp
            : Date.now(), forBacktest = false) {
        if (forBacktest) {
            throw new Error('Synthetic lifecycle evaluation (evaluateLotTick) is deprecated and disabled for backtesting. Backtests must use ExecutionSimulator.');
        }
        ...
        if (lot.status === 'CLOSED' || lot.status === 'CANCELLED' || lot.remainingQuantity <= 0) {
            return { lot, events: [], isClosed: true, state: shared_1.SignalState.EXPIRED };
        }
        ...
        if (isStopHit) {
            ...
            return {
                ...
                events: newEvents,
                isClosed: true,
                state: lot.realizedPnl >= 0 ? shared_1.SignalState.TP1_HIT : shared_1.SignalState.SL_HIT,
            };
        }
        ...
        if (isTp3Hit) {
            ...
            return { lot, events: newEvents, isClosed: true, state: shared_1.SignalState.TP3_HIT };
        }
        ...
        if (isTp2Hit && !hasAlreadyTp2) {
            ...
            return {
                ...
                state: shared_1.SignalState.TP2_HIT,
            };
        }
        ...
        if (isTp1Hit && !hasAlreadyTp1) {
            ...
            return {
                ...
                events: newEvents,
                isClosed: false,
                state: shared_1.SignalState.TP1_HIT,
            };
        }
        return {
            lot,
            events: newEvents,
            isClosed: false,
            state: hasAlreadyTp1 ? shared_1.SignalState.TP1_HIT : shared_1.SignalState.ACTIVE,
        };
    }
    /**
     * Compatibility wrapper for signal evaluation against raw candles
     */
    static evaluateTick(signal, candle) {
        const { direction, state, entryZone, stopLoss, takeProfits, riskRewardRatios, symbol } = signal;
        const isLong = direction === shared_1.Direction.BULLISH;
        ...
        // 1. Pending Signal
        if (state === shared_1.SignalState.PENDING) {
            ...
            if (isEntryHit) {
                return {
                    signalId: signal.id,
                    symbol,
                    previousState: shared_1.SignalState.PENDING,
                    newState: shared_1.SignalState.ACTIVE,
            ...
            if (isInvalidBeforeEntry) {
                return {
                    ...
                    symbol,
                    previousState: shared_1.SignalState.PENDING,
                    newState: shared_1.SignalState.INVALIDATED,
                    currentPrice,
                    pnlRMultiple: 0,
                    isClosed: true,
            ...
            return {
                ...
                symbol,
                previousState: shared_1.SignalState.PENDING,
                newState: shared_1.SignalState.PENDING,
                currentPrice,
                pnlRMultiple: 0,
                isClosed: false,
        ...
        // 2. Active Trade
        if (state === shared_1.SignalState.ACTIVE) {
            const isSlHit = isLong ? low <= stopLoss : high >= stopLoss;
            if (isSlHit) {
                return {
                    signalId: signal.id,
                    symbol,
                    previousState: shared_1.SignalState.ACTIVE,
                    newState: shared_1.SignalState.SL_HIT,
            ...
            if (isTp3Hit) {
                return {
                    ...
                    symbol,
                    previousState: shared_1.SignalState.ACTIVE,
                    newState: shared_1.SignalState.TP3_HIT,
            ...
            if (isTp2Hit) {
                return {
                    ...
                    symbol,
                    previousState: shared_1.SignalState.ACTIVE,
                    newState: shared_1.SignalState.TP2_HIT,
            ...
            if (isTp1Hit) {
                return {
                    ...
                    symbol,
                    previousState: shared_1.SignalState.ACTIVE,
                    newState: shared_1.SignalState.TP1_HIT,
        ...
        if (state === shared_1.SignalState.TP1_HIT) {
            const isTp3Hit = isLong ? high >= takeProfits.tp3 : low <= takeProfits.tp3;
            if (isTp3Hit) {
                return {
                    signalId: signal.id,
                    symbol,
                    previousState: shared_1.SignalState.TP1_HIT,
                    newState: shared_1.SignalState.TP3_HIT,
                    currentPrice: takeProfits.tp3,
                    pnlRMultiple: rr3,
                    isClosed: true,
...
exports.TradeLifecycleManager = TradeLifecycleManager;
//# sourceMappingURL=trade-lifecycle-manager.js.map