...
exports.ExecutionSafetyGate = void 0;
const shared_1 = require("@quant/shared");
const portfolio_risk_manager_1 = require("./portfolio-risk-manager");
class ExecutionSafetyGate {
    ...
    static evaluate(params) {
        ...
        const isBull = String(params.direction).toUpperCase() === 'LONG' ||
        ...
        if (!dataPass)
            rejectionReasons.push(checks['1_DATA_QUALITY'].message);
        // 2. SMC Setup Confluence Check
        const smcPass = Boolean(params.smcSetup &&
        ...
        if (!smcPass)
            rejectionReasons.push(checks['2_SMC_SETUP'].message);
        ...
        const fvComplete = Boolean(fv && fvKeys.every((k) => typeof fv[k] === 'number' && !isNaN(fv[k]) && isFinite(fv[k])));
        ...
        if (!fvComplete)
            rejectionReasons.push(checks['3_FEATURE_VECTOR'].message);
        ...
        if (!modelHealthy)
            rejectionReasons.push(checks['4_MODEL_HEALTH'].message);
        ...
        if (!calibPass)
            rejectionReasons.push(checks['5_PROBABILITY_CALIBRATION'].message);
        ...
        if (!expPass)
            rejectionReasons.push(checks['6_EXPECTANCY_R'].message);
        ...
        if (!ddPass)
            rejectionReasons.push(checks['7_DRAWDOWN_GUARD'].message);
        // 8. Portfolio Exposure & Sector Limits Check
        const portStatus = portfolio_risk_manager_1.PortfolioRiskManager.validateNewPosition(params.accountEquity, params.openPositions || [], params.positionSizing, params.instrument.includes('BTC') ? 'CRYPTO' : 'EQUITY', params.riskConfig || {});
        ...
        if (!portStatus.isAllowed)
            rejectionReasons.push(checks['8_PORTFOLIO_EXPOSURE'].message);
        // 9. Position Sizing Validation Check
        const sizePass = Boolean(params.positionSizing &&
        ...
        if (!sizePass)
            rejectionReasons.push(checks['9_POSITION_SIZING'].message);
        ...
        if (!slSidePass)
            rejectionReasons.push(checks['10_STOP_LOSS_STRUCTURE'].message);
        ...
        if (!sessionPass)
            rejectionReasons.push(checks['11_MARKET_SESSION'].message);
        // 12. No Conflicting Open Position Check
        const conflicting = (params.openPositions || []).find((p) => p.symbol.toUpperCase() === params.instrument.toUpperCase() &&
            (isBull ? p.direction !== shared_1.Direction.BULLISH : p.direction !== shared_1.Direction.BEARISH));
        ...
        if (!noConflictPass)
            rejectionReasons.push(checks['12_NO_CONFLICTING_POSITION'].message);
...
exports.ExecutionSafetyGate = ExecutionSafetyGate;
//# sourceMappingURL=execution-safety-gate.js.map