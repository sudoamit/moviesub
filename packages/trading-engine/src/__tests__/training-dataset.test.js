...
function createMockSignal(direction = shared_1.Direction.BULLISH, entryTime = new Date('2026-08-20T04:00:00.000Z')) {
...
describe('PHASE 2: Training Dataset & Labels', () => {
    ...
    describe('1. TradeLabelGenerator — Deterministic Outcomes', () => {
        it('generates label = 1 (+2.5R) when a LONG trade reaches Target before Stop Loss', () => {
            ...
            const outcome = ai_trade_learning_engine_1.TradeLabelGenerator.evaluateOutcome({
    ...
    describe('2. Ambiguous Same-Bar Breach Policy Handling', () => {
        ...
        it('returns label = null under default AMBIGUOUS policy (excluded from dataset)', () => {
            const outcome = ai_trade_learning_engine_1.TradeLabelGenerator.evaluateOutcome({
                ...
                ambiguousPolicy: ai_trade_learning_engine_1.AmbiguousLabelPolicy.AMBIGUOUS,
            });
            expect(outcome).not.toBeNull();
        ...
        it('returns label = 0 under CONSERVATIVE policy (treated as loss)', () => {
            const outcome = ai_trade_learning_engine_1.TradeLabelGenerator.evaluateOutcome({
                ...
                entryTimestamp: entryTime,
                subsequentCandles: [ambiguousCandle],
                ambiguousPolicy: ai_trade_learning_engine_1.AmbiguousLabelPolicy.CONSERVATIVE,
            });
            expect(outcome).not.toBeNull();
            expect(outcome.isAmbiguous).toBe(true);
            expect(outcome.label).toBe(0);
    ...
    describe('3. TrainingDatasetBuilder & Causality Verification', () => {
        it('builds a verified training example with availableForTrainingAt strictly > predictionTimestamp', () => {
            const signal = createMockSignal(shared_1.Direction.BULLISH, entryTime);
            ...
            const example = ai_trade_learning_engine_1.TrainingDatasetBuilder.buildExample({
                id: 'example-1',
                signal,
            ...
            expect(example.featureSchemaVersion).toBe(ai_trade_learning_engine_1.FEATURE_SCHEMA_VERSION);
        ...
        it('strictly preserves chronological time series ordering without shuffling', () => {
            ...
            const sorted = ai_trade_learning_engine_1.TrainingDatasetBuilder.sortChronologically(examples);
            expect(sorted[0].id).toBe('ex-1');
            expect(sorted[1].id).toBe('ex-2');
            expect(sorted[2].id).toBe('ex-3');
            const validation = ai_trade_learning_engine_1.TrainingDatasetBuilder.validateDataset(sorted);
        ...
        it('detects and flags temporal causality leaks in datasets', () => {
            ...
            const validation = ai_trade_learning_engine_1.TrainingDatasetBuilder.validateDataset(invalidDataset);
            expect(validation.isValid).toBe(false);
            expect(validation.errors.some((e) => e.includes('causality leak'))).toBe(true);
        });
    });
});
//# sourceMappingURL=training-dataset.test.js.map