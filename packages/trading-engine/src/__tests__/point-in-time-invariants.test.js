...
describe('Point-in-Time Correctness & Look-Ahead Invariants Suite', () => {
    ...
    describe('Invariant 1: FVG Point-in-Time State', () => {
        it('evaluates FVG state incrementally so future candles cannot mutate historical state at T1', () => {
            ...
            // Analyze at T1
            const fvgAtT1_before = index_1.FVGEngine.detectFVGs(t1Candles, { asOfTimestamp: t1 });
            ...
            const fvgAtT1_after = index_1.FVGEngine.detectFVGs(t2Candles, { asOfTimestamp: t1 });
            ...
            const fvgAtT2 = index_1.FVGEngine.detectFVGs(t2Candles, { asOfTimestamp: t2 });
    ...
    describe('Invariant 2: Order Block Point-in-Time State', () => {
        it('evaluates OB confirmation and mitigation strictly point-in-time', () => {
            ...
            const obAtT1_before = index_1.OrderBlockEngine.detectOrderBlocks(t1Candles, [], [], {
            ...
            expect(obAtT1_before.allOrderBlocks[0].confirmedAtTimestamp).toEqual(new Date(t1Candles[3].timestamp));
            ...
            const obAtT1_after = index_1.OrderBlockEngine.detectOrderBlocks(t2Candles, [], [], {
                displacementThresholdAtr: 0.5,
                asOfTimestamp: t1,
            });
            ...
            // Evaluate at T2
            const obAtT2 = index_1.OrderBlockEngine.detectOrderBlocks(t2Candles, [], [], {
                displacementThresholdAtr: 0.5,
                asOfTimestamp: t2,
            });
    ...
    describe('Invariant 3: MTF Closed-Candle Isolation', () => {
        it('strictly forbids unclosed HTF bars from leaking into lower timeframe analysis', () => {
            ...
            const filteredAt1015 = index_1.MultiTimeframeAnalyzer.filterClosedHTFCandles(htfCandles, shared_1.Timeframe.H1, asOf1015.getTime());
            ...
            const filteredAt1100 = index_1.MultiTimeframeAnalyzer.filterClosedHTFCandles(htfCandles, shared_1.Timeframe.H1, asOf1100.getTime());
    ...
    describe('Invariant 4: Dealing Range & Trend Point-in-Time', () => {
        it('dealing range at T uses only structural levels confirmed by T', () => {
            ...
            const rangeAtT = index_1.DealingRangeEngine.calculateDealingRange(pastSwings);
            ...
            const rangeConfirmedAtT = index_1.DealingRangeEngine.calculateDealingRange(swingsConfirmedAtT);
    ...
    describe('Invariant 5: Full Immutability Regression Test', () => {
        it('produces identical analysis at timestamp T whether analyzed in real-time or from expanded future dataset', () => {
            ...
            const resultAtT_realtime = index_1.SMCAnalyzer.analyze(baseCandles, { asOfTimestamp: tDecision });
            // Create expanded dataset with 30 additional future candles
            const futureExpandedCandles = [...baseCandles];
            for (let i = 40; i < 70; i++) {
                futureExpandedCandles.push(createCandle(i, 120 + (i % 5), 125 + (i % 5), 115 + (i % 5), 122 + (i % 5)));
            }
            // Re-run analysis on expanded future dataset asking for state asOf tDecision
            const resultAtT_historical = index_1.SMCAnalyzer.analyze(futureExpandedCandles, {
            ...
            expect(resultAtT_historical.breaksOfStructure.length).toBe(resultAtT_realtime.breaksOfStructure.length);
    ...
    describe('Invariant 6: Future-Data Extreme Shock Resistance', () => {
        it('historical signals and scores at T remain completely unaltered even if future price experiences +20% / -20% shocks', () => {
            ...
            const signalAtT_original = index_1.SignalGenerator.generateSignal({
                symbol: 'NIFTY',
            ...
            const signalAtT_shocked = index_1.SignalGenerator.generateSignal({
                symbol: 'NIFTY',
    ...
    describe('Invariant 7: SnapshotBuilder Neutral Direction Preservation', () => {
        it('preserves NEUTRAL direction in flat/ranging markets without fabricating a BULLISH fallback', () => {
            ...
            const snapshot = index_1.SnapshotBuilder.buildSnapshot({
                symbol: 'NIFTY',
                executionCandles: flatCandles,
                executionTimeframe: shared_1.Timeframe.M15,
                asOfTimestamp: tEnd,
            });
            expect(snapshot.trace.smc.bias).toBe(shared_1.Direction.NEUTRAL);
            expect(snapshot.trace.finalDecision).toBe('NO_TRADE');
            expect(snapshot.score.grade).toBe(shared_1.SignalGrade.NO_TRADE);
        });
    });
    ...
    describe('Invariant 8: MTF Recomputation Immunity', () => {
        it('always recomputes MTF analysis and never reuses precomputed analysis across different datasets', () => {
            ...
            const resultA = index_1.MultiTimeframeAnalyzer.analyzeMTF({ timeframe: shared_1.Timeframe.H1, candles: execCandles }, { timeframe: shared_1.Timeframe.H1, candles: datasetA });
            const resultB = index_1.MultiTimeframeAnalyzer.analyzeMTF({ timeframe: shared_1.Timeframe.H1, candles: execCandles }, { timeframe: shared_1.Timeframe.H1, candles: datasetB });
            expect(resultA.htf1Trend).toBe(shared_1.Direction.BULLISH);
            expect(resultB.htf1Trend).toBe(shared_1.Direction.BEARISH);
        });
    });
    // 9. Reusable Point-in-Time Invariant Assertion Helper
    describe('Invariant 9: assertPointInTimeInvariant Helper', () => {
        function assertPointInTimeInvariant(candles, asOfTimestamp, analyzer, keySelector = (res) => res) {
        ...
        it('validates SMC, MTF, MultiHorizon, SnapshotBuilder and SignalGenerator using assertPointInTimeInvariant', () => {
            ...
            // SMC
            assertPointInTimeInvariant(baseCandles, tAsOf, (c, t) => index_1.SMCAnalyzer.analyze(c, { asOfTimestamp: t }), (r) => ({ trend: r.currentTrend, swings: r.swingPoints.length, bos: r.breaksOfStructure.length }));
            // SnapshotBuilder
            assertPointInTimeInvariant(baseCandles, tAsOf, (c, t) => index_1.SnapshotBuilder.buildSnapshot({ symbol: 'NIFTY', executionCandles: c, asOfTimestamp: t }), (r) => ({ decision: r.trace.finalDecision, bias: r.trace.smc.bias, score: r.score.totalScore }));
            // SignalGenerator
            assertPointInTimeInvariant(baseCandles, tAsOf, (c, t) => index_1.SignalGenerator.generateSignal({ symbol: 'NIFTY', executionCandles: c, asOfTimestamp: t }), (r) => ({ dir: r.direction, score: r.score, grade: r.grade, reasons: r.reasons }));
        });
    });
    ...
    describe('Invariant 10: Mandatory Incremental Replay Test', () => {
        it('verifies process(C1..Ci) === analyze(C1..Cn, asOfTimestamp=Ti) for every closed timestamp', () => {
            ...
            for (let i = 15; i < fullCandles.length; i++) {
                ...
                const incremental = index_1.SMCAnalyzer.analyze(sliced, { asOfTimestamp: ti });
                const batch = index_1.SMCAnalyzer.analyze(fullCandles, { asOfTimestamp: ti });
    ...
    describe('Invariant 11: Future-Data Torture Test', () => {
        it('guarantees zero historical leakage under wild future structural events', () => {
            ...
            const baselineSnapshot = index_1.SnapshotBuilder.buildSnapshot({
                symbol: 'NIFTY',
            ...
            const torturedSnapshot = index_1.SnapshotBuilder.buildSnapshot({
                symbol: 'NIFTY',
    ...
    describe('Invariant 12: MultiHorizon Future Invariance', () => {
        it('evaluates MultiHorizon identically regardless of future candles appended across all timeframes', () => {
            ...
            const resA = index_1.MultiHorizonEngine.evaluateMultiHorizon(history, htfHistory, macroHistory, {
                asOfTimestamp: t,
                executionTimeframe: '15m',
                htfTimeframe: '1h',
                macroTimeframe: '4h',
            });
            const resB = index_1.MultiHorizonEngine.evaluateMultiHorizon([...history, ...futureExec], [...htfHistory, ...futureHTF], [...macroHistory, ...futureMacro], {
                asOfTimestamp: t,
                executionTimeframe: '15m',
                htfTimeframe: '1h',
                macroTimeframe: '4h',
    ...
    describe('Invariant 13: MTF Stale Analysis Rejection', () => {
        it('ignores stale precomputed analysis when candles are passed', () => {
            ...
            const result = index_1.MultiTimeframeAnalyzer.analyzeMTF({ timeframe: shared_1.Timeframe.H1, candles: execCandles }, { timeframe: shared_1.Timeframe.H1, candles: datasetA, analysis: staleBearishAnalysis });
            // Must recompute and evaluate to BULLISH from datasetA, ignoring staleBearishAnalysis
            expect(result.htf1Trend).toBe(shared_1.Direction.BULLISH);
        });
    });
    ...
    describe('Invariant 14: Unclosed Candle Rejection', () => {
        it('excludes candles that have not closed as of the decision timestamp', () => {
            ...
            const closed = index_1.CandleNormalizer.getClosedCandlesAsOf(candles, '15m', asOf1020);
    ...
    describe('Invariant 15: SnapshotBuilder Integration Future Invariance', () => {
        it('guarantees all decision-relevant snapshot fields are identical regardless of future data', () => {
            ...
            const snapshotA = index_1.SnapshotBuilder.buildSnapshot({
                symbol: 'NIFTY',
                executionCandles: history,
                asOfTimestamp: tBaseline,
            });
            ...
            const snapshotB = index_1.SnapshotBuilder.buildSnapshot({
                symbol: 'NIFTY',
                executionCandles: tortureCandles,
                asOfTimestamp: tBaseline,
            });
    ...
    describe('Test C: Stale MTF Precomputed Analysis Rejection', () => {
        it('returns NEUTRAL and ignores caller-supplied fake bullish precomputed analysis when no valid candles exist at asOf', () => {
            ...
            const res = index_1.MultiTimeframeAnalyzer.analyzeMTF({ timeframe: shared_1.Timeframe.M15, candles: [] }, { timeframe: shared_1.Timeframe.H1, candles: [futureHtfCandle], analysis: fakeAnalysis }, undefined, shared_1.MTFMode.BALANCED, asOf);
            expect(res.htf1Trend).toBe(shared_1.Direction.NEUTRAL);
            expect(res.htfBias).toBe(shared_1.Direction.NEUTRAL);
        });
    });
    ...
    describe('Test D: Unclosed Candle Temporal Boundary', () => {
        it('excludes 10:00 candle at 10:05 asOf, and includes it at 10:15 if isClosed !== false', () => {
            ...
            const closedAt1005 = index_1.CandleNormalizer.getClosedCandlesAsOf([candle], '15m', asOf1005);
            expect(closedAt1005).toHaveLength(0);
            const asOf1015 = new Date('2026-01-01T10:15:00.000Z');
            const closedAt1015 = index_1.CandleNormalizer.getClosedCandlesAsOf([candle], '15m', asOf1015);
    ...
    describe('Test E: Full Snapshot Future Invariance', () => {
        it('snapshot(history, T) equals snapshot(history + arbitraryFutureData, T)', () => {
            ...
            const T = index_1.CandleNormalizer.getCandleCloseTimestamp(history[35], '15m');
            const snapshotA = index_1.SnapshotBuilder.buildSnapshot({
                symbol: 'NIFTY',
                executionCandles: history,
                asOfTimestamp: T,
    ...
    describe('Test F: Signal Future-Data Invariance', () => {
        it('SignalGenerator.generateSignal(D, T) equals SignalGenerator.generateSignal(D+F, T) for all decision-relevant fields', () => {
            ...
            const T = index_1.CandleNormalizer.getCandleCloseTimestamp(D[35], '15m');
            const signalA = index_1.SignalGenerator.generateSignal({
                symbol: 'BTCUSDT',
                executionCandles: D,
                asOfTimestamp: T,
            });
            ...
            const signalB = index_1.SignalGenerator.generateSignal({
                symbol: 'BTCUSDT',
    ...
    describe('Test G: Incremental Replay Equivalence', () => {
        it('produces identical states between incremental history feed and full batch with asOfTimestamp', () => {
            const allCandles = [];
            for (let i = 0; i < 50; i++) {
                allCandles.push(createCandle(i, 100 + Math.sin(i / 3) * 5, 105 + Math.sin(i / 3) * 5, 95 + Math.sin(i / 3) * 5, 102 + Math.sin(i / 3) * 5, 1000 + i * 10, 15 * 60 * 1000));
            }
            for (let i = 20; i < 45; i += 5) {
                const targetCandle = allCandles[i];
                const T = index_1.CandleNormalizer.getCandleCloseTimestamp(targetCandle, '15m');
                const incrementalHistory = allCandles.slice(0, i + 1);
                const incrementalState = index_1.SnapshotBuilder.buildSnapshot({
                    symbol: 'BTCUSDT',
                    executionCandles: incrementalHistory,
                    asOfTimestamp: T,
                });
                const batchState = index_1.SnapshotBuilder.buildSnapshot({
                    symbol: 'BTCUSDT',
    ...
    describe('Invariant Liquidity Test C: Future-Data Invariance for Liquidity', () => {
        it('liquidity pools and sweeps at T are identical regardless of future candles', () => {
            const history = [];
            for (let i = 0; i < 40; i++) {
                history.push(createCandle(i, 100 + Math.sin(i / 2) * 4, 105 + Math.sin(i / 2) * 4, 95 + Math.sin(i / 2) * 4, 101 + Math.sin(i / 2) * 4, 1000, 15 * 60 * 1000));
            }
            const T = index_1.CandleNormalizer.getCandleCloseTimestamp(history[35], '15m');
            const smcA = index_1.SMCAnalyzer.analyze(history, { asOfTimestamp: T, timeframe: shared_1.Timeframe.M15 });
            const future = [
            ...
            const smcB = index_1.SMCAnalyzer.analyze([...history, ...future], { asOfTimestamp: T, timeframe: shared_1.Timeframe.M15 });
    ...
    describe('Invariant Liquidity Test D: Incremental vs Batch Liquidity State', () => {
        it('incremental liquidity state at T matches batch liquidity state with asOfTimestamp = T', () => {
            const allCandles = [];
            for (let i = 0; i < 50; i++) {
                allCandles.push(createCandle(i, 100 + Math.cos(i / 3) * 5, 105 + Math.cos(i / 3) * 5, 95 + Math.cos(i / 3) * 5, 102 + Math.cos(i / 3) * 5, 1000, 15 * 60 * 1000));
            }
            for (let i = 25; i < 45; i += 5) {
                const targetCandle = allCandles[i];
                const T = index_1.CandleNormalizer.getCandleCloseTimestamp(targetCandle, '15m');
                const incrementalHistory = allCandles.slice(0, i + 1);
                const smcIncremental = index_1.SMCAnalyzer.analyze(incrementalHistory, { asOfTimestamp: T });
                const smcBatch = index_1.SMCAnalyzer.analyze(allCandles, { asOfTimestamp: T });
    ...
    describe('Invariant Section 11: SMC Propagation Test', () => {
        it('proves liquidity availability fix propagates cleanly through SMCAnalyzer.analyze', () => {
            ...
            const T = index_1.CandleNormalizer.getCandleCloseTimestamp(history[30], '15m');
            const stateA = index_1.SMCAnalyzer.analyze(history, { asOfTimestamp: T });
            const torture = [
                ...history,
                createCandle(35, 105, 500, 104, 490, 1000000),
                createCandle(36, 490, 500, 10, 15, 2000000),
            ];
            const stateB = index_1.SMCAnalyzer.analyze(torture, { asOfTimestamp: T });
...
//# sourceMappingURL=point-in-time-invariants.test.js.map