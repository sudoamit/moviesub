...
describe('SwingDetector', () => {
    it('should detect a Swing High strictly after confirmation bars close (No look-ahead)', () => {
        ...
        const swings = swing_detector_1.SwingDetector.detectSwings(candles, {
            leftBars: 2,
            rightBars: 2,
        ...
        expect(swings[0].type).toBe(shared_1.StructureType.SWING_HIGH);
    ...
    it('should detect and classify Higher Highs, Higher Lows, Lower Highs, and Lower Lows', () => {
        ...
        const hasSwingHigh = swings.some((s) => s.type === shared_1.StructureType.SWING_HIGH || s.type === shared_1.StructureType.HIGHER_HIGH);
        const hasSwingLow = swings.some((s) => s.type === shared_1.StructureType.SWING_LOW || s.type === shared_1.StructureType.HIGHER_LOW);
        expect(hasSwingHigh).toBe(true);
        expect(hasSwingLow).toBe(true);
    });
});
//# sourceMappingURL=swing-detector.test.js.map