...
function createSyntheticDataset(count: number, signalToNoiseRatio = 0.8): TrainingExample[] {
...
describe('PHASE 3: Baseline Logistic Regression TradePredictionModel', () => {
  it('predicts win probabilities strictly bounded inside [0.0001, 0.9999]', () => {
    const model = new TradePredictionModel('v1.0.0');
    const features: TradeFeatureVector = {
      smcScore: 0.9,
      obStrength: 0.85,
      fvgSize: 0.7,
  ...
  it('performs online stochastic gradient descent and updates weights correctly', () => {
    const model = new TradePredictionModel('v1.0.0', { learningRate: 0.1 });
    const features: TradeFeatureVector = {
      smcScore: 0.9,
      obStrength: 0.9,
      fvgSize: 0.5,
  ...
  it('extracts feature importance ranked by absolute weight magnitude', () => {
    ...
    expect(importance.length).toBe(FEATURE_VECTOR_DIMENSION);
