...
exports.TradeLevelsCalculator = void 0;
...
class TradeLevelsCalculator {
     ...
     */
    static calculateLevels(direction, candles, anchorSwing, orderBlock, fvg) {
        if (direction === shared_1.Direction.NEUTRAL || !candles || candles.length === 0) {
            return null;
        }
        ...
        const currentAtr = rawAtr && !isNaN(Number(rawAtr)) && Number(rawAtr) > 0
            ? Number(rawAtr)
        ...
        if (direction === shared_1.Direction.BULLISH) {
            ...
            let entryMax = 0;
            let localLow = 0;
            if (orderBlock && orderBlock.direction === shared_1.Direction.BULLISH) {
            ...
            else if (fvg && fvg.direction === shared_1.Direction.BULLISH) {
            ...
            let entryMax = 0;
            let localHigh = 0;
            if (orderBlock && orderBlock.direction === shared_1.Direction.BEARISH) {
                entryMin = Number(orderBlock.low);
                entryMax = Number(orderBlock.high);
                entryOptimal = Number(((entryMin + entryMax) / 2).toFixed(2));
                localHigh = Number(orderBlock.high);
            }
            else if (fvg && fvg.direction === shared_1.Direction.BEARISH) {
                entryMin = Number(fvg.lowerBound);
                entryMax = Number(fvg.upperBound);
                entryOptimal = Number(((entryMin + entryMax) / 2).toFixed(2));
...
exports.TradeLevelsCalculator = TradeLevelsCalculator;
//# sourceMappingURL=trade-levels.js.map