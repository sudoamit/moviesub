...
class ReasoningGenerator {
    ...
    static generateReasoning(inputs) {
        ...
        if (scoring.htfAligned)
            checklist.push(`${mtf.htf1Timeframe} ${mtf.htfBias} trend alignment`);
        // 2. Liquidity Reason
        let liquidityReason = '';
        if (direction === shared_1.Direction.BULLISH) {
            liquidityReason = scoring.hasLiquiditySweep
        ...
        if (scoring.hasLiquiditySweep)
            checklist.push(direction === shared_1.Direction.BULLISH
        ...
        if (scoring.hasBOSOrCHOCH)
            checklist.push(`${timeframe} Execution Structure Break (BOS/CHoCH)`);
        if (scoring.hasOBOrFVG)
            checklist.push('Institutional Order Block / FVG footprint confluence');
        ...
        // 5. Unified Summary
        const dirText = direction === shared_1.Direction.BULLISH ? 'LONG' : 'SHORT';
...
exports.ReasoningGenerator = ReasoningGenerator;
//# sourceMappingURL=reasoning-generator.js.map