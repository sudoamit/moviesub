...
export class VolatilityEngine {
   ...
   */
  public static calculateParkinson(candles: ICandle[], window = 20): number {
   ...
   */
  public static calculateGarmanKlass(candles: ICandle[], window = 20): number {
    if (!candles || candles.length < 2) return 0;
    const slice = candles.slice(-window);
    const n = slice.length;
    if (n === 0) return 0;
   ...
   */
  public static calculateRealizedVolatility(closes: number[], window = 30): number {
   ...
   */
  public static fitGarch11(returns: number[]): IGarchParams | null {
    if (!returns || returns.length < 25) return null;
    ...
    sampleVar /= n;
    if (sampleVar <= 1e-12) return null;
    ...
    if (!isFinite(bestNLL)) return null;
   ...
   */
  public static forecastEwma(returns: number[], lambda = 0.94): number {
   ...
   */
  public static computeVolatilityState(candles: ICandle[]): VolatilityState {
    ...
    let modelUsed: 'GARCH' | 'EWMA' | 'REALIZED' = 'REALIZED';
    let confidence = 70;
    ...
    // 6. Bucket
    let volatilityBucket: 'P20' | 'P40' | 'P60' | 'P80' | 'HIGH' = 'P40';
    if (percentile < 20) volatilityBucket = 'P20';
    else if (percentile < 40) volatilityBucket = 'P40';
    else if (percentile < 60) volatilityBucket = 'P60';
    else if (percentile < 80) volatilityBucket = 'P80';
    else volatilityBucket = 'HIGH';
