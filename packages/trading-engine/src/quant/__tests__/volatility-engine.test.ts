...
describe('VolatilityEngine', () => {
  const createMockCandles = (count = 50, base = 100): ICandle[] => {
  ...
  it('should calculate Parkinson and Garman-Klass volatility correctly', () => {
    ...
    const parkinson = VolatilityEngine.calculateParkinson(candles, 20);
    const garmanKlass = VolatilityEngine.calculateGarmanKlass(candles, 20);
  ...
  it('should fit GARCH(1,1) with variance targeting', () => {
    ...
    const garch = VolatilityEngine.fitGarch11(returns);
  ...
  it('should gracefully fallback from GARCH to EWMA and Realized without throwing', () => {
    ...
    const volState = VolatilityEngine.computeVolatilityState(shortCandles);
