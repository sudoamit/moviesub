...
exports.CanonicalMLEngineV2 = void 0;
const shared_1 = require("@quant/shared");
const quant_types_1 = require("./quant-types");
class CanonicalMLEngineV2 {
    static SCHEMA_VERSION = quant_types_1.CANONICAL_FEATURE_SCHEMA_VERSION;
    static FEATURE_NAMES = quant_types_1.CANONICAL_FEATURE_NAMES_V2;
    static DIMENSION = quant_types_1.CANONICAL_V2_DIMENSION;
    /**
     * Weights initialized with Xavier-scaled prior distribution.
     */
    static weights = quant_types_1.CANONICAL_FEATURE_NAMES_V2.map((_, i) => {
        const scale = Math.sqrt(2.0 / (quant_types_1.CANONICAL_V2_DIMENSION + 1));
     ...
     */
    static clamp(v, min = 0.0, max = 1.0) {
     ...
     */
    static extractFeatures(snapshot) {
        const isBullish = snapshot.smc?.currentTrend === shared_1.Direction.BULLISH;
        ...
        const mtfAlignment = horizon.alignment === 'ALIGNED' ? 1.0 : horizon.alignment === 'PARTIALLY_ALIGNED' ? 0.6 : 0.2;
        ...
        const trendRegime = regime.regime === 'BULLISH_TREND'
            ? isBullish
                ? 1.0
                : 0.0
        ...
        const distanceToHTFLevel = this.clamp((quant.smcQuant?.distanceToHTFOrderBlockPct || 1.0) / 5.0);
    ...
    static toArray(vector) {
        return quant_types_1.CANONICAL_FEATURE_NAMES_V2.map((name) => vector[name] ?? 0.5);
    }
    /**
     * Evaluates the ML model to produce calibrated trade quality predictions and Expected Value.
     */
    static predict(features, expectedWinR = null) {
        const arr = this.toArray(features);
        let logit = this.bias;
        for (let i = 0; i < quant_types_1.CANONICAL_V2_DIMENSION; i++) {
        ...
        if (expectedWinR === null || expectedWinR === undefined || !Number.isFinite(expectedWinR)) {
            return {
                ...
                uncertainty: null,
                calibrated: false,
                featureSchemaVersion: quant_types_1.CANONICAL_FEATURE_SCHEMA_VERSION,
            };
        }
        ...
        const expectedR = Number((probabilityWin * expectedWinR - (1.0 - probabilityWin) * 1.0).toFixed(2));
        const confidence = Math.round(probabilityWin * 100);
        const uncertainty = Number((Math.sqrt(probabilityWin * (1.0 - probabilityWin)) * 0.5).toFixed(3));
        return {
            probabilityWin,
...
exports.CanonicalMLEngineV2 = CanonicalMLEngineV2;
//# sourceMappingURL=canonical-ml-v2.js.map