...
exports.LLMContextLayer = void 0;
class LLMContextLayer {
    static PROMPT_VERSION = '2.0.0';
    static MODEL_VERSION = 'gemini-1.5-flash-quant';
    /**
     * Builds the structured JSON prompt for LLM qualitative context evaluation.
     */
    static buildStructuredPrompt(snapshot) {
        const payload = {
     ...
     */
    static getDeterministicFallback(snapshot) {
        const isApproved = snapshot.score.totalScore >= 75 && snapshot.multiHorizon.alignment !== 'CONFLICTED';
        const isWait = snapshot.multiHorizon.alignment === 'CONFLICTED' ||
            snapshot.regime.regime === 'HIGH_VOLATILITY';
        return {
            decision: isApproved ? 'APPROVE' : isWait ? 'WAIT' : 'REJECT',
            confidence: snapshot.score.totalScore,
            concerns: snapshot.multiHorizon.alignment === 'CONFLICTED'
     ...
     */
    static parseAssessment(rawJson, snapshot) {
        try {
...
exports.LLMContextLayer = LLMContextLayer;
//# sourceMappingURL=llm-context-layer.js.map