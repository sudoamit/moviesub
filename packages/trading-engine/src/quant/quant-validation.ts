...
export class QuantValidationEngine {
  ...
  public static validateCandles(
    ...
    for (let i = 0; i < candles.length; i++) {
      ...
      if (
        ...
        hasInvalidOhlc = true;
        errors.push(
          `Invalid OHLC geometry at index ${i}: O=${c.open}, H=${c.high}, L=${c.low}, C=${c.close}`,
        );
      }
    }
   ...
   */
  public static testLookaheadInvariance<T>(
    featureExtractor: (candles: ICandle[]) => T,
    fullCandles: ICandle[],
    evalIndex: number,
  ): { isLeakageFree: boolean; historicalResult: T; futureAppendedResult: T } {
