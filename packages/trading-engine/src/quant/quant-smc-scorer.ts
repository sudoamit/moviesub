import { Direction, MarketRegimeType, SignalGrade } from '@quant/shared';
...
export class QuantSMCScorer {
  ...
  public static score(inputs: IQuantScoringInputs): QuantSMCScore {
    ...
    let regimeScore = 0;
    if (
      (isBull && inputs.regime === MarketRegimeType.BULLISH_TREND) ||
      (!isBull && inputs.regime === MarketRegimeType.BEARISH_TREND)
    ) {
      regimeScore = 10;
      rankedFactors.push({
      ...
      inputs.regime === MarketRegimeType.RANGE &&
    ...
    } else if (inputs.regime === MarketRegimeType.HIGH_VOLATILITY) {
      regimeScore = 4;
    } else {
      regimeScore = 5;
    }
    ...
    let grade = SignalGrade.NO_TRADE;
    if (totalScore >= 88) grade = SignalGrade.A_PLUS;
    else if (totalScore >= 75) grade = SignalGrade.A;
    else if (totalScore >= 60) grade = SignalGrade.B;
    else if (totalScore >= 45) grade = SignalGrade.C;
    else grade = SignalGrade.NO_TRADE;
