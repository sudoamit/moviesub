...
exports.RegimeClusteringEngine = void 0;
...
class RegimeClusteringEngine {
     ...
     */
    static runKMeans(data, k = 4, maxIterations = 25) {
        ...
        while (centroids.length < actualK) {
            ...
            for (const pt of data) {
                ...
                for (const c of centroids) {
                    let distSq = 0;
                    for (let j = 0; j < d; j++)
                        distSq += Math.pow(pt[j] - c[j], 2);
                    if (distSq < minDistSqToAny)
                        minDistSqToAny = distSq;
                }
        ...
        for (let iter = 0; iter < maxIterations; iter++) {
            ...
            for (let i = 0; i < n; i++) {
                ...
                for (let c = 0; c < actualK; c++) {
                    let distSq = 0;
                    for (let j = 0; j < d; j++)
                        distSq += Math.pow(data[i][j] - centroids[c][j], 2);
     ...
     */
    static mapCentroidToRegime(centroid) {
        ...
        if (volatilityPercent > 0.65 || bollingerWidth > 0.7) {
            return shared_1.MarketRegimeType.HIGH_VOLATILITY;
        }
        // Strong Bullish Trend
        if (trendStrength > 0.35 && directionalBias > 0.15 && rsiMomentum > 0.1) {
            return shared_1.MarketRegimeType.BULLISH_TREND;
        }
        // Strong Bearish Trend
        if (trendStrength > 0.35 && directionalBias < -0.15 && rsiMomentum < -0.1) {
            return shared_1.MarketRegimeType.BEARISH_TREND;
        }
        // Low Volatility compression
        if (volatilityPercent < 0.2 && trendStrength < 0.25) {
            return shared_1.MarketRegimeType.LOW_VOLATILITY;
        }
        // Default: Range
        return shared_1.MarketRegimeType.RANGE;
    }
    /**
    ...
    static classifyRegime(candles, swings = []) {
        ...
        const bbSeries = (0, indicators_1.calculateBollingerBands)(closes, 20, 2);
        ...
        for (let i = n - sampleWindow; i < n; i++) {
            ...
            if (i >= 20) {
                let sumV = 0;
                for (let v = i - 20; v < i; v++)
                    sumV += candles[v].volume;
                avgVol = sumV / 20;
            }
        ...
        const volState = volatility_engine_1.VolatilityEngine.computeVolatilityState(candles);
...
exports.RegimeClusteringEngine = RegimeClusteringEngine;
//# sourceMappingURL=regime-clustering-engine.js.map