...
class NormalizationEngine {
     ...
     */
    static rollingZScore(values, window = 20, clampRange = [-3.0, 3.0]) {
     ...
     */
    static rollingMinMax(values, window = 20) {
        if (!values || values.length === 0)
            return 0.5;
        const slice = values.slice(-window);
     ...
     */
    static rollingPercentileRank(values, window = 30) {
        if (!values || values.length === 0)
            return 0.5;
        const slice = values.slice(-window);
        const current = values[values.length - 1];
     ...
     */
    static rollingRobustScale(values, window = 30) {
