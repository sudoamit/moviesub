...
exports.ModelDriftDetector = exports.OnlineLearningEngine = exports.PostMortemAnalyticsEngine = exports.ModelRegistry = exports.ModelPersistenceManager = exports.ExpectedValueEngine = exports.PlattScaler = exports.ProbabilityCalibrationEngine = exports.ModelPromotionEngine = exports.DEFAULT_PROMOTION_CRITERIA = exports.WalkForwardValidator = exports.ChronologicalSplitter = exports.DEFAULT_SPLIT_CONFIG = exports.TradePredictionModel = exports.ModelEvaluationEngine = exports.DEFAULT_HYPERPARAMETERS = exports.TrainingDatasetBuilder = exports.TradeLabelGenerator = exports.AmbiguousLabelPolicy = exports.FeatureVectorExtractor = exports.FEATURE_VECTOR_DIMENSION = exports.FEATURE_NAMES = exports.FEATURE_SCHEMA_VERSION = void 0;
 ...
 */
exports.FEATURE_SCHEMA_VERSION = '1.0';
/**
...
exports.FEATURE_VECTOR_DIMENSION = exports.FEATURE_NAMES.length;
/**
...
class FeatureVectorExtractor {
    static SCHEMA_VERSION = exports.FEATURE_SCHEMA_VERSION;
    static FEATURE_NAMES = exports.FEATURE_NAMES;
    static DIMENSION = exports.FEATURE_VECTOR_DIMENSION;
    /**
    ...
    static extract(context) {
        ...
        const isBullish = signal.direction === 'BULLISH' || signal.direction === shared_1.Direction.BULLISH;
        ...
        if (smcAnalysis?.orderBlocks && smcAnalysis.orderBlocks.length > 0) {
            const activeOB = smcAnalysis.orderBlocks.find((ob) => (isBullish
                ? ob.direction === 'BULLISH' || ob.direction === shared_1.Direction.BULLISH
                : ob.direction === 'BEARISH' || ob.direction === shared_1.Direction.BEARISH) &&
                !ob.isMitigated) || smcAnalysis.orderBlocks[0];
        ...
        const sessionInfo = session_filter_1.SessionFilter.getSessionInfo(evalTime, signal.symbol);
        ...
        if (regime === shared_1.MarketRegimeType.BULLISH_TREND ||
            regime === shared_1.MarketRegimeType.BEARISH_TREND ||
            regime === 'TRENDING') {
            trendRegime = 1.0;
        }
        else if (regime === shared_1.MarketRegimeType.RANGE || regime === 'RANGING') {
            trendRegime = 0.0;
        }
        else if (regime === shared_1.MarketRegimeType.HIGH_VOLATILITY || regime === 'VOLATILE') {
            trendRegime = 0.3;
        }
    ...
    static toArray(features) {
        return exports.FEATURE_NAMES.map((name) => features[name]);
    }
    /**
    ...
    static fromArray(arr) {
        if (!arr || arr.length !== exports.FEATURE_VECTOR_DIMENSION) {
            throw new Error(`FeatureVectorExtractor.fromArray: expected array of dimension ${exports.FEATURE_VECTOR_DIMENSION}, received ${arr ? arr.length : 0}`);
        }
        const vector = {};
        exports.FEATURE_NAMES.forEach((name, idx) => {
            vector[name] = arr[idx];
        });
        return vector;
    }
     ...
     */
    static validateVector(features) {
        const errors = [];
        exports.FEATURE_NAMES.forEach((name) => {
...
exports.FeatureVectorExtractor = FeatureVectorExtractor;
/**
 * Policy for handling candles where both Take Profit and Stop Loss are breached on the same bar.
 */
var AmbiguousLabelPolicy;
(function (AmbiguousLabelPolicy) {
    /** Default: Do not arbitrarily guess winner; exclude from training dataset */
    AmbiguousLabelPolicy["AMBIGUOUS"] = "AMBIGUOUS";
    /** Conservative: Treat any ambiguous same-bar breach as a loss (label = 0) */
    AmbiguousLabelPolicy["CONSERVATIVE"] = "CONSERVATIVE";
    /** Placeholder for high-frequency sub-minute tick / intrabar resolution */
    AmbiguousLabelPolicy["INTRABAR_DATA"] = "INTRABAR_DATA";
})(AmbiguousLabelPolicy || (exports.AmbiguousLabelPolicy = AmbiguousLabelPolicy = {}));
/**
 * TradeLabelGenerator
...
class TradeLabelGenerator {
    static evaluateOutcome(params) {
        const { direction, entryPrice, stopLoss, targetPrice, entryTimestamp, subsequentCandles, ambiguousPolicy = AmbiguousLabelPolicy.AMBIGUOUS, maxHoldingBars = 200, } = params;
        const isBullish = direction === shared_1.Direction.BULLISH || direction === 'BULLISH';
...
exports.TradeLabelGenerator = TradeLabelGenerator;
/**
...
class TrainingDatasetBuilder {
     ...
     */
    static buildExample(params) {
        const { id, signalId, signal, historicalCandlesUpToEntry, subsequentCandlesAfterEntry, smcAnalysis, htfCandles, smtDivergenceScore, ambiguousPolicy = AmbiguousLabelPolicy.AMBIGUOUS, targetSelection = 'TP2', } = params;
        const entryTimestamp = signal.timestamp ? new Date(signal.timestamp) : new Date();
        const entryPrice = signal.entryZone.optimal;
        const stopLoss = signal.stopLoss;
        const targetPrice = targetSelection === 'TP1'
        ...
        if (exitTimestamp.getTime() <= entryTimestamp.getTime()) {
            throw new Error(`TrainingDatasetBuilder: causality violation: exitTimestamp (${exitTimestamp.toISOString()}) <= entryTimestamp (${entryTimestamp.toISOString()})`);
        }
        return {
            id,
            signalId,
            symbol: signal.symbol,
            featureSchemaVersion: exports.FEATURE_SCHEMA_VERSION,
            features,
     ...
     */
    static sortChronologically(examples) {
        return [...examples].sort((a, b) => new Date(a.predictionTimestamp).getTime() - new Date(b.predictionTimestamp).getTime());
    }
    /**
     * Validates a complete dataset for temporal causality, label validity, and feature schema integrity.
     */
    static validateDataset(examples) {
        const errors = [];
        ...
        examples.forEach((ex, idx) => {
            ...
            if (ex.featureSchemaVersion !== exports.FEATURE_SCHEMA_VERSION) {
                errors.push(`Example [${idx}] schema version mismatch: expected ${exports.FEATURE_SCHEMA_VERSION}, got ${ex.featureSchemaVersion}`);
            }
            // Check feature dimension
            if (!ex.featureArray || ex.featureArray.length !== exports.FEATURE_VECTOR_DIMENSION) {
                errors.push(`Example [${idx}] feature array dimension invalid: expected ${exports.FEATURE_VECTOR_DIMENSION}, got ${ex.featureArray?.length}`);
            }
            // Check label
            ...
            if (availTime <= predTime) {
                errors.push(`Example [${idx}] causality leak: availableForTrainingAt (${ex.availableForTrainingAt}) <= predictionTimestamp (${ex.predictionTimestamp})`);
            }
...
exports.TrainingDatasetBuilder = TrainingDatasetBuilder;
exports.DEFAULT_HYPERPARAMETERS = {
    learningRate: 0.05,
...
class ModelEvaluationEngine {
     ...
     */
    static calculateMetrics(examples, model) {
        ...
        for (let i = 0; i < N; i++) {
            ...
            if (predBinary === 1 && y === 1)
                tp++;
            else if (predBinary === 1 && y === 0)
                fp++;
            else if (predBinary === 0 && y === 0)
                tn++;
            else if (predBinary === 0 && y === 1)
                fn++;
            // Log loss
...
exports.ModelEvaluationEngine = ModelEvaluationEngine;
/**
...
class TradePredictionModel {
    ...
    bias;
    hyperparameters;
    constructor(modelVersion = 'v1.0.0', hyperparameters = {}, initialWeights, initialBias = 0.0) {
        this.modelVersion = modelVersion;
        this.hyperparameters = { ...exports.DEFAULT_HYPERPARAMETERS, ...hyperparameters };
        if (initialWeights && initialWeights.length === exports.FEATURE_VECTOR_DIMENSION) {
            this.weights = [...initialWeights];
        }
        else {
            // Deterministic Xavier/Glorot-style small initial weights
            const scale = Math.sqrt(2.0 / (exports.FEATURE_VECTOR_DIMENSION + 1));
            this.weights = exports.FEATURE_NAMES.map((_, i) => {
     ...
     */
    calculateLogit(featureArray) {
        let z = this.bias;
        for (let i = 0; i < exports.FEATURE_VECTOR_DIMENSION; i++) {
    ...
    train(dataset) {
        ...
        for (let epoch = 0; epoch < maxEpochs; epoch++) {
            ...
            for (let startIdx = 0; startIdx < N; startIdx += batchSize) {
                ...
                const gradW = new Array(exports.FEATURE_VECTOR_DIMENSION).fill(0.0);
                let gradB = 0.0;
                for (let b = 0; b < B; b++) {
                    const ex = batch[b];
    ...
    setWeights(weights, bias) {
        if (weights.length !== exports.FEATURE_VECTOR_DIMENSION) {
            throw new Error(`Invalid weights dimension: expected ${exports.FEATURE_VECTOR_DIMENSION}, got ${weights.length}`);
        }
    ...
    getFeatureImportance() {
        return exports.FEATURE_NAMES.map((name, idx) => ({
            feature: name,
...
exports.TradePredictionModel = TradePredictionModel;
...
class ChronologicalSplitter {
    static split(dataset, config = {}) {
        const cfg = { ...exports.DEFAULT_SPLIT_CONFIG, ...config };
...
exports.ChronologicalSplitter = ChronologicalSplitter;
/**
...
class WalkForwardValidator {
     ...
     */
    static runWalkForward(dataset, modelFactory, numFolds = 3) {
        const sorted = TrainingDatasetBuilder.sortChronologically(dataset);
        const N = sorted.length;
        if (N < numFolds * 10) {
        ...
        const folds = [];
        const foldTestLosses = [];
        const foldTestAccs = [];
        const foldTestAucs = [];
        const foldTestBriers = [];
        ...
        for (let f = 0; f < numFolds; f++) {
            ...
            if (trainFold.length === 0 || testFold.length === 0)
                continue;
        ...
        const variance = foldTestLosses.reduce((acc, l) => acc + Math.pow(l - meanTestLogLoss, 2), 0) /
...
exports.WalkForwardValidator = WalkForwardValidator;
exports.DEFAULT_PROMOTION_CRITERIA = {
    minSampleSize: 20,
...
class ModelPromotionEngine {
     ...
     */
    static evaluatePromotion(candidateModel, outOfSampleDataset, baselineModel, criteria = {}) {
        const cfg = { ...exports.DEFAULT_PROMOTION_CRITERIA, ...criteria };
        const reasons = [];
        ...
        if (N < cfg.minSampleSize) {
            reasons.push(`Insufficient out-of-sample sample size (${N} observations < required minimum ${cfg.minSampleSize})`);
            return {
                isPromoted: false,
        ...
        if (candidateMetrics.logLoss > cfg.maxOutOfSampleLogLoss) {
            reasons.push(`Candidate out-of-sample Log Loss (${candidateMetrics.logLoss}) exceeds maximum tolerance (${cfg.maxOutOfSampleLogLoss})`);
        }
        // 3. ROC-AUC Barrier
        if (candidateMetrics.rocAuc < cfg.minOutOfSampleRocAuc) {
            reasons.push(`Candidate out-of-sample ROC-AUC (${candidateMetrics.rocAuc}) is below minimum threshold (${cfg.minOutOfSampleRocAuc})`);
        }
        // 4. Maximum Drawdown Barrier
        if (candidateMetrics.maxDrawdownR > cfg.maxDrawdownToleranceR) {
            reasons.push(`Candidate out-of-sample Max Drawdown (${candidateMetrics.maxDrawdownR}R) exceeds tolerance (${cfg.maxDrawdownToleranceR}R)`);
        }
        ...
        if (cfg.mustBeatBaseline && baselineMetrics) {
            if (candidateMetrics.logLoss > baselineMetrics.logLoss) {
                reasons.push(`Candidate Log Loss (${candidateMetrics.logLoss}) does not improve over baseline (${baselineMetrics.logLoss})`);
            }
            if (candidateMetrics.rocAuc < baselineMetrics.rocAuc) {
                reasons.push(`Candidate ROC-AUC (${candidateMetrics.rocAuc}) is inferior to baseline (${baselineMetrics.rocAuc})`);
            }
        }
        const isPromoted = reasons.length === 0;
        if (isPromoted) {
            reasons.push(`Candidate model passed all out-of-sample validation tests (LogLoss: ${candidateMetrics.logLoss}, ROC-AUC: ${candidateMetrics.rocAuc}, Accuracy: ${candidateMetrics.accuracy})`);
        }
        return {
            isPromoted,
...
exports.ModelPromotionEngine = ModelPromotionEngine;
/**
...
class ProbabilityCalibrationEngine {
     ...
     */
    static generateCalibrationReport(items, numBins = 10) {
     ...
     */
    static calculateConfidenceInterval(probability, supportingSampleSize, minSampleSize = 15, confidenceLevel = 0.95) {
...
class PlattScaler {
     ...
     */
    fit(probabilities, labels, maxIter = 50, lr = 0.05) {
...
exports.PlattScaler = PlattScaler;
/**
 * ExpectedValueEngine
...
class ExpectedValueEngine {
     ...
     */
    static calculateExpectedValue(winProbability, payoff) {
     ...
     */
    static evaluateRecommendation(params) {
        const { probability, payoff, supportingSampleSize, calibrationStatus = 'NOT_AVAILABLE', minSampleSize = 25, modelVersion = 'v1.0.0', } = params;
        const evResult = this.calculateExpectedValue(probability, payoff);
        const confidenceInterval = ProbabilityCalibrationEngine.calculateConfidenceInterval(probability, supportingSampleSize, minSampleSize);
        const isCalibrated = calibrationStatus === 'EXCELLENT' ||
        ...
        if (supportingSampleSize < minSampleSize) {
            reasons.push(`INSUFFICIENT_DATA: Supporting sample size (${supportingSampleSize}) is below minimum threshold (${minSampleSize}) required for statistical conviction.`);
            return {
                recommendation: 'WAIT',
        ...
        if (evResult.expectedValueR <= 0.0) {
            reasons.push(`NEGATIVE_EXPECTANCY: Expected value (${evResult.expectedValueR}R) is non-positive despite setup score.`);
            return {
                recommendation: 'WAIT',
                winProbability: evResult.winProbability,
                expectedValueR: evResult.expectedValueR,
        ...
        if (calibrationStatus === 'POOR') {
            reasons.push('POOR_CALIBRATION: Probability reliability error is elevated. Model recommendation downgraded to WAIT.');
            return {
                recommendation: 'WAIT',
                winProbability: evResult.winProbability,
                expectedValueR: evResult.expectedValueR,
        ...
        const isAsymmetricHigh = payoff.targetR >= 2.5 && evResult.winProbability >= 0.5 && evResult.expectedValueR >= 0.8;
        if ((isStandardHigh || isAsymmetricHigh) &&
            (calibrationStatus === 'EXCELLENT' || calibrationStatus === 'GOOD')) {
            reasons.push(`High conviction setup (${(evResult.winProbability * 100).toFixed(1)}% win rate, strong +${evResult.expectedValueR}R positive expectancy) across ${supportingSampleSize} validated observations.`);
            return {
        ...
        if (isStandardModerate || isAsymmetricModerate) {
            reasons.push(`Favorable statistical edge (${(evResult.winProbability * 100).toFixed(1)}% win rate, +${evResult.expectedValueR}R expectancy) backed by ${supportingSampleSize} observations.`);
            return {
        ...
        if (isStandardLow || isAsymmetricLow) {
            reasons.push(`Positive mathematical expectation (+${evResult.expectedValueR}R) with ${(evResult.winProbability * 100).toFixed(1)}% win probability.`);
            return {
...
exports.ExpectedValueEngine = ExpectedValueEngine;
/**
...
class ModelPersistenceManager {
    ...
    static serialize(model, meta) {
        ...
        return {
            ...
            bias: model.getBias(),
            hyperparameters: { ...exports.DEFAULT_HYPERPARAMETERS, ...meta.hyperparameters },
    ...
    static deserialize(state) {
        ...
        if (state.featureSchemaVersion !== exports.FEATURE_SCHEMA_VERSION) {
            throw new Error(`ModelPersistenceManager.deserialize: feature schema mismatch: expected ${exports.FEATURE_SCHEMA_VERSION}, received ${state.featureSchemaVersion}`);
        }
        if (!state.weights || state.weights.length !== exports.FEATURE_VECTOR_DIMENSION) {
            throw new Error(`ModelPersistenceManager.deserialize: weights dimension mismatch: expected ${exports.FEATURE_VECTOR_DIMENSION}, received ${state.weights?.length}`);
        }
        return new TradePredictionModel(state.version, state.hyperparameters, state.weights, state.bias !== undefined ? state.bias : 0.0);
    }
}
exports.ModelPersistenceManager = ModelPersistenceManager;
/**
 * In-memory model registry managing model version history, promotions, and active production routing.
 */
class ModelRegistry {
    versions = new Map();
    assetActiveVersions = new Map();
    activeVersionId = null;
    /**
     * Registers a new model version in the registry. Never overwrites existing versions without explicit promotion.
     */
    registerVersion(state, asset) {
        if (this.versions.has(state.version)) {
            throw new Error(`ModelRegistry: model version '${state.version}' already exists in registry. Overwriting immutable versions is prohibited.`);
        }
    ...
    getAllVersions() {
        return Array.from(this.versions.values()).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    }
    clear() {
        this.versions.clear();
...
class PostMortemAnalyticsEngine {
     ...
     */
    static analyzeTrade(input) {
        const { symbol, direction, entryPrice, stopLoss, targets, entryTimestamp, subsequentCandles, marketRegime = 'UNKNOWN', htfTrendAligned = true, atrAtEntry, entrySignalConfirmed = true, hasNewsEventDuringTrade = false, } = input;
        ...
        const timeToResolutionMinutes = Math.max(1, Math.round((exitTimestamp.getTime() - entryTimestamp.getTime()) / 60000));
     ...
     */
    static classifyOutcome(params) {
        const { outcome, direction, entryPrice, stopLoss, targets, subsequentCandles, hitSlIndex, htfTrendAligned, entrySignalConfirmed, hasNewsEventDuringTrade, atrAtEntry, } = params;
        const factors = [];
        // 1. Success cases
...
exports.PostMortemAnalyticsEngine = PostMortemAnalyticsEngine;
/**
...
class OnlineLearningEngine {
     ...
     */
    updateModel(model, tradeOutcome) {
        const { tradeId, symbol, features, featureSchemaVersion, actualLabel } = tradeOutcome;
        // 1. Feature Schema Guard
        if (featureSchemaVersion && featureSchemaVersion !== exports.FEATURE_SCHEMA_VERSION) {
            return {
                tradeId,
                symbol,
        ...
        if (this.recentLosses.length > 20)
...
exports.OnlineLearningEngine = OnlineLearningEngine;
/**
...
class ModelDriftDetector {
     ...
     */
    evaluateDrift(asset = 'GLOBAL', modelVersion = '1.0.0', baselineMetrics) {
        if (this.samples.length < 10) {
            return {
                ...
                recommendedAction: 'CONTINUE_LIVE',
        ...
        const rollingExpectancyR = Number((this.samples.reduce((acc, s) => acc + s.realizedR, 0) / n).toFixed(2));
        // 1. Calculate Brier Score: 1/N * sum((p - y)^2)
        const brierScore = Number((this.samples.reduce((acc, s) => acc + Math.pow(s.predictionProbability - s.actualOutcome, 2), 0) / n).toFixed(4));
        // 2. Calculate ECE & MCE across 10 decile probability bins
        const numBins = 10;
        const bins = Array.from({ length: numBins }, () => ({ count: 0, sumProb: 0, sumActual: 0 }));
        ...
        const reasons = [];
        let recommendedAction = 'CONTINUE_LIVE';
...
//# sourceMappingURL=ai-trade-learning-engine.js.map