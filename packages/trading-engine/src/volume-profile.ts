...
export interface IVolumeProfileResult {
  ...
  vah: number; // Value Area High (70% upper bound)
  val: number; // Value Area Low (70% lower bound)
  totalVolume: number;
  bins: IVolumeProfileBin[];
  cvd: { timestamp: string; delta: number; cumulativeDelta: number }[];
}
...
export class VolumeProfileAnalyzer {
  ...
  public static compute(
    ...
    // Initialize bins
    const bins: IVolumeProfileBin[] = Array.from({ length: binCount }, (_, i) => ({
    ...
    let runningCVD = 0;
    const cvdList: { timestamp: string; delta: number; cumulativeDelta: number }[] = [];
