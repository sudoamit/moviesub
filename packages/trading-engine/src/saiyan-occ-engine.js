...
exports.SaiyanOCCEngine = void 0;
const shared_1 = require("@quant/shared");
const indicators_1 = require("@quant/indicators");
class SaiyanOCCEngine {
     ...
     * w_i = exp(-((i - m)^2) / (2 * s^2))
     */
    static calculateALMA(src, len, offset = 0.85, sigma = 5) {
     ...
     */
    static calculateHullMA(src, len) {
        ...
        const diff = src.map((_, i) => 2 * wmaHalf[i] - wmaFull[i]);
        return this.calculateWMA(diff, sqrtLen);
    }
    /**
     * Moving Average Variant selector matching PineScript variant()
     */
    static calculateVariant(type, src, len, sigma = 5, almaOffset = 0.85) {
        if (type === 'ALMA')
            return this.calculateALMA(src, len, almaOffset, sigma);
        if (type === 'HullMA')
            return this.calculateHullMA(src, len);
        if (type === 'EMA') {
            const k = 2 / (len + 1);
            const res = [src[0]];
     ...
     */
    static smoothAlternateResolution(series, factor = 8) {
    ...
    static analyze(candles, config = {}) {
        ...
        if (n < 20) {
            ...
            return {
                ...
                tp3: 0,
                signalScore: 0,
                signalGrade: shared_1.SignalGrade.NO_TRADE,
            };
        }
        ...
        let direction = shared_1.Direction.NEUTRAL;
        ...
        const isBull = direction === shared_1.Direction.BULLISH;
        ...
        if (isLongTrigger || isShortTrigger)
            signalScore += 15; // Fresh crossover momentum
        if (bosBoxes.length > 0)
            signalScore += 10; // Structure breakout confirmation
        ...
        const signalGrade = signalScore >= 90
            ? shared_1.SignalGrade.A_PLUS
            : signalScore >= 80
                ? shared_1.SignalGrade.A
                : signalScore >= 70
                    ? shared_1.SignalGrade.B
                    : shared_1.SignalGrade.NO_TRADE;
        return {
            direction,
            isLongTrigger,
            isShortTrigger,
     ...
     */
    static generateSignal(symbol, candles, timeframe = '15m') {
        ...
        if (isBull) {
            if (currentPrice >= analysis.tp3) {
                ...
                state = shared_1.SignalState.SL_HIT;
            }
            else {
                state = shared_1.SignalState.ACTIVE;
            }
        }
        else if (analysis.direction === shared_1.Direction.BEARISH) {
            if (currentPrice <= analysis.tp3) {
                state = 'TP3_HIT';
            }
            else if (currentPrice <= analysis.tp2) {
...
exports.SaiyanOCCEngine = SaiyanOCCEngine;
//# sourceMappingURL=saiyan-occ-engine.js.map