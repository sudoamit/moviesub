...
exports.AblationSimulator = void 0;
...
class AblationSimulator {
     ...
     */
    static runAblationStudy(options) {
        ...
        for (const variant of variants) {
            ...
            if (variant === 'SMC_ONLY')
                minScore = 50;
            else if (variant === 'FULL_SYSTEM')
                minScore = 75;
            else
                minScore = 65;
            const simResult = backtest_simulator_1.BacktestSimulator.runSimulation({
                ...options,
                minScore,
            });
            rows.push({
        ...
        const sorted = [...rows].sort((a, b) => b.expectancy * (b.sharpeRatio || 1) - a.expectancy * (a.sharpeRatio || 1));
        const best = sorted[0];
     ...
     */
    static runFeatureComponentAblation(options) {
        const symbol = options.symbol.toUpperCase();
        const timeframe = options.timeframe || '15m';
        // 1. Run baseline
        const baseline = backtest_simulator_1.BacktestSimulator.runSimulation({
            ...options,
        ...
        const results = components.map((variant) => {
            ...
            return {
                ...
                maxDrawdownPercent: Number((baseline.maxDrawdownPercent * (1 + Math.abs(delta) * 1.5)).toFixed(1)),
...
exports.AblationSimulator = AblationSimulator;
//# sourceMappingURL=ablation-simulator.js.map