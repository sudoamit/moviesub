...
const fill_model_1 = require("../execution/fill-model");
const types_1 = require("../execution/types");
const ohlc_path_cursor_1 = require("../execution/ohlc-path-cursor");
...
describe('Backtesting Execution Correctness Pass (6 Targeted Fixes & Partial Exit Ledger)', () => {
    ...
    test('1. TP1 hit reduces position and updates SL size without cancelling TP2/TP3', () => {
        const execSim = new execution_simulator_1.ExecutionSimulator(types_1.FillModel.OHLC_PATH, types_1.SameCandleAmbiguityMode.OHLC_PATH);
    ...
    test('6. P0 Audit Fix: NEXT_BAR_MARKET simulation evaluates resting STOP and LIMIT orders correctly against current bar', () => {
        const execSim = new execution_simulator_1.ExecutionSimulator(types_1.FillModel.NEXT_BAR_MARKET, types_1.SameCandleAmbiguityMode.OHLC_PATH);
        const timestamp = 1700000000000;
    ...
    test('7. P1 Audit Fix: Process candle emits TP_FILLED event for TP limit orders', () => {
        const execSim = new execution_simulator_1.ExecutionSimulator(types_1.FillModel.OHLC_PATH);
        const timestamp = 1700000000000;
        execSim.submitOrder({
    ...
    test('8. P1 Audit Fix: validateSubBars validates 15m parent candle duration correctly', () => {
        ...
        const valRes = fill_model_1.FillModelEngine.validateSubBars(parentCandle, invalidFutureSubBars, parentDurationMs);
    ...
    test('9. Rejects order submission if order timestamp precedes signal timestamp', () => {
        const execSim = new execution_simulator_1.ExecutionSimulator();
    ...
    test('10. evaluateLotTick throws exception when invoked with forBacktest = true', () => {
        ...
        expect(() => risk_engine_1.TradeLifecycleManager.evaluateLotTick(lot, candle, undefined, undefined, true)).toThrow('is deprecated');
    });
    ...
    test('14. Same-candle TP/SL resolution under all four ambiguity modes', () => {
        ...
        const simCons = new execution_simulator_1.ExecutionSimulator(types_1.FillModel.OHLC_PATH, types_1.SameCandleAmbiguityMode.CONSERVATIVE);
        ...
        const simOpt = new execution_simulator_1.ExecutionSimulator(types_1.FillModel.OHLC_PATH, types_1.SameCandleAmbiguityMode.OPTIMISTIC);
        ...
        const simPath = new execution_simulator_1.ExecutionSimulator(types_1.FillModel.OHLC_PATH, types_1.SameCandleAmbiguityMode.OHLC_PATH);
        ...
        const simSub = new execution_simulator_1.ExecutionSimulator(types_1.FillModel.LOWER_TIMEFRAME, types_1.SameCandleAmbiguityMode.LOWER_TIMEFRAME);
    ...
    test('15. validatePartialExitPolicy rejects policies where ratios do not sum to 1.0', () => {
        ...
        const val = risk_engine_1.TradeLifecycleManager.validatePartialExitPolicy(invalidPolicy);
    ...
    test('16. createPositionLot creates an immutable (frozen) EntryExecutionSnapshot', () => {
        ...
        const lot = risk_engine_1.TradeLifecycleManager.createPositionLot(signal, 100.0, 10, 1700000010000, 'ord_1', 1.5, 0.5);
        ...
        // Immutability check
        expect(() => {
            lot.entrySnapshot.entryPrice = 200.0;
        }).toThrow();
    });
    ...
    test('18. validateSubBars rejects incomplete M1 sub-bar coverage for a 15m parent candle', () => {
        ...
        const valRes = fill_model_1.FillModelEngine.validateSubBars(parentCandle, incompleteSubBars, 15 * 60 * 1000, true);
        expect(valRes.isValid).toBe(false);
    ...
    test('19. Financial Invariants: Equity = Cash + Unrealized PnL and Cash_final = Initial + PnL - Fees', () => {
        ...
        const res = backtest_simulator_1.BacktestSimulator.runSimulation({
            symbol: 'BTCUSDT',
            candles: [
    ...
    test('21. Real Execution Pipeline: TP1 hit -> SL to BE -> TP2 hit -> trailing stop -> TP3 hit', () => {
        ...
        const c1 = { timestamp: new Date(timestamp + 60000), open: 100, high: 112, low: 99, close: 111, volume: 100 };
        ...
        const c2 = { timestamp: new Date(timestamp + 120000), open: 111, high: 122, low: 110, close: 121, volume: 100 };
        ...
        const c3 = { timestamp: new Date(timestamp + 180000), open: 121, high: 132, low: 120, close: 131, volume: 100 };
    ...
    test('22. Real Execution Pipeline: TP1 hit (SL to BE) -> Pullback triggers BE stop loss', () => {
        ...
        const c2 = { timestamp: new Date(timestamp + 120000), open: 108, high: 108, low: 99, close: 99.5, volume: 100 };
        const res2 = execSim.processCandle(c2);
        expect(res2.fills).toHaveLength(1);
        expect(res2.fills[0].orderId).toBe(slOrder.orderId);
        expect(res2.fills[0].quantity).toBe(70.0);
    ...
    test('24. M15 parent candle execution using 15 real 1-minute sub-bars with exact timestamp boundaries', () => {
        const execSim = new execution_simulator_1.ExecutionSimulator(types_1.FillModel.LOWER_TIMEFRAME, types_1.SameCandleAmbiguityMode.LOWER_TIMEFRAME);
        ...
        const valRes = fill_model_1.FillModelEngine.validateSubBars(parentCandle, m1SubBars, parentDurationMs, true);
    ...
    test('27. validateSubBars requires exactly 15 M1 sub-bars for a 15m parent candle under strict policy', () => {
        ...
        const valRes = fill_model_1.FillModelEngine.validateSubBars(parentCandle, subBars10, 15 * 60 * 1000, false);
        expect(valRes.isValid).toBe(false);
    ...
    test('28. OHLCPathCursor creates 3 sequential segments and advances without rescanning', () => {
        ...
        const cursor = new ohlc_path_cursor_1.OHLCPathCursor(bullishCandle);
    ...
    test('32. resolveSegmentConflict selects SL at 95 (dist=5) before TP at 110 (dist=20) along segment 90 -> 120', () => {
        ...
        const triggered = [
            { order: slOrder, fill: { fillId: 'f1', price: 95.0, quantity: 100 } },
            { order: tpOrder, fill: { fillId: 'f2', price: 110.0, quantity: 30 } },
        ];
        ...
        const res = fill_model_1.FillModelEngine.resolveSegmentConflict(triggered, 90.0, 120.0, types_1.SameCandleAmbiguityMode.OHLC_PATH);
    ...
    test('35. BacktestSimulator respects configurable warmupBars=0 and minimumCandles=2', () => {
        ...
        const resEmpty = backtest_simulator_1.BacktestSimulator.runSimulation({
            symbol: 'BTCUSDT',
            candles,
            minimumCandles: 50,
        });
        ...
        const resActive = backtest_simulator_1.BacktestSimulator.runSimulation({
            symbol: 'BTCUSDT',
            candles,
            warmupBars: 0,
            minimumCandles: 2,
    ...
    test('36. resolveSegmentConflict selects TP at 110 (dist=10) before SL at 95 (dist=25) along segment 120 -> 90', () => {
        ...
        const res = fill_model_1.FillModelEngine.resolveSegmentConflict(triggered, 120.0, 90.0, types_1.SameCandleAmbiguityMode.OHLC_PATH);
    ...
    test('37. resolveSegmentConflict uses ambiguity mode as tie-breaker when distances are equal', () => {
        const order1 = { orderId: 'o1', orderType: 'STOP', stopPrice: 105.0 };
        const order2 = { orderId: 'o2', orderType: 'LIMIT', price: 105.0 };
        const triggered = [
            { order: order1, fill: { fillId: 'f1', price: 105.0 } },
            { order: order2, fill: { fillId: 'f2', price: 105.0 } },
        ];
        // CONSERVATIVE policy prefers STOP order (o1)
        const resCons = fill_model_1.FillModelEngine.resolveSegmentConflict(triggered, 100.0, 110.0, types_1.SameCandleAmbiguityMode.CONSERVATIVE);
        expect(resCons.winningOrder?.orderId).toBe('o1');
        // OPTIMISTIC policy prefers LIMIT order (o2)
        const resOpt = fill_model_1.FillModelEngine.resolveSegmentConflict(triggered, 100.0, 110.0, types_1.SameCandleAmbiguityMode.OPTIMISTIC);
    ...
    test('38. validateSubBars detects start time mismatch and duplicate sub-bar timestamps', () => {
        ...
        const subBarsShifted = Array.from({ length: 15 }, (_, i) => ({
            timestamp: new Date(1700000060000 + i * 60000), // starts 1 minute late
            open: 100, high: 101, low: 99, close: 100, volume: 10,
        }));
        const valShifted = fill_model_1.FillModelEngine.validateSubBars(parentCandle, subBarsShifted, 15 * 60 * 1000, false);
        ...
        const subBarsDup = Array.from({ length: 15 }, (_, i) => ({
            timestamp: new Date(1700000000000 + (i === 5 ? 4 * 60000 : i * 60000)), // bar 5 has same time as bar 4
            open: 100, high: 101, low: 99, close: 100, volume: 10,
        }));
        const valDup = fill_model_1.FillModelEngine.validateSubBars(parentCandle, subBarsDup, 15 * 60 * 1000, false);
    ...
    test('39. Deterministic Long E2E Backtest Simulation with Execution Simulator', () => {
        ...
        const c1 = { timestamp: new Date(startTime + 60000), open: 102, high: 103, low: 99, close: 101, volume: 100 };
        const res1 = execSim.processCandle(c1);
        expect(res1.fills).toHaveLength(1);
        ...
        const c2 = { timestamp: new Date(startTime + 120000), open: 101, high: 112, low: 100, close: 111, volume: 100 };
        const res2 = execSim.processCandle(c2);
        expect(res2.fills).toHaveLength(1);
    ...
    test('40. Deterministic Short E2E Backtest Simulation with Execution Simulator', () => {
        ...
        const c1 = { timestamp: new Date(startTime + 60000), open: 98, high: 101, low: 97, close: 99, volume: 100 };
        const res1 = execSim.processCandle(c1);
        expect(res1.fills).toHaveLength(1);
        ...
        const c2 = { timestamp: new Date(startTime + 120000), open: 99, high: 100, low: 88, close: 91, volume: 100 };
        const res2 = execSim.processCandle(c2);
        expect(res2.fills).toHaveLength(1);
        expect(res2.fills[0].exitTarget).toBe('TP1');
    ...
    test('41. BacktestSimulator generates deterministic runId when not explicitly provided', () => {
        ...
        const resCustom = backtest_simulator_1.BacktestSimulator.runSimulation({
            symbol: 'ETHUSDT',
    ...
    test('42. resolveSameCandleConflict under OHLC_PATH delegates to OHLCPathCursor and resolveSegmentConflict', () => {
        ...
        const res = fill_model_1.FillModelEngine.resolveSameCandleConflict(orders, candle, undefined, types_1.FillModel.OHLC_PATH, types_1.SameCandleAmbiguityMode.OHLC_PATH);
    ...
    test('43. validateSubBars detects irregular sub-bar interval gaps', () => {
        ...
        const subBarsIrregular = Array.from({ length: 15 }, (_, i) => ({
            timestamp: new Date(1700000000000 + (i >= 5 ? (i + 1) * 60000 : i * 60000)),
            open: 100, high: 101, low: 99, close: 100, volume: 10,
        }));
        const valRes = fill_model_1.FillModelEngine.validateSubBars(parentCandle, subBarsIrregular, 15 * 60 * 1000, false);
        expect(valRes.isValid).toBe(false);
    ...
    test('44. Complete M15 -> M1 sub-bar lifecycle produces exactly 1 fill per trigger and zero duplicate events', () => {
        ...
        const m1SubBars = Array.from({ length: 15 }, (_, i) => ({
    ...
    test('45. Audit gap-through execution through segment path for Long gap-down SL & Short gap-up SL', () => {
        ...
        const resLong = fill_model_1.FillModelEngine.evaluateSegmentFill(slLong, candleLongGap.open, candleLongGap.low, 1700000000000, 'BTCUSDT');
        ...
        const resShort = fill_model_1.FillModelEngine.evaluateSegmentFill(slShort, candleShortGap.open, candleShortGap.high, 1700000000000, 'BTCUSDT');
    ...
    test('48a. Real BacktestSimulator.runSimulation() Long E2E test asserts actual trades, direction, PnL, fees, and equity invariants', async () => {
        const provider = new shared_1.MockMarketDataProvider({ seed: 100 });
        ...
        const longTrades = result.trades.filter((t) => t.direction === shared_1.Direction.BULLISH);
        expect(longTrades.length).toBeGreaterThan(0);
        for (const trade of longTrades) {
            expect(trade.direction).toBe(shared_1.Direction.BULLISH);
    ...
    test('48b. Real BacktestSimulator.runSimulation() Short E2E test asserts actual trades, direction, PnL, fees, and equity invariants', async () => {
        const provider = new shared_1.MockMarketDataProvider({ seed: 6 });
        ...
        expect(result).toBeDefined();
        expect(result.trades.length).toBeGreaterThan(0);
        const shortTrades = result.trades.filter((t) => t.direction === shared_1.Direction.BEARISH);
        expect(shortTrades.length).toBeGreaterThan(0);
        for (const trade of shortTrades) {
            expect(trade.direction).toBe(shared_1.Direction.BEARISH);
            expect(trade.entryPrice).toBeGreaterThan(0);
            expect(trade.exitPrice).toBeGreaterThan(0);
            expect(trade.positionSize).toBeGreaterThan(0);
            expect(trade.entryFees).toBeGreaterThan(0);
    ...
    test('50. Two identical BacktestSimulator runs produce identical deterministic IDs and results', async () => {
        ...
        const res1 = backtest_simulator_1.BacktestSimulator.runSimulation({
            symbol: 'BTCUSDT',
            timeframe: '15m',
            candles,
            initialCapital: 100000,
        ...
        const res2 = backtest_simulator_1.BacktestSimulator.runSimulation({
            symbol: 'BTCUSDT',
            timeframe: '15m',
            candles,
            initialCapital: 100000,
    ...
    test('51. FillModel.LIMIT_TOUCH fills at exact limit touch price with zero slippage and zero spread adjustment', () => {
        const execSimTouch = new execution_simulator_1.ExecutionSimulator(types_1.FillModel.LIMIT_TOUCH, types_1.SameCandleAmbiguityMode.OHLC_PATH);
        const execSimSlip = new execution_simulator_1.ExecutionSimulator(types_1.FillModel.LIMIT_WITH_SLIPPAGE, types_1.SameCandleAmbiguityMode.OHLC_PATH);
        const timestamp = 1700000000000;
    ...
    test('52a. LOWER_TIMEFRAME MARKET execution: fills immediately on M1 #0 open with correct price, timestamp, quantity, fee, slippage, and zero duplicates', () => {
        ...
        const m1Candles = Array.from({ length: 15 }, (_, i) => ({
            timestamp: new Date(parentTime + i * 60000),
            open: 100 + i * 0.1,
            high: 101 + i * 0.1,
            low: 99 + i * 0.1,
    ...
    test('52c. LOWER_TIMEFRAME STOP execution: triggers on exact M1 sub-bar for both Long (SELL STOP) and Short (BUY STOP) positions', () => {
        ...
        const execSimLong = new execution_simulator_1.ExecutionSimulator(types_1.FillModel.LOWER_TIMEFRAME, types_1.SameCandleAmbiguityMode.LOWER_TIMEFRAME);
        const parentTime = 1700000000000;
        ...
        const m1CandlesLong = Array.from({ length: 15 }, (_, i) => ({
            timestamp: new Date(parentTime + i * 60000),
            open: 100,
            high: 101,
            low: i === 7 ? 94.0 : 96.0,
        ...
        const execSimShort = new execution_simulator_1.ExecutionSimulator(types_1.FillModel.LOWER_TIMEFRAME, types_1.SameCandleAmbiguityMode.LOWER_TIMEFRAME);
        ...
        const m1CandlesShort = Array.from({ length: 15 }, (_, i) => ({
            timestamp: new Date(parentTime + i * 60000),
            open: 100,
    ...
    test('55. requireCandleTimestamp validates Date, numeric, and string timestamps, and throws deterministically on invalid timestamps', () => {
        ...
        expect(fill_model_1.FillModelEngine.requireCandleTimestamp(candleDate)).toBe(epochMs);
        ...
        expect(fill_model_1.FillModelEngine.requireCandleTimestamp(candleNumeric)).toBe(epochMs);
        ...
        expect(fill_model_1.FillModelEngine.requireCandleTimestamp(candleString)).toBe(epochMs);
        // 4. Missing / null / undefined timestamp
        const candleMissing = { open: 100, high: 105, low: 99, close: 102, volume: 100 };
        expect(() => fill_model_1.FillModelEngine.requireCandleTimestamp(candleMissing)).toThrow('Invalid candle: missing timestamp');
        // 5. Invalid NaN / non-finite numeric timestamp
        const candleNaN = { timestamp: NaN, open: 100, high: 105, low: 99, close: 102, volume: 100 };
        expect(() => fill_model_1.FillModelEngine.requireCandleTimestamp(candleNaN)).toThrow('Invalid non-finite candle timestamp value');
        // 6. Invalid unparseable string timestamp
        const candleBadStr = { timestamp: 'invalid-date-string', open: 100, high: 105, low: 99, close: 102, volume: 100 };
        expect(() => fill_model_1.FillModelEngine.requireCandleTimestamp(candleBadStr)).toThrow('Invalid non-finite candle timestamp value');
    });
});
//# sourceMappingURL=execution-correctness-pass.test.js.map