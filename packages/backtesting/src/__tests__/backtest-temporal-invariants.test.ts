...
function createSyntheticCandles(count: number, startPrice = 100): ICandle[] {
...
describe('Backtesting Temporal Invariants', () => {
  it('Future-Data Invariance: Backtest(D, asOf=T) === Backtest(D + FutureData, asOf=T)', () => {
    ...
    const asOfTimestamp = new Date(
      new Date(cutoffCandle.timestamp).getTime() + 15 * 60 * 1000,
    );
  ...
  it('Incremental Replay: Step-by-step replay matches batch backtest at every timestamp T', () => {
    ...
    const cutoffTime = new Date(
      new Date(candles[cutoffIndex].timestamp).getTime() + 15 * 60 * 1000,
    );
