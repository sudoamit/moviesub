import { IBacktestResult, ICandle, SignalGrade, Timeframe } from '@quant/shared';
...
export type AblationVariant = 'SMC_ONLY' | 'SMC_PLUS_REGIME' | 'SMC_PLUS_VOLATILITY' | 'SMC_PLUS_ML' | 'SMC_PLUS_QUANT' | 'FULL_SYSTEM';
...
export type ComponentAblationVariant = 'FULL_SYSTEM_BASELINE' | 'WITHOUT_LIQUIDITY' | 'WITHOUT_ORDER_BLOCK' | 'WITHOUT_FVG' | 'WITHOUT_DISPLACEMENT' | 'WITHOUT_REGIME' | 'WITHOUT_VOLATILITY' | 'WITHOUT_ML' | 'WITHOUT_VOLUME';
...
export interface IBacktestSimulationResult extends Omit<IBacktestResult, 'sharpeRatio' | 'finalEquity' | 'winRate' | 'profitFactor' | 'netPnL' | 'expectancy' | 'maxDrawdownPercent' | 'maxConsecutiveLosses' | 'totalTrades' | 'winningTrades' | 'losingTrades' | 'averageR'>, IQuantitativeMetrics {
    runId?: string;
    symbol: string;
    timeframe: string;
    initialCapital: number;
