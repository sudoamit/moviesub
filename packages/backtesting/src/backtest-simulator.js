...
exports.BacktestSimulator = void 0;
...
class BacktestSimulator {
    ...
    static submitRestingExitOrders(execSim, lot, symbol, policy = risk_engine_1.DEFAULT_PARTIAL_EXIT_POLICY, timestamp) {
        const isLong = lot.direction === shared_1.Direction.BULLISH;
        const exitSide = isLong ? 'SELL' : 'BUY';
        const remainingQty = lot.remainingQuantity;
        if (remainingQty <= 0)
            return;
        const val = risk_engine_1.TradeLifecycleManager.validatePartialExitPolicy(policy);
        if (!val.isValid) {
        ...
        const hasAlreadyTp1 = existingOrders.some((o) => o.exitTarget === 'TP1');
        const hasAlreadyTp2 = existingOrders.some((o) => o.exitTarget === 'TP2');
        const hasAlreadyTp3 = existingOrders.some((o) => o.exitTarget === 'TP3');
        const tp1Qty = Math.round(lot.initialQuantity * policy.tp1Ratio);
        const tp2Qty = policy.tp3Ratio > 0
     ...
     */
    static runSimulation(options) {
        const symbol = options.symbol.toUpperCase();
        const timeframe = options.timeframe || options.executionTimeframe || shared_1.Timeframe.M15;
        ...
        const fillModel = options.fillModel || execution_1.FillModel.NEXT_BAR_MARKET;
        const ambiguityMode = options.ambiguityMode || execution_1.SameCandleAmbiguityMode.CONSERVATIVE;
        const partialPolicy = options.partialExitPolicy || risk_engine_1.DEFAULT_PARTIAL_EXIT_POLICY;
        ...
        if (options.asOfTimestamp) {
            const cutoffMs = options.asOfTimestamp instanceof Date
                ? options.asOfTimestamp.getTime()
                : new Date(options.asOfTimestamp).getTime();
            inputCandles = inputCandles.filter((c) => {
                const openMs = c.timestamp instanceof Date ? c.timestamp.getTime() : new Date(c.timestamp).getTime();
        ...
        const execSim = new execution_1.ExecutionSimulator(fillModel, ambiguityMode);
        let currentCash = initialCapital;
        let currentEquity = initialCapital;
        const runId = options.runId || `bt_${symbol}_${timeframe}_${options.strategyMode || 'SMC'}`;
        const trades = [];
        const positionLots = [];
        ...
        if (!executionCandles || executionCandles.length < minimumCandles) {
            const emptyMetrics = metrics_calculator_1.MetricsCalculator.calculateMetrics([], initialCapital, equityCurve);
            return {
                runId,
                id: `${runId}_res`,
                symbol,
        ...
        let activeLot = null;
        let pendingEntryOrder = null;
        let pendingEntrySignal = null;
        ...
        for (let i = warmupBars; i < executionCandles.length; i++) {
            ...
            if (pendingEntryOrder && pendingEntryOrder.status === 'FILLED') {
                ...
                if (fill && pendingEntrySignal) {
                    const isLong = pendingEntrySignal.direction === shared_1.Direction.BULLISH;
                    ...
                    if (riskDriftRatio > 0.25) {
                        ...
                        activeLot = risk_engine_1.TradeLifecycleManager.createPositionLot(pendingEntrySignal, fill.price, fill.quantity, fill.timestamp, pendingEntryOrder.orderId, fill.fee, fill.slippage);
            ...
            if (activeLot && activeLot.status !== 'CLOSED') {
                const isLong = activeLot.direction === shared_1.Direction.BULLISH;
                ...
                for (const exitFill of tradeFills) {
                    ...
                    const initialRiskPerUnit = Math.max(0.0001, Math.abs(activeLot.entryPrice - activeLot.initialStopLoss));
                    ...
                    activeLot.remainingQuantity = Number(Math.max(0, activeLot.remainingQuantity - fillQty).toFixed(4));
                    cumulativeFees += exitFill.fee;
                    cumulativeSlippage += exitFill.slippage;
                    const filledOrder = execSim.getOrder(exitFill.orderId);
                    const targetType = exitFill.exitTarget ||
                    ...
                    if (activeLot.remainingQuantity <= 0) {
                        ...
                        if (targetType === 'TP1' && partialPolicy.moveStopToBreakevenOnTp1) {
                            ...
                            const slOrder = Array.from(execSim.orders.values()).find((o) => o.tradeId === activeLot.tradeId && o.orderType === 'STOP' && o.status === 'PENDING');
                            if (slOrder) {
                                slOrder.stopPrice = activeLot.entryPrice;
                            }
                        }
                    }
                }
                ...
                if (activeLot && activeLot.status === 'CLOSED') {
                    ...
                    const tradeRecord = {
                        ...
                        pnl: netPnl,
                        pnlRMultiple: Number((netPnl / Math.max(1, initialRiskDist * activeLot.initialQuantity)).toFixed(2)),
                        exitReason: lastFill?.targetType === 'SL' ||
                            lastFill?.targetType === 'STOP' ||
                            lastFill?.targetType === 'STOP_LOSS' ||
                            lastFill?.targetType === 'TRAILING_STOP'
                            ? shared_1.SignalState.SL_HIT
                            : lastFill?.targetType === 'TP3'
                                ? shared_1.SignalState.TP3_HIT
                                : lastFill?.targetType === 'TP2'
                                    ? shared_1.SignalState.TP2_HIT
                                    : shared_1.SignalState.TP1_HIT,
                        signalTimestamp: new Date(activeLot.entrySnapshot?.signalTimestamp || activeLot.openedAt),
                        orderCreatedAt: new Date(activeLot.entrySnapshot?.orderCreatedAt || activeLot.entrySnapshot?.signalTimestamp || activeLot.openedAt),
                        orderSubmittedAt: new Date(activeLot.entrySnapshot?.orderSubmittedAt || activeLot.entrySnapshot?.signalTimestamp || activeLot.openedAt),
                ...
                currentEquity = Number((currentCash + (activeLot ? activeLot.realizedPnl + activeLot.unrealizedPnl : 0)).toFixed(2));
                ...
                equitySnapshots.push({
                    ...
                    marginUsed: activeLot
                        ...
                        : 0,
                    availableMargin: Math.max(0, currentEquity -
                    ...
                    netExposure: activeLot
                        ? (activeLot.direction === shared_1.Direction.BULLISH ? 1 : -1) *
            ...
            if (!activeLot && !pendingEntryOrder) {
                ...
                if (signal.direction !== shared_1.Direction.NEUTRAL &&
                    signal.score >= minScore &&
                    signal.grade !== shared_1.SignalGrade.NO_TRADE) {
                    pendingEntrySignal = signal;
                    const isLong = signal.direction === shared_1.Direction.BULLISH;
                    // Fail-closed position sizing using reference decision price
                    const sizing = risk_engine_1.PositionSizer.calculatePosition({
                    ...
                    if (sizing.isValid && sizing.roundedUnits > 0) {
                        ...
                        const orderType = fillModel === execution_1.FillModel.NEXT_BAR_MARKET ? 'MARKET' : 'LIMIT';
        ...
        const metrics = metrics_calculator_1.MetricsCalculator.calculateMetrics(trades, initialCapital, equityCurve, equitySnapshots, positionLots);
        return {
            runId,
            id: `${runId}_res`,
            symbol,
...
exports.BacktestSimulator = BacktestSimulator;
