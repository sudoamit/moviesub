...
describe('Worker Processors', () => {
    describe('CandleProcessor', () => {
        let processor;
        beforeEach(() => {
            processor = new candle_processor_1.CandleProcessor();
        });
    ...
    describe('ScannerProcessor', () => {
        ...
        beforeEach(() => {
            ...
            processor = new scanner_processor_1.ScannerProcessor(mockPrisma, mockRedis);
        });
    ...
    describe('PositionMonitorProcessor', () => {
        ...
        beforeEach(() => {
            mockPrisma = {
                    ...
                    findMany: jest.fn().mockResolvedValue([
                        {
                            ...
                            quantity: new library_1.Decimal(65),
                            entryPrice: new library_1.Decimal(24100.0),
                            currentPrice: new library_1.Decimal(24100.0),
                            stopLoss: new library_1.Decimal(24050.0),
                            initialStopLoss: new library_1.Decimal(24050.0),
                            target1: new library_1.Decimal(24175.0),
                            target2: new library_1.Decimal(24225.0),
                            target3: new library_1.Decimal(24300.0),
                            initialTarget1: new library_1.Decimal(24175.0),
                            initialTarget2: new library_1.Decimal(24225.0),
                            initialTarget3: new library_1.Decimal(24300.0),
                            leverage: new library_1.Decimal(5.0),
                            usedMargin: new library_1.Decimal(31330.0),
                ...
                candle: {
                    findFirst: jest.fn().mockResolvedValue({ close: new library_1.Decimal(24150.0) }),
                    findMany: jest.fn().mockResolvedValue([]),
                },
            ...
            processor = new position_monitor_processor_1.PositionMonitorProcessor(mockPrisma, mockRedis);
        });
        ...
        it('2. stale ticker returns null and does not close position', async () => {
            ...
            mockRedis.get = jest.fn().mockResolvedValue(JSON.stringify({ price: 24040.0, lastUpdated: Date.now() - 10000 }));
            const mockJob = {
                id: 'monitor-stale',
                name: 'monitor-positions',
                data: {},
            ...
            mockPrisma.paperPosition.findMany.mockResolvedValueOnce([
                {
                    ...
                    target1: new library_1.Decimal(24200.0),
                    target2: new library_1.Decimal(24250.0),
                    target3: new library_1.Decimal(24300.0),
                    initialTarget1: new library_1.Decimal(24200.0),
                    initialTarget2: new library_1.Decimal(24250.0),
                    initialTarget3: new library_1.Decimal(24300.0),
                    leverage: new library_1.Decimal(5.0),
                    usedMargin: new library_1.Decimal(31330.0),
                    unrealizedPnL: new library_1.Decimal(0.0),
            ...
            mockPrisma.paperPosition.findMany.mockResolvedValueOnce([
                {
                    ...
                    entryPrice: new library_1.Decimal(24100.0),
                    currentPrice: new library_1.Decimal(24100.0),
                    stopLoss: new library_1.Decimal(24000.0),
                    initialStopLoss: new library_1.Decimal(24000.0),
                    target1: null,
                    target2: null,
                    target3: null,
                    initialTarget1: null,
        ...
        it('9 & 10. missing risk anchor produces riskDistance = 0 and realizedR = 0', async () => {
            ...
            expect(mockPrisma.paperPosition.update).toHaveBeenCalledWith(expect.objectContaining({
                data: expect.objectContaining({
                    unrealizedR: new library_1.Decimal(0),
                }),
            }));
        });
            ...
            mockPrisma.paperPosition.findMany.mockResolvedValueOnce([
                {
                    ...
                    stopLoss: new library_1.Decimal(24000.0),
                    initialStopLoss: new library_1.Decimal(24000.0),
                    status: shared_1.PositionState.EXIT_PENDING,
                    openedAt: new Date(Date.now() - 60000),
                    chargesJson: { totalCharges: 30.0 },
                },
            ]);
            // Stale tick (10s old)
            mockRedis.get = jest.fn().mockResolvedValue(JSON.stringify({ price: 24150.0, lastUpdated: Date.now() - 10000 }));
            const mockJob = {
        ...
        it('12. EXIT_PENDING closes only with a fresh live ticker', async () => {
            ...
            expect(result.closed).toBe(1);
            expect(mockPrisma.$transaction).toHaveBeenCalled();
            expect(mockPrisma.paperTrade.create).toHaveBeenCalledWith(expect.objectContaining({
                data: expect.objectContaining({
        ...
        it('4b. invalid or far future timestamp is rejected and does not close position', async () => {
            ...
            mockRedis.get = jest.fn().mockResolvedValue(JSON.stringify({ price: 24040.0, lastUpdated: Date.now() + 600000 }));
            const mockJob = {
        ...
        it('13. Execution price provenance records ExecutionPriceSource.LIVE_TICK and sourceTimestamp', async () => {
            ...
            mockRedis.get = jest.fn().mockResolvedValue(JSON.stringify({ price: 24040.0, lastUpdated: tickTime.toISOString() }));
            const mockJob = {
        ...
        it('15. should use configured maxMarketDataAgeSeconds = X dynamically', async () => {
            ...
            jest.clearAllMocks();
            mockRedis.get = jest.fn().mockResolvedValue(JSON.stringify({ price: 24040.0, lastUpdated: Date.now() - 20000 }));
            const jobRejected = {
        ...
        it('16. should use configured maxSlippageBps = X on BUY and SELL position exits', async () => {
            ...
            mockPrisma.paperPosition.findMany = jest.fn().mockResolvedValue([
                {
                    ...
                    quantity: new library_1.Decimal(50),
                    entryPrice: new library_1.Decimal(24100.0),
                    currentPrice: new library_1.Decimal(24100.0),
                    stopLoss: new library_1.Decimal(24150.0),
                    initialStopLoss: new library_1.Decimal(24150.0),
                    target1: new library_1.Decimal(24000.0),
                    target2: new library_1.Decimal(23950.0),
                    target3: new library_1.Decimal(23900.0),
                    leverage: new library_1.Decimal(5.0),
                    usedMargin: new library_1.Decimal(30000.0),
                    unrealizedPnL: new library_1.Decimal(0.0),
                    status: shared_1.PositionState.OPEN,
                    openedAt: new Date(Date.now() - 60000),
                    chargesJson: { totalCharges: 30.0 },
            ...
            mockRedis.get = jest.fn().mockResolvedValue(JSON.stringify({ price: 24160.0, lastUpdated: Date.now() }));
            const jobSellExit = {
        ...
        it('19. worker close performs exact and non-duplicative accounting updates on paperAccount', async () => {
            ...
            expect(mockPrisma.paperAccount.update).toHaveBeenCalledWith(expect.objectContaining({
                where: { id: 'acc-1' },
                data: expect.objectContaining({
        ...
        it('Test A — worker vs worker: simultaneous close operations against same position result in exactly 1 trade, 1 account update, 1 audit event', async () => {
            ...
            const workerJobA = { id: 'job-worker-1', name: 'monitor-positions', data: {} };
            const workerJobB = { id: 'job-worker-2', name: 'monitor-positions', data: {} };
        ...
        it('Test C — worker retry after successful close: recognized existing trade idempotently without duplicate mutations', async () => {
            ...
            const closedPos = {
                ...
                quantity: new library_1.Decimal(50),
                entryPrice: new library_1.Decimal(24100.0),
                currentPrice: new library_1.Decimal(24040.0),
                stopLoss: new library_1.Decimal(24050.0),
                initialStopLoss: new library_1.Decimal(24050.0),
                status: shared_1.PositionState.EXIT_PENDING,
                openedAt: new Date(Date.now() - 60000),
            ...
            const mockExistingTrade = { id: 'trade-existing-1', positionId: 'pos-already-closed', exitPrice: new library_1.Decimal(24040.0) };
            mockPrisma.paperTrade.findFirst = jest.fn().mockResolvedValue(mockExistingTrade);
            const jobRetry = { id: 'job-retry', name: 'monitor-positions', data: {} };
    ...
    describe('LearningProcessor', () => {
        ...
        beforeEach(() => {
            mockPrisma = {
                ...
                candle: {
                    findMany: jest.fn().mockResolvedValue(Array.from({ length: 50 }, (_, i) => ({
                        timestamp: new Date(Date.now() - (50 - i) * 15 * 60000),
                        open: new library_1.Decimal(24000 + i * 2),
                        high: new library_1.Decimal(24010 + i * 2),
                        low: new library_1.Decimal(23990 + i * 2),
                        close: new library_1.Decimal(24005 + i * 2),
                        volume: new library_1.Decimal(1000),
                    }))),
                },
            };
            mockRedis = {
            ...
            processor = new learning_processor_1.LearningProcessor(mockPrisma, mockRedis);
            // Mock buildTrainingDataset to return 40 valid training examples
            processor.buildTrainingDataset = jest.fn().mockResolvedValue(Array.from({ length: 40 }, (_, i) => ({
                id: `ex_${i}`,
                symbol: 'NIFTY',
