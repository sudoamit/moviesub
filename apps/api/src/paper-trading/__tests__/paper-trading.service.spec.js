...
describe('PaperTradingService Persistent Execution & Safety', () => {
    ...
    beforeEach(async () => {
                ...
                update: jest.fn().mockImplementation((args) => {
                    ...
                    if (acc) {
                        if (args.data.cashBalance?.decrement) {
                            acc.cashBalance = new library_1.Decimal(Number(acc.cashBalance) - Number(args.data.cashBalance.decrement));
                        }
                        if (args.data.cashBalance?.increment) {
                            acc.cashBalance = new library_1.Decimal(Number(acc.cashBalance) + Number(args.data.cashBalance.increment));
                        }
                        if (args.data.usedMargin?.increment) {
                            acc.usedMargin = new library_1.Decimal(Number(acc.usedMargin) + Number(args.data.usedMargin.increment));
                        }
                        if (args.data.usedMargin?.decrement) {
                            acc.usedMargin = new library_1.Decimal(Number(acc.usedMargin) - Number(args.data.usedMargin.decrement));
                        }
                        if (args.data.realizedPnL?.increment) {
                            acc.realizedPnL = new library_1.Decimal(Number(acc.realizedPnL) + Number(args.data.realizedPnL.increment));
                        }
                        if (args.data.totalChargesPaid?.increment) {
                            acc.totalChargesPaid = new library_1.Decimal(Number(acc.totalChargesPaid) + Number(args.data.totalChargesPaid.increment));
                        }
                    }
                    return Promise.resolve(acc);
                }),
            ...
            paperPosition: {
                ...
                updateMany: jest.fn().mockImplementation((args) => {
                    ...
                    for (const pos of dbPositions) {
                        ...
                        const matchesStatus = !args.where?.status?.in || args.where.status.in.includes(pos.status);
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [
                paper_trading_service_1.PaperTradingService,
                { provide: prisma_service_1.PrismaService, useValue: mockPrisma },
                { provide: candles_service_1.CandlesService, useValue: mockCandlesService },
                { provide: real_market_streamer_service_1.RealMarketStreamerService, useValue: mockRealMarketStreamer },
            ],
        }).compile();
        service = module.get(paper_trading_service_1.PaperTradingService);
    });
    ...
    it('should reject order if live market data is unavailable and NO fake price or candle fallback exists', async () => {
        ...
        mockRealMarketStreamer.getValidatedTicker.mockImplementation(() => {
            throw new shared_1.MarketDataUnavailableError('UNKNOWN_SYM', 'No stream');
        });
        ...
        expect(rejectedOrder.status).toBe(shared_1.OrderState.REJECTED);
    ...
    it('should close position, book realized PnL and persist PaperTrade', async () => {
        ...
        expect(dbPositions[0].status).toBe(shared_1.PositionState.CLOSED);
    });
    ...
    it('should fail-closed and mark position EXIT_PENDING when real-time exit price cannot be resolved', async () => {
        ...
        mockRealMarketStreamer.getValidatedTicker.mockImplementation(() => {
            throw new shared_1.MarketDataUnavailableError('NIFTY', 'Stream disconnected');
        });
        ...
        expect(dbPositions[0].status).toBe(shared_1.PositionState.EXIT_PENDING);
    });
    it('should prevent concurrency race condition over-allocations when multiple orders execute concurrently', async () => {
        ...
        dbAccounts[0].cashBalance = new library_1.Decimal(300000.0);
        dbAccounts[0].usedMargin = new library_1.Decimal(0.0);
        const orderPromises = [
            service.placeOrder({
                symbol: 'NIFTY',
                direction: 'BUY',
    ...
    it('should execute full end-to-end paper trading lifecycle from live tick entry to exit with preserved snapshots and exact fill prices', async () => {
        ...
        expect(pos.status).toBe(shared_1.PositionState.OPEN);
        expect(pos.featureSnapshotJson).toEqual(featureSnapshot);
    ...
    it('should reject close request if position is already in CLOSING state by worker', async () => {
        ...
        dbPositions[0].status = shared_1.PositionState.CLOSING;
        await expect(service.closePosition(pos.id, 'API Close Race')).rejects.toThrow(/is currently being closed/);
    });
    it('should maintain strict accounting invariants across full trade lifecycle', async () => {
        // Initial account state
        dbAccounts[0].cashBalance = new library_1.Decimal(500000.0);
        dbAccounts[0].usedMargin = new library_1.Decimal(0.0);
        dbAccounts[0].realizedPnL = new library_1.Decimal(0.0);
        dbAccounts[0].totalChargesPaid = new library_1.Decimal(0.0);
        mockRealMarketStreamer.getValidatedTicker.mockReturnValue({
            symbol: 'NIFTY',
            price: 24100.0,
            timestamp: Date.now(),
...
//# sourceMappingURL=paper-trading.service.spec.js.map