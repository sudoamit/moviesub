...
let OptionsService = OptionsService_1 = class OptionsService {
     ...
     */
    async fetchLiveNSEChain(symbol) {
        ...
        try {
            ...
            this.logger.debug(`Live NSE options fetch notice: ${err.message}`);
        }
        return cached?.data || [];
    }
    /**
     ...
     */
    async getOptionChain(symbol = 'NIFTY', targetExpiryDate, spotPriceOverride) {
        const sym = symbol.toUpperCase();
        ...
        const expiries = trading_engine_1.IndianOptionsExpiryEngine.getUpcomingExpiries(sym);
        const selectedExpiryInfo = expiries.find((e) => e.dateString === targetExpiryDate) || expiries[0];
        const timeToExpiryYears = selectedExpiryInfo.timeToExpiryYears;
        const daysToExpiry = selectedExpiryInfo.daysToExpiry;
        // Lot Size Mapping
        const lotSize = sym === 'NIFTY'
            ? 65
            : sym === 'BANKNIFTY'
                ? 15
                : sym === 'FINNIFTY'
        ...
        for (let i = -7; i <= 7; i++) {
            ...
            const exchangeContract = rawExchangeChains.find((c) => Math.round(c.strikePrice / 100) === strikePrice);
            ...
            const bsCall = trading_engine_1.BlackScholesModel.calculate(spotPrice, strikePrice, timeToExpiryYears, 0.07, callIV, 'CE');
            const bsPut = trading_engine_1.BlackScholesModel.calculate(spotPrice, strikePrice, timeToExpiryYears, 0.07, putIV, 'PE');
            ...
            strikes.push({
                ...
                call: {
                    ...
                    oi: callEffectiveOI,
                    oiChange: exchangeContract?.callOption?.oiChange ||
                ...
                put: {
                    ...
                    oi: putEffectiveOI,
                    oiChange: exchangeContract?.putOption?.oiChange ||
        ...
        for (const s of strikes) {
            netGammaExposure += s.netGex || 0;
            if (Math.abs(s.netGex || 0) < minGexDistance) {
                minGexDistance = Math.abs(s.netGex || 0);
    ...
    async getSmartOptionRecommendation(symbol, direction, spotTarget, spotStopLoss, targetExpiryDate, spotPriceOverride, strikeOverride) {
    ...
    async getSmartStrikeRecommendation(symbol, direction, spotTarget, spotStopLoss, targetExpiryDate, spotPriceOverride, strikeOverride) {
