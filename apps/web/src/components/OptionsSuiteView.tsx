...
export const OptionsSuiteView: React.FC<OptionsSuiteViewProps> = ({
  initialSymbol = 'NIFTY',
  liveSpotPrice,
}) => {
  ...
  const [viewMode, setViewMode] = useState<'chain' | 'oi_bars' | 'greeks'>('chain');

  const fetchOptionChain = async (sym: string, expiry?: string, silent = false) => {
    try {
  ...
  return (
    <div className="bg-[#0B0F19] border border-slate-800 rounded-xl p-4 sm:p-6 font-mono space-y-6">
      {/* Header & Controls */}
              ...
              {data.strikes.map((row: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* STRIKE */}
                    <td
                      className={`py-2 px-3 text-center font-black border-r border-slate-800 ${
                        isATM
