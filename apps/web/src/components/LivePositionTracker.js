...
exports.LivePositionTracker = void 0;
 ...
 */
function getMarketAwareTimestamps(symbol, signalTimestamp) {
...
const LivePositionTracker = ({ symbol, signal, livePrice, onClosePosition, }) => {
    const isCrypto = symbol === 'BTCUSDT' ||
    ...
    const [isOptionMode, setIsOptionMode] = (0, react_1.useState)(!isCrypto && !isGold);
    (0, react_1.useEffect)(() => {
        setIsOptionMode(!isCrypto && !isGold);
    }, [isCrypto, isGold]);
    const [optionData, setOptionData] = (0, react_1.useState)(null);
    ...
    const [lockedExecutionEntryTime, setLockedExecutionEntryTime] = (0, react_1.useState)('2026-09-03T09:15:00.000Z');
    (0, react_1.useEffect)(() => {
        let isMounted = true;
        const fetchPaperPosition = async () => {
            try {
                ...
                const found = positions.find((p) => p.symbol === symbol && p.direction === expectedSide) || positions.find((p) => p.symbol === symbol);
                if (!isMounted)
                    return;
    ...
    (0, react_1.useEffect)(() => {
        if (typeof window === 'undefined')
            return;
        if (paperPosition?.entryTime || paperPosition?.openedAt)
            return;
    ...
    const { entryDate, estCloseDate, isMarketOpen, marketSessionLabel, isNSE } = (0, react_1.useMemo)(() => {
        const authoritativeEntryTime = paperPosition?.entryTime || paperPosition?.openedAt || lockedExecutionEntryTime;
    ...
    const [elapsedSeconds, setElapsedSeconds] = (0, react_1.useState)(2700);
    (0, react_1.useEffect)(() => {
    ...
    const formattedElapsed = (0, react_1.useMemo)(() => {
    ...
    const [isBreakevenActive, setIsBreakevenActive] = (0, react_1.useState)(false);
    const [isTrailingSLActive, setIsTrailingSLActive] = (0, react_1.useState)(false);
    const [isAutoScaledOut, setIsAutoScaledOut] = (0, react_1.useState)(false);
    const [scaledOutPnL, setScaledOutPnL] = (0, react_1.useState)(0);
    const [manualCloseToast, setManualCloseToast] = (0, react_1.useState)(null);
    // Sanity check: ensure spot price matches instrument magnitude (prevents NIFTY price in BTC)
    const isSaneSpot = react_1.default.useCallback((sym, price) => {
        if (!price || price <= 0 || isNaN(price))
            return false;
        if (sym === 'BTCUSDT' || sym.toUpperCase().includes('BTC'))
            return price >= 50000;
        ...
        if (sym === 'BANKNIFTY')
            return price >= 35000 && price <= 75000;
        if (sym === 'RELIANCE')
            return price >= 500 && price <= 5000;
        if (sym === 'HDFCBANK')
            return price >= 300 && price <= 2000;
        if (sym === 'INFY')
            return price >= 500 && price <= 3000;
        return true;
    }, []);
    const isSaneOptionPremium = react_1.default.useCallback((sym, price) => {
        return ((sym === 'NIFTY' || sym === 'BANKNIFTY') &&
            Number.isFinite(price) &&
            price > 0 &&
            price < 5000);
    }, []);
    const isSanePersistedExit = react_1.default.useCallback((sym, price) => {
    ...
    const [isPositionCut, setIsPositionCut] = (0, react_1.useState)(false);
    const [closedTradeSummary, setClosedTradeSummary] = (0, react_1.useState)(null);
    // Client-side hydration sync & cross-window/tab event synchronization
    (0, react_1.useEffect)(() => {
        if (typeof window !== 'undefined') {
            const isCut = localStorage.getItem(cutStorageKey) === 'true' ||
            ...
            if (savedPnl)
                setScaledOutPnL(Number(savedPnl));
        }
    }, [
        symbol,
        positionLockKey,
    ...
    (0, react_1.useEffect)(() => {
        if (typeof window === 'undefined')
            return;
        const handleTradeClosedEvent = (e) => {
        ...
        const handleTradeOpenedEvent = (e) => {
            const targetSym = e?.detail?.symbol;
            if (!targetSym || targetSym === 'ALL' || targetSym === symbol) {
            ...
            : symbol === 'BTCUSDT'
                ...
                : symbol === 'XAUUSD' || symbol === 'GOLD'
                    ...
                    : symbol === 'BANKNIFTY'
                        ? 57400
                        : 24060;
    const [lockedSpotEntry, setLockedSpotEntry] = (0, react_1.useState)(initialSpotCandidate);
    const lockedSpotRef = react_1.default.useRef(lockedSpotEntry);
    ...
    const defaultStrike = (0, react_1.useMemo)(() => {
        if (symbol === 'NIFTY')
            return Math.round(spotEntryPrice / 50) * 50;
        if (symbol === 'BANKNIFTY')
            return Math.round(spotEntryPrice / 100) * 100;
        return Math.round(spotEntryPrice);
    }, [symbol, spotEntryPrice]);
    const [lockedStrike, setLockedStrike] = (0, react_1.useState)(defaultStrike);
    ...
    const [lockedEntryPremium, setLockedEntryPremium] = (0, react_1.useState)(0);
    const lockedEntryRef = react_1.default.useRef(lockedEntryPremium);
    (0, react_1.useEffect)(() => {
    ...
    const rawOptionRiskPts = optionEntryPremium > 40
    ...
    const safeInitialSL = react_1.default.useMemo(() => {
        const rawSL = Number(signal?.stopLoss);
        if (isOptionMode && !isCrypto)
            return optionStopLoss;
        ...
        if (isBull) {
            if (rawSL > 0 && rawSL < spotEntryPrice) {
                ...
                if (isCrypto && dist > 250)
                    return Number((spotEntryPrice - 180.0).toFixed(2));
                return rawSL;
            }
    ...
    const trailingSL = react_1.default.useMemo(() => {
        if (!isTrailingSLActive || riskPerUnit <= 0)
            return originalSL;
        if (isOptionMode && !isCrypto) {
    ...
    const isSpotSLHit = isBull
        ...
        : currentSL > spotEntryPrice && currentCMP > 0 && currentCMP >= currentSL;
    const isOptionSLHit = isOptionMode &&
        !isCrypto &&
    ...
    const autoCutExecutedRef = react_1.default.useRef(null);
    const handleCutTrade = async (reason, exitP, partialRatio = 1.0) => {
        const isValidExitPrice = (price) => isOptionMode && !isCrypto ? isSaneOptionPremium(symbol, price) : isSaneSpot(symbol, price);
        ...
        if (partialRatio < 1.0) {
            ...
            setManualCloseToast(`✂️ Partial Scale-Out Executed: 50% booked @ ${currencySymbol}${finalExitPrice.toFixed(2)} (+${currencySymbol}${finalPnL.toFixed(2)} / ${finalR}R). SL moved to BREAKEVEN (0 Risk)!`);
            setTimeout(() => setManualCloseToast(null), 8000);
            return;
        }
        setIsPositionCut(true);
        const isManualCut = reason.toLowerCase().includes('manual') ||
        ...
        if (paperPosition && paperPosition.id) {
            try {
        ...
        setManualCloseToast(`⚡ Position Exited: ${reason} @ ${currencySymbol}${finalExitPrice.toFixed(2)} | Realized: ${finalPnL + scaledOutPnL >= 0 ? '+' : ''}${currencySymbol}${(finalPnL + scaledOutPnL).toFixed(2)} (${finalR}R)`);
        setTimeout(() => setManualCloseToast(null), 8000);
    };
    ...
    (0, react_1.useEffect)(() => {
        ...
        if (isSLReached && autoCutExecutedRef.current !== `SL_${symbol}_${direction}`) {
            ...
            handleCutTrade(isTrailingSLActive
                ? 'Trailing Stop Loss Hit (Capital Protected)'
                : 'Stop Loss Hit (Automatic Exit)', currentSL);
        }
    ...
    const handleToggleBreakeven = () => {
        ...
        setManualCloseToast(!isBreakevenActive
    ...
    const handleToggleTrailing = () => {
        ...
        setManualCloseToast(!isTrailingSLActive
    ...
    if (isSetupClosed) {
        return (<div className="bg-[#111827]/80 backdrop-blur-md border border-slate-800 rounded-xl p-5 shadow-xl font-mono flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleReopenTrade} className="text-xs text-slate-300 hover:text-white bg-slate-800/80 hover:bg-slate-700 border border-slate-700 px-3 py-1.5 rounded-lg transition-all font-bold" title="Reset tracker to track next setup">
            Track Next Setup
          </button>
          <span className="text-xs text-cyan-400 font-bold bg-cyan-950/60 border border-cyan-800/60 px-3 py-1.5 rounded-lg flex items-center gap-1.5 animate-pulse">
            <lucide_react_1.Radio className="w-3.5 h-3.5"/> Engine Standing By for Valid Setup
          </span>
        </div>
      </div>);
    }
    return (<div className="bg-[#111827]/95 backdrop-blur-md border border-cyan-500/30 rounded-xl p-5 shadow-2xl space-y-4 relative overflow-hidden">
      {/* Top Background Glow Effect */}
      <div className={`absolute -top-12 -right-12 w-40 h-40 rounded-full blur-3xl opacity-20 pointer-events-none ${runningPnL >= 0 ? 'bg-emerald-500' : 'bg-rose-500'}`}/>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800/80 pb-3">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-black uppercase tracking-wider font-mono flex items-center gap-1.5 text-white">
              <lucide_react_1.Zap className="w-4 h-4 text-cyan-400"/>
          ...
          <span className={`px-2 py-0.5 rounded text-[10px] font-bold font-mono flex items-center gap-1 ${isBull
            ? 'bg-emerald-950/80 text-emerald-400 border border-emerald-800/80'
            : 'bg-rose-950/80 text-rose-400 border border-rose-800/80'}`}>
            {isBull ? (<lucide_react_1.TrendingUp className="w-3.5 h-3.5"/>) : (<lucide_react_1.TrendingDown className="w-3.5 h-3.5"/>)}
          ...
          {!isCrypto && (<button onClick={() => setIsOptionMode(!isOptionMode)} className={`text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-full border transition-all flex items-center gap-1 shadow-sm ${isOptionMode
                ? 'bg-gradient-to-r from-cyan-500/20 to-indigo-500/20 text-cyan-300 border-cyan-500/50 shadow-cyan-500/10'
                : 'bg-slate-900 text-slate-400 border-slate-700 hover:text-white'}`}>
              <lucide_react_1.Sliders className="w-3 h-3 text-cyan-400"/>
              {isOptionMode ? '⚡ OPTION PREMIUM MODE' : '📊 SPOT PRICE MODE'}
            </button>)}

          {/* Market Status Pill */}
          <span suppressHydrationWarning className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded border flex items-center gap-1 ${isMarketOpen
            ? 'bg-emerald-950/80 text-emerald-300 border-emerald-700'
            : 'bg-amber-950/80 text-amber-300 border-amber-700'}`}>
            {isMarketOpen ? (<lucide_react_1.Sun className="w-3 h-3 text-emerald-400"/>) : (<lucide_react_1.Moon className="w-3 h-3 text-amber-400"/>)}
            {marketSessionLabel}
          </span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 font-mono text-[11px]">
          <span className="bg-slate-900 border border-slate-800 px-2.5 py-1 rounded text-cyan-300 flex items-center gap-1 font-bold" suppressHydrationWarning>
            <lucide_react_1.Calendar className="w-3.5 h-3.5 text-cyan-400"/>
            ENTRY: {formatDateTimeIST(entryDate)}
          </span>

          <span className="bg-slate-900 border border-slate-800 px-2.5 py-1 rounded text-amber-300 flex items-center gap-1 font-bold" suppressHydrationWarning>
            <lucide_react_1.Timer className="w-3.5 h-3.5 text-amber-400"/>
      ...
      {isOptionMode && !isCrypto && optionData && (<div className="bg-gradient-to-r from-slate-900 via-indigo-950/60 to-slate-900 border border-indigo-500/40 rounded-xl p-3 font-mono space-y-2 text-xs shadow-inner">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-3 text-[11px]">
              ...
              <span className="text-emerald-400 font-bold flex items-center gap-1">
                <lucide_react_1.ShieldCheck className="w-3.5 h-3.5 text-emerald-400"/>
          ...
          {lockedStrikeLabel && (<div className="flex flex-wrap items-center gap-1.5 pt-1">
              <span className="text-[10px] text-slate-400 flex items-center gap-1 mr-1">
                <lucide_react_1.Radio className="w-3 h-3 text-cyan-400"/> Locked Strike:
              </span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3 font-mono">
        ...
        {(() => {
            ...
            return (<div className={`p-3.5 rounded-xl border col-span-2 sm:col-span-2 relative overflow-hidden ${isClosed
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  <span className={`text-xs font-black px-2 py-0.5 rounded ${isProfitableDisplay
              ...
              <div className="flex items-baseline gap-2 mt-1">
                <span className={`text-2xl sm:text-3xl font-black tracking-tight ${isProfitableDisplay ? 'text-emerald-400' : 'text-rose-400'}`}>
                  {displayPnL >= 0 ? '+' : ''}
                  {currencySymbol}
                ...
                <span className={`text-sm font-bold ${isProfitableDisplay ? 'text-emerald-400/90' : 'text-rose-400/90'}`}>
              ...
              <span className="text-[10px] text-slate-300 block mt-1.5 font-bold">
                {isClosed ? (<span className="text-slate-400 block mt-0.5">
        ...
        <div className="bg-slate-900/90 border border-slate-800 p-3.5 rounded-xl">
          <div className="flex items-center justify-between">
            ...
            {isProfitLocked ? (<span className="text-[9px] bg-emerald-950 text-emerald-300 border border-emerald-800/80 px-1.5 py-0.5 rounded font-bold">
                PROFIT LOCKED
              </span>) : isBreakevenActive ? (<span className="text-[9px] bg-cyan-950 text-cyan-400 border border-cyan-800/80 px-1.5 py-0.5 rounded font-bold">
                BE ACTIVE
              </span>) : null}
          </div>
          <span className={`text-xl font-black block mt-1 ${isProfitLocked
            ? 'text-emerald-400'
            : isBreakevenActive
                ? 'text-cyan-400'
                : 'text-rose-400'}`}>
      ...
      <div className="bg-slate-900/80 border border-slate-800 p-3 rounded-lg space-y-1.5 font-mono">
        <div className="flex items-center justify-between text-[11px]">
          <span className="text-slate-400 flex items-center gap-1 font-bold">
            <lucide_react_1.Target className="w-3.5 h-3.5 text-cyan-400"/>
            {isOptionMode && !isCrypto
        ...
        <div className="w-full bg-slate-950 rounded-full h-2.5 overflow-hidden border border-slate-800 relative">
          <div className={`h-full transition-all duration-300 rounded-full ${isTP2Reached
            ? 'bg-gradient-to-r from-emerald-500 to-teal-400'
            : isTP1Reached
                ? 'bg-gradient-to-r from-cyan-500 to-emerald-400'
                : 'bg-cyan-500'}`} style={{ width: `${progressPercent}%` }}/>
        </div>
      ...
      {isAutoScaledOut && !isPositionCut && (<div className="bg-emerald-950/60 border border-emerald-500/50 p-2.5 rounded-lg font-mono flex items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-2 text-emerald-300 font-bold">
            <lucide_react_1.Award className="w-4 h-4 text-emerald-400 shrink-0"/>
            <span>
      ...
      {isPositionCut ? (<div className="w-full bg-slate-900/90 border border-slate-800 p-3 rounded-xl flex flex-wrap items-center justify-between gap-3 font-mono text-xs shadow-lg">
          <div className="flex items-center gap-2 text-slate-300">
            <lucide_react_1.CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0"/>
            <span>
        ...
        </div>) : (<div className="flex flex-wrap items-center justify-between gap-3 pt-1 font-mono text-xs">
          <div className="flex flex-wrap items-center gap-2">
            {/* Move to Breakeven Toggle */}
            <button onClick={handleToggleBreakeven} className={`px-3 py-1.5 rounded-lg border font-bold flex items-center gap-1.5 transition-all ${isBreakevenActive || isAutoScaledOut
            ...
            <button onClick={handleToggleTrailing} className={`px-3 py-1.5 rounded-lg border font-bold flex items-center gap-1.5 transition-all ${isTrailingSLActive
                ? 'bg-amber-400 text-slate-950 border-amber-300 shadow-md shadow-amber-400/20'
                : 'bg-slate-900 border-slate-700 text-slate-300 hover:bg-slate-800'}`}>
              <lucide_react_1.TrendingUp className="w-3.5 h-3.5"/>
              {isTrailingSLActive
            ...
            {!isAutoScaledOut && (<button onClick={() => handleCutTrade('Manual 50% Scale Out', effectiveCurrentPrice, 0.5)} className="bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 border border-emerald-500/40 px-3 py-1.5 rounded-lg font-bold flex items-center gap-1.5 transition-all">
                <lucide_react_1.Award className="w-3.5 h-3.5"/>
                Scale Out 50% Now
              </button>)}
          </div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => handleCutTrade('Manual Market Cut @ Current Price')} className="bg-rose-500 hover:bg-rose-400 text-slate-950 font-black px-4 py-1.5 rounded-lg font-mono flex items-center gap-1.5 shadow-lg shadow-rose-500/20 transition-all text-xs">
              <lucide_react_1.XCircle className="w-4 h-4"/>⚡ Exit Trade @ Market ({currencySymbol}
      ...
      {/* Action Toast Banner */}
      {manualCloseToast && (<div className="bg-slate-900 border border-cyan-500 p-2.5 rounded-lg text-xs font-mono text-cyan-300 animate-in fade-in flex items-center gap-2">
          <lucide_react_1.CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0"/>
          <span>{manualCloseToast}</span>
        </div>)}
    </div>);
};
exports.LivePositionTracker = LivePositionTracker;
//# sourceMappingURL=LivePositionTracker.js.map