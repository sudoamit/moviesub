...
exports.AITradeLearningWidget = void 0;
const react_1 = __importStar(require("react"));
const lucide_react_1 = require("lucide-react");
const AITradeLearningWidget = ({ initialSymbol = 'NIFTY', }) => {
    const [selectedSymbol, setSelectedSymbol] = (0, react_1.useState)(initialSymbol);
    const [selectedTimeframe, setSelectedTimeframe] = (0, react_1.useState)('15m');
    const [modelState, setModelState] = (0, react_1.useState)(null);
    const [prediction, setPrediction] = (0, react_1.useState)(null);
    const [insights, setInsights] = (0, react_1.useState)(null);
    const [isLoading, setIsLoading] = (0, react_1.useState)(true);
    const [isPredicting, setIsPredicting] = (0, react_1.useState)(false);
    // Retrain State
    const [isRetraining, setIsRetraining] = (0, react_1.useState)(false);
    const [retrainJobId, setRetrainJobId] = (0, react_1.useState)(null);
    const [retrainStatus, setRetrainStatus] = (0, react_1.useState)(null);
    // 1. Fetch Model State
    const fetchModelState = (0, react_1.useCallback)(async () => {
        try {
    ...
    // 2. Fetch Insights
    const fetchInsights = (0, react_1.useCallback)(async () => {
        try {
    ...
    // 3. Fetch Prediction
    const fetchPrediction = (0, react_1.useCallback)(async (sym, tf) => {
    ...
    return (<div className="space-y-6 font-mono text-slate-200">
      {/* Header Banner */}
      <div className="bg-[#0B0F19]/90 border border-slate-800 rounded-xl p-5 shadow-2xl backdrop-blur-xl flex flex-wrap items-center justify-between gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="bg-emerald-950/80 text-emerald-400 border border-emerald-500/40 text-[10px] font-bold px-2 py-0.5 rounded flex items-center gap-1">
                <lucide_react_1.CheckCircle2 className="w-3 h-3"/> ACTIVE PRODUCTION
              </span>
            </div>
        ...
        <div className="flex items-center gap-2.5">
          <button onClick={handleExportModelArtifact} className="flex items-center gap-1.5 px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs rounded-lg border border-slate-700 transition-all shadow" title="Download Active Model JSON Artifact">
            <lucide_react_1.Download className="w-3.5 h-3.5 text-cyan-400"/>
            <span>EXPORT MODEL JSON</span>
          </button>

          <button onClick={handleStartRetrain} disabled={isRetraining} className="flex items-center gap-2 px-3.5 py-2 bg-gradient-to-r from-cyan-600 to-teal-500 hover:from-cyan-500 hover:to-teal-400 text-slate-950 font-black text-xs rounded-lg shadow-lg shadow-cyan-500/25 transition-all disabled:opacity-50">
            <lucide_react_1.RefreshCw className={`w-3.5 h-3.5 ${isRetraining ? 'animate-spin' : ''}`}/>
      ...
      {retrainStatus && (<div className="bg-slate-900/90 border border-cyan-500/40 rounded-xl p-4 shadow-xl animate-in fade-in duration-200 space-y-2.5">
          ...
          <div className="w-full bg-slate-800 rounded-full h-2 overflow-hidden">
            <div className="bg-gradient-to-r from-cyan-500 via-teal-400 to-emerald-400 h-2 transition-all duration-300 rounded-full" style={{ width: `${retrainStatus.progressPercent}%` }}/>
          </div>

          {retrainStatus.promotionDecision && (<div className={`text-xs p-2 rounded border font-bold flex items-center gap-2 ${retrainStatus.isPromoted
              ...
              {retrainStatus.isPromoted ? (<>
                  ...
                  <lucide_react_1.Info className="w-4 h-4 text-amber-400 shrink-0"/>
                  <span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-7 bg-[#111827]/90 border border-slate-800 rounded-xl p-5 shadow-xl space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-3">
            <div className="flex items-center gap-2">
              <lucide_react_1.Target className="w-4 h-4 text-cyan-400"/>
            ...
            <div className="flex items-center gap-1.5 bg-slate-900 border border-slate-800 p-1 rounded-lg">
              {symbols.map((sym) => (<button key={sym} onClick={() => setSelectedSymbol(sym)} className={`px-2.5 py-0.5 rounded text-xs font-bold transition-all ${selectedSymbol === sym
          ...
          {prediction ? (<div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="bg-slate-900/90 border border-slate-800 p-3 rounded-lg text-center">
                  ...
                  <div className="flex items-center justify-center gap-1.5 mt-1">
                    <span className={`text-xl font-black ${prediction.aiPrediction.expectedValueR >= 0
                ? 'text-emerald-400'
                : 'text-rose-400'}`}>
              ...
              <div className="bg-slate-900/80 border border-slate-800 p-3.5 rounded-lg flex items-center justify-between gap-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className={`px-3 py-1 rounded font-black text-xs border tracking-wider flex items-center gap-1.5 ${prediction.aiPrediction.recommendation === 'HIGH_CONFIDENCE'
              ...
              <div className="bg-slate-950/60 p-3 rounded-lg border border-slate-800/80 text-xs text-slate-300 space-y-1">
                {prediction.aiPrediction.reasons.map((r, idx) => (<div key={idx} className="flex items-start gap-2">
              ...
              <div className="text-[10px] text-slate-500 border-t border-slate-800 pt-2 flex items-center gap-1.5">
                <lucide_react_1.Shield className="w-3.5 h-3.5 text-slate-400 shrink-0"/>
                <span>
                  Deterministic Risk Engine strictly manages invalidation price ($
                  {prediction.stopLoss}) and position size.
                </span>
              </div>
            </div>) : (<div className="h-40 flex items-center justify-center text-slate-500 text-xs">
        ...
        <div className="lg:col-span-5 bg-[#111827]/90 border border-slate-800 rounded-xl p-5 shadow-xl space-y-4">
          ...
          <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
            ...
            return (<div key={fi.feature} className="text-xs font-mono">
                  ...
                  <div className="w-full bg-slate-900 rounded-full h-1.5 overflow-hidden flex items-center">
                    <div className={`h-1.5 rounded-full ${isPositive
                    ? 'bg-gradient-to-r from-teal-500 to-emerald-400'
                    : 'bg-gradient-to-r from-amber-500 to-rose-400'}`} style={{ width: `${Math.max(5, widthPct)}%` }}/>
                  </div>
                </div>);
        })}
          </div>
      ...
      <div className="bg-[#111827]/90 border border-slate-800 rounded-xl p-5 shadow-xl space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            <lucide_react_1.Sliders className="w-4 h-4 text-cyan-400"/>
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-200">
        ...
        <div className="bg-slate-950/70 border border-slate-800/80 p-4 rounded-xl space-y-3">
          ...
          <div className="relative h-48 w-full">
            ...
            return (<rect key={`hist-${i}`} x={x + 3} y={160 - barHeight} width={barWidth} height={barHeight} fill="#0f172a" stroke="#1e293b" rx="2" opacity="0.7"/>);
        })}

              {/* Realized Curve Polyline */}
              <polyline fill="none" stroke="url(#calibGradient)" strokeWidth="2.5" points={calibrationDeciles
            .map((bin, i) => {
            ...
            return (<g key={`pt-${i}`} className="group cursor-pointer">
                    <circle cx={x} cy={y} r="4.5" fill={isOptimal ? '#34d399' : '#38bdf8'} stroke="#0f172a" strokeWidth="2" className="transition-transform group-hover:scale-150"/>
                    <circle cx={x} cy={y} r="8" fill="transparent" stroke={isOptimal ? '#34d399' : '#38bdf8'} strokeWidth="1" opacity="0.3" className="animate-ping"/>
                  </g>);
        })}
            ...
            return (<text key={`xlabel-${i}`} x={x} y="176" fill="#64748b" fontSize="8" textAnchor="middle">
                    {bin.range}
                  </text>);
        })}
            </svg>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 lg:grid-cols-10 gap-2 pt-1 font-mono">
          {calibrationDeciles.map((bin) => {
            ...
            return (<div key={bin.range} className="bg-slate-900/90 border border-slate-800 rounded-lg p-2 text-center text-xs space-y-1">
                ...
                <div className="w-full bg-slate-800 rounded-full h-1 overflow-hidden">
                  <div className={`h-1 rounded-full ${isGood ? 'bg-emerald-400' : 'bg-cyan-400'}`} style={{ width: `${bin.obs * 100}%` }}/>
                </div>
      ...
      <div className="bg-[#111827]/90 border border-slate-800 rounded-xl p-5 shadow-xl space-y-3">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            <lucide_react_1.Clock className="w-4 h-4 text-cyan-400"/>
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-200">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs font-mono">
            ...
            return (<tr key={idx} className="hover:bg-slate-800/30 transition-colors">
                    <td className="py-2.5 font-bold text-slate-200 flex items-center gap-1.5">
                      <span>{pm.symbol}</span>
                      <span className={`text-[9px] px-1 py-0.5 rounded ${pm.direction === 'BULLISH' ? 'bg-emerald-950 text-emerald-400' : 'bg-rose-950 text-rose-400'}`}>
                        {pm.direction}
                      </span>
                    </td>
                    <td className="py-2.5">
                    ...
                    <td className="py-2.5">
                      <div className="space-y-0.5">
                        <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-bold border ${pm.classification === 'TARGET_ACHIEVED'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-[#111827]/90 border border-slate-800 rounded-xl p-5 shadow-xl space-y-3">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs font-mono">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {insights?.performanceByRegime.map((r) => (<tr key={r.regime} className="hover:bg-slate-800/30 transition-colors">
        ...
        <div className="bg-[#111827]/90 border border-slate-800 rounded-xl p-5 shadow-xl space-y-3">
          <div className="flex items-center gap-2 border-b border-slate-800 pb-3">
            <lucide_react_1.Award className="w-4 h-4 text-teal-400"/>
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-200">
          ...
          <div className="space-y-2.5">
            {insights?.postMortemPatterns.map((pm, idx) => (<div key={idx} className="bg-slate-900/80 border border-slate-800 p-3 rounded-lg text-xs space-y-1 hover:border-slate-700 transition-colors">
...
exports.AITradeLearningWidget = AITradeLearningWidget;
//# sourceMappingURL=AITradeLearningWidget.js.map