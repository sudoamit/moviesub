...
exports.AICopilotModal = void 0;
const react_1 = __importStar(require("react"));
const lucide_react_1 = require("lucide-react");
const AICopilotModal = ({ isOpen, onClose, selectedSymbol, }) => {
    const [activeTab, setActiveTab] = (0, react_1.useState)('symbol_ai');
    const [symbolData, setSymbolData] = (0, react_1.useState)(null);
    const [briefingData, setBriefingData] = (0, react_1.useState)(null);
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      <div className="bg-[#111827] border border-cyan-500/40 rounded-2xl w-full max-w-3xl shadow-2xl overflow-hidden font-mono text-slate-200 flex flex-col max-h-[85vh]">
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-800 bg-[#0B0F19]">
          ...
          <div className="flex items-center gap-2">
            <button onClick={fetchAIAnalysis} className="p-1.5 rounded-lg bg-slate-800 text-slate-400 hover:text-cyan-300 transition-colors" title="Refresh AI Analysis">
        ...
        <div className="flex border-b border-slate-800 bg-slate-900/60 px-4 text-xs font-bold">
          <button onClick={() => setActiveTab('symbol_ai')} className={`py-3 px-4 border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'symbol_ai'
          ...
          <button onClick={() => setActiveTab('daily_briefing')} className={`py-3 px-4 border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'daily_briefing'
            ? 'border-cyan-400 text-cyan-300'
            : 'border-transparent text-slate-400 hover:text-slate-200'}`}>
        ...
        <div className="p-5 space-y-5 overflow-y-auto flex-1">
          {isLoading ? (<div className="py-12 text-center text-xs text-slate-400 space-y-2">
              <lucide_react_1.RefreshCw className="w-6 h-6 text-cyan-400 animate-spin mx-auto"/>
              <p>Analyzing order blocks, multi-timeframe liquidity sweeps, and market regime...</p>
            </div>) : activeTab === 'symbol_ai' && symbolData ? (<div className="space-y-4">
              ...
              <div className="bg-slate-900/90 border border-slate-800 p-4 rounded-xl space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-2.5 py-0.5 rounded text-xs font-black border ${symbolData.bias === 'BULLISH'
              ...
              {/* Risk Execution Plan */}
              {symbolData.riskParameters && (<div className="bg-slate-900/90 border border-slate-800 p-4 rounded-xl space-y-2 text-xs">
              ...
              <div className="bg-slate-900/60 border border-slate-800 p-3.5 rounded-xl space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                  {symbolData.confirmedChecklist?.map((item, idx) => (<div key={idx} className="flex items-center gap-1.5 text-[11px] text-slate-300">
                      <lucide_react_1.CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0"/>
                      <span>{item}</span>
                    </div>))}
                </div>
              </div>
              ...
              {briefingData?.marketBreadth && (<div className="grid grid-cols-3 gap-3">
              ...
              <div className="space-y-3">
                ...
                {briefingData?.topOpportunities?.map((opp) => (<div key={opp.symbol} className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-2 text-xs">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 font-bold">
                        <span className="text-white text-sm">{opp.symbol}</span>
                        <span className={`px-2 py-0.5 rounded text-[10px] ${opp.direction === 'BULLISH'
...
exports.AICopilotModal = AICopilotModal;
//# sourceMappingURL=AICopilotModal.js.map