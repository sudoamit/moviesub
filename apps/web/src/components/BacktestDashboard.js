...
exports.BacktestDashboard = void 0;
const react_1 = __importStar(require("react"));
const lucide_react_1 = require("lucide-react");
const BacktestDashboard = ({ initialSymbol = 'NIFTY', }) => {
    const [symbol, setSymbol] = (0, react_1.useState)(initialSymbol);
    const [timeframe, setTimeframe] = (0, react_1.useState)('15m');
    const [initialCapital, setInitialCapital] = (0, react_1.useState)(50000);
    ...
    const equityCurve = results?.equityCurve || results?.parametersJson?.equityCurve || [];
    return (<div className="space-y-6 font-mono">
      ...
      <div className="bg-[#111827]/95 border border-cyan-500/30 rounded-2xl p-5 shadow-2xl backdrop-blur-md">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-4">
          <div>
            <h3 className="text-base font-black text-white flex items-center gap-2">
              <lucide_react_1.BarChart3 className="w-5 h-5 text-cyan-400"/>
          ...
          <button onClick={runBacktest} disabled={loading} className="bg-gradient-to-r from-cyan-500 to-emerald-400 hover:from-cyan-400 hover:to-emerald-300 text-slate-950 font-black px-6 py-2.5 rounded-xl text-xs flex items-center gap-2 shadow-lg shadow-cyan-500/20 transition-all disabled:opacity-50">
            <lucide_react_1.Play className={`w-4 h-4 fill-current ${loading ? 'animate-spin' : ''}`}/>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 bg-slate-900/60 border border-slate-800 p-2.5 rounded-xl text-xs mt-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {[25000, 50000, 100000, 200000, 500000, 1000000].map((cap) => (<button key={cap} onClick={() => setInitialCapital(cap)} className={`px-2.5 py-1 rounded-lg font-bold text-[11px] transition-all ${initialCapital === cap
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mt-4 text-xs">
          ...
          <div>
            ...
            <select value={symbol} onChange={(e) => setSymbol(e.target.value)} className="w-full bg-slate-900 border border-slate-700 text-white rounded-lg p-2 font-bold focus:border-cyan-400 outline-none">
          ...
          <div>
            ...
            <select value={timeframe} onChange={(e) => setTimeframe(e.target.value)} className="w-full bg-slate-900 border border-slate-700 text-white rounded-lg p-2 font-bold focus:border-cyan-400 outline-none">
          ...
          <div>
            ...
            <input type="number" value={initialCapital} onChange={(e) => setInitialCapital(Number(e.target.value))} className="w-full bg-slate-900 border border-slate-700 text-white rounded-lg p-2 font-bold focus:border-cyan-400 outline-none"/>
          </div>
          ...
          <div>
            ...
            <input type="number" step="0.1" value={riskPercent} onChange={(e) => setRiskPercent(Number(e.target.value))} className="w-full bg-slate-900 border border-slate-700 text-white rounded-lg p-2 font-bold focus:border-cyan-400 outline-none"/>
          </div>
          ...
          <div>
            ...
            <select value={minScore} onChange={(e) => setMinScore(Number(e.target.value))} className="w-full bg-slate-900 border border-slate-700 text-white rounded-lg p-2 font-bold focus:border-cyan-400 outline-none">
      ...
      {error && (<div className="bg-rose-950/80 border border-rose-500/80 p-3 rounded-xl text-xs text-rose-300 flex items-center gap-2 font-mono">
          <lucide_react_1.XCircle className="w-4 h-4 text-rose-400 shrink-0"/>
          <span>{error}</span>
        </div>)}

      {/* Performance Summary Metrics Matrix */}
      {results && (<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {/* 1. Net PnL */}
          <div className={`p-4 rounded-xl border col-span-2 ${isProfitable
      ...
      {results && equityCurve.length > 0 && (<div className="bg-[#111827]/95 border border-slate-800 rounded-2xl p-5 shadow-2xl">
          <div className="flex items-center justify-between mb-4">
            <h4 className="text-xs font-black text-white uppercase flex items-center gap-1.5">
              <lucide_react_1.TrendingUp className="w-4 h-4 text-emerald-400"/>
            ...
            <svg className="w-full h-full" viewBox="0 0 500 100" preserveAspectRatio="none">
              ...
              {(() => {
                ...
                return (<>
                    ...
                    <polyline points={points} fill="none" stroke="#10B981" strokeWidth="2.5" strokeLinecap="round"/>
                  </>);
            })()}
            </svg>
          </div>
      ...
      {results && results.trades && (<div className="bg-[#111827]/95 border border-slate-800 rounded-2xl p-5 shadow-2xl">
          <div className="flex items-center justify-between mb-4 border-b border-slate-800 pb-3">
            <h4 className="text-xs font-black text-white uppercase flex items-center gap-1.5">
              <lucide_react_1.Award className="w-4 h-4 text-cyan-400"/>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left border-collapse min-w-[900px]">
              ...
              <tbody className="divide-y divide-slate-800/60 font-mono">
                ...
                return results.trades.map((tr, idx) => {
                    ...
                    return (<tr key={idx} className="hover:bg-slate-800/30 transition-colors">
                        ...
                        <td className="py-2 px-3">
                          <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${tr.direction === 'BULLISH'
                        ...
                        <td className={`py-2 px-3 text-right font-black ${isWin ? 'text-emerald-400' : 'text-rose-400'}`}>
                          {isWin ? '+' : ''}
                          {currSym}
...
exports.BacktestDashboard = BacktestDashboard;
//# sourceMappingURL=BacktestDashboard.js.map