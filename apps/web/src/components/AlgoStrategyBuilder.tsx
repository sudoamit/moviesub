...
export const AlgoStrategyBuilder: React.FC = () => {
  ...
  const [smcCondition, setSmcCondition] = useState<string>('ORDER_BLOCK');
  const [lots, setLots] = useState<number>(1);
  const [autoExecutePaper, setAutoExecutePaper] = useState<boolean>(true);
  const [notifyWebhook, setNotifyWebhook] = useState<boolean>(true);
    ...
    <div className="space-y-5 font-mono">
      ...
      <div className="bg-[#111827]/95 backdrop-blur-md border border-cyan-500/30 rounded-xl p-5 shadow-2xl space-y-4">
        ...
        <form onSubmit={handleCreateBot} className="bg-slate-900/90 border border-slate-800 p-4 rounded-xl space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
            <div>
              ...
              <input
                ...
                placeholder="e.g. NIFTY Premium Sweeper"
                className="w-full bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1.5 text-white placeholder-slate-600 focus:outline-none focus:border-cyan-500"
              />
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs pt-1">
            ...
            <div className="flex items-end">
              <button
                type="submit"
                disabled={isCreating}
                className="w-full bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-black py-2 rounded-lg text-xs transition-colors flex items-center justify-center gap-1.5 shadow-lg shadow-cyan-500/20 disabled:opacity-50"
              >
          ...
          {statusMsg && (
            <div className="p-2.5 rounded-lg bg-slate-950 border border-cyan-500/40 text-cyan-300 text-xs text-center">
              {statusMsg}
            </div>
          )}
        </form>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {bots.map((bot) => (
              ...
              >
                ...
                {bot.lastTriggerDetails && (
                  <div className="p-2 rounded bg-slate-950 border border-slate-800/80 text-[10px] text-slate-400">
