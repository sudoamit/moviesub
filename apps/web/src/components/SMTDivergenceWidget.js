...
exports.SMTDivergenceWidget = void 0;
const react_1 = __importStar(require("react"));
const lucide_react_1 = require("lucide-react");
const SMTDivergenceWidget = () => {
    const [assetA, setAssetA] = (0, react_1.useState)('NIFTY');
    const [assetB, setAssetB] = (0, react_1.useState)('BANKNIFTY');
    const [loading, setLoading] = (0, react_1.useState)(true);
    const [mtfData, setMtfData] = (0, react_1.useState)(null);
    const fetchSMT = async () => {
        try {
            setLoading(true);
            const res = await fetch(`http://localhost:3001/api/accuracy/smt-mtf?assetA=${assetA}&assetB=${assetB}`);
    ...
    return (<div className="bg-[#0B0F19] border border-slate-800 rounded-xl p-5 font-mono space-y-5 shadow-2xl">
      {/* Header & Quick Benchmark Presets */}
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800/80 pb-4">
        <div>
          <div className="flex items-center gap-2">
            <lucide_react_1.Zap className="w-5 h-5 text-amber-400 animate-pulse"/>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 bg-slate-900/90 border border-slate-800 p-1 rounded-lg">
            <button onClick={() => selectPreset('NIFTY', 'BANKNIFTY')} className={`px-2 py-1 rounded text-[11px] font-black transition-all ${assetA === 'NIFTY' && assetB === 'BANKNIFTY'
            ? 'bg-cyan-500 text-slate-950 shadow'
            : 'text-slate-400 hover:text-slate-200'}`}>
              NIFTY / BANKNIFTY
            </button>
            <button onClick={() => selectPreset('BTCUSDT', 'ETHUSDT')} className={`px-2 py-1 rounded text-[11px] font-black transition-all ${assetA === 'BTCUSDT' && assetB === 'ETHUSDT'
            ? 'bg-cyan-500 text-slate-950 shadow'
            : 'text-slate-400 hover:text-slate-200'}`}>
              BTC / ETH
            </button>
            <button onClick={() => selectPreset('XAUUSD', 'BTCUSDT')} className={`px-2 py-1 rounded text-[11px] font-black transition-all ${assetA === 'XAUUSD' && assetB === 'BTCUSDT'
            ? 'bg-amber-400 text-slate-950 shadow'
            : 'text-slate-400 hover:text-slate-200'}`}>
              🥇 GOLD / BTC
            </button>
            <button onClick={() => selectPreset('RELIANCE', 'HDFCBANK')} className={`px-2 py-1 rounded text-[11px] font-black transition-all ${assetA === 'RELIANCE' && assetB === 'HDFCBANK'
            ? 'bg-cyan-500 text-slate-950 shadow'
            : 'text-slate-400 hover:text-slate-200'}`}>
              RELIANCE / HDFC
            </button>
          ...
          <div className="flex items-center bg-slate-900 border border-slate-800 rounded-lg p-1 text-xs">
            <select value={assetA} onChange={(e) => setAssetA(e.target.value)} className="bg-transparent text-cyan-400 font-bold outline-none px-2 cursor-pointer">
            ...
            <lucide_react_1.ArrowRightLeft className="w-3.5 h-3.5 text-slate-500 mx-1"/>
            <select value={assetB} onChange={(e) => setAssetB(e.target.value)} className="bg-transparent text-indigo-400 font-bold outline-none px-2 cursor-pointer">
          ...
          <button onClick={fetchSMT} disabled={loading} className="p-2 bg-slate-900 hover:bg-slate-800 text-slate-300 rounded-lg border border-slate-800 transition-colors">
      ...
      {primary && (<div className={`p-5 rounded-xl border transition-all ${isBearish
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center border ${isBearish
                ? 'bg-rose-500/20 text-rose-400 border-rose-500/50'
                : isBullish
                    ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/50'
                    : 'bg-slate-800 text-slate-400 border-slate-700'}`}>
                {isBearish ? (<lucide_react_1.TrendingDown className="w-6 h-6"/>) : isBullish ? (<lucide_react_1.TrendingUp className="w-6 h-6"/>) : (<lucide_react_1.Layers className="w-6 h-6"/>)}
              </div>
              ...
              <div>
                ...
                <h3 className={`text-base sm:text-lg font-black tracking-tight mt-0.5 ${isBearish ? 'text-rose-400' : isBullish ? 'text-emerald-400' : 'text-slate-300'}`}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="bg-slate-900/90 border border-slate-800 px-3.5 py-1.5 rounded-lg text-right">
                ...
                <span className={`text-xs font-black px-2 py-0.5 rounded inline-block mt-0.5 ${primary.actionableSignal === 'STRONG_SELL'
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 pb-2.5">
          <div className="flex items-center gap-2">
            <lucide_react_1.Activity className="w-4 h-4 text-cyan-400"/>
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-200">
              Live Multi-Timeframe Divergence Heatmap Matrix
            </h3>
          </div>
          {mtfData && (<span className="text-[11px] font-bold text-slate-400 flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 pt-1">
          {mtfData?.timeframeResults?.map((tfItem) => {
            ...
            return (<div key={tfItem.timeframe} className={`p-3 rounded-xl border text-center transition-all ${isTFBearish
                ...
                <div className="flex items-center justify-between text-[10px] mb-1 font-bold">
                  ...
                  {isTFActive ? (<span className="px-1 py-0.5 rounded text-[8px] bg-cyan-950 text-cyan-300 border border-cyan-700">
                      {tfItem.convictionScore}%
                    </span>) : (<span className="text-slate-600 text-[8px]">SYNC</span>)}
                </div>

                <div className="mt-1">
                  <span className={`text-xs font-black block tracking-tight ${isTFBearish
                    ? 'text-rose-400'
                    : isTFBullish
                        ? 'text-emerald-400'
                        : 'text-slate-500'}`}>
                ...
                <div className="mt-2 pt-1.5 border-t border-slate-800/80 text-[8px] text-slate-400 flex items-center justify-center gap-1">
                  {isTFBearish ? (<span className="text-rose-400 font-bold">⚠️ Liquidity Sweep</span>) : isTFBullish ? (<span className="text-emerald-400 font-bold">⚡ Accumulation</span>) : (<span className="text-slate-500">In Equilibrium</span>)}
                </div>
              </div>);
        })}
        </div>
      ...
      {primary && hasDivergence && (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
...
exports.SMTDivergenceWidget = SMTDivergenceWidget;
//# sourceMappingURL=SMTDivergenceWidget.js.map