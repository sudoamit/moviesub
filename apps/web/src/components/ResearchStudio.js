...
exports.ResearchStudio = void 0;
const react_1 = __importStar(require("react"));
const lucide_react_1 = require("lucide-react");
const ResearchStudio = () => {
    const [activeSubTab, setActiveSubTab] = (0, react_1.useState)('scorecard');
    const [scorecardData, setScorecardData] = (0, react_1.useState)(null);
    const [experiments, setExperiments] = (0, react_1.useState)([]);
    const [hypotheses, setHypotheses] = (0, react_1.useState)([]);
    const [ablationData, setAblationData] = (0, react_1.useState)(null);
    const [counterfactuals, setCounterfactuals] = (0, react_1.useState)(null);
    const [knowledgeGraph, setKnowledgeGraph] = (0, react_1.useState)(null);
    const [isLoading, setIsLoading] = (0, react_1.useState)(true);
    const [isRunningExperiment, setIsRunningExperiment] = (0, react_1.useState)(false);
    const [selectedSymbol, setSelectedSymbol] = (0, react_1.useState)('NIFTY');
    const fetchResearchData = (0, react_1.useCallback)(async () => {
        setIsLoading(true);
        try {
            ...
            if (hypRes && Array.isArray(hypRes))
                setHypotheses(hypRes);
            if (cfRes)
                setCounterfactuals(cfRes);
            if (kgRes)
    ...
    const handleTriggerExperiment = async (hypothesisId) => {
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-[#0B111E]/90 border border-slate-800/80 rounded-2xl p-6 backdrop-blur-md shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <div className="p-2.5 rounded-xl bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 shadow-inner">
                <lucide_react_1.FlaskConical className="w-6 h-6 animate-pulse"/>
              </div>
              <div>
          ...
          <div className="flex items-center gap-3">
            <select value={selectedSymbol} onChange={(e) => setSelectedSymbol(e.target.value)} className="bg-slate-900 border border-slate-700 text-slate-200 text-xs rounded-xl px-3 py-2 font-mono focus:outline-none focus:border-cyan-500">
            ...
            <button onClick={() => handleTriggerExperiment()} disabled={isRunningExperiment} className="px-4 py-2 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white text-xs font-bold font-mono rounded-xl shadow-lg shadow-cyan-500/20 flex items-center gap-2 transition-all disabled:opacity-50">
              <lucide_react_1.Play className={`w-3.5 h-3.5 ${isRunningExperiment ? 'animate-spin' : ''}`}/>
              {isRunningExperiment ? 'RUNNING RESEARCH OOS...' : 'EXECUTE NEW EXPERIMENT'}
            </button>

            <button onClick={fetchResearchData} className="p-2 bg-slate-900 border border-slate-700 text-slate-400 hover:text-white rounded-xl transition-all" title="Refresh Research State">
              <lucide_react_1.RefreshCw className={`w-4 h-4 ${isLoading ? 'animate-spin' : ''}`}/>
            </button>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-800 pb-3 overflow-x-auto font-mono text-xs">
        <button onClick={() => setActiveSubTab('scorecard')} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all ${activeSubTab === 'scorecard'
        ...
        <button onClick={() => setActiveSubTab('experiments')} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all ${activeSubTab === 'experiments'
            ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 font-bold shadow-lg shadow-cyan-500/10'
            : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900/50'}`}>
          <lucide_react_1.FlaskConical className="w-4 h-4"/>
          Experiment Registry ({experiments.length})
        </button>

        <button onClick={() => setActiveSubTab('hypotheses')} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all ${activeSubTab === 'hypotheses'
            ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 font-bold shadow-lg shadow-cyan-500/10'
            : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900/50'}`}>
        ...
        <button onClick={() => {
            ...
            if (!ablationData)
                handleRunAblation();
        }} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all ${activeSubTab === 'ablation'
            ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 font-bold shadow-lg shadow-cyan-500/10'
            : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900/50'}`}>
        ...
        <button onClick={() => setActiveSubTab('counterfactuals')} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all ${activeSubTab === 'counterfactuals'
            ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 font-bold shadow-lg shadow-cyan-500/10'
            : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900/50'}`}>
          <lucide_react_1.Scale className="w-4 h-4"/>
          Counterfactual Exit Inefficiencies
        </button>

        <button onClick={() => setActiveSubTab('knowledgeGraph')} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all ${activeSubTab === 'knowledgeGraph'
            ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 font-bold shadow-lg shadow-cyan-500/10'
            : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900/50'}`}>
      ...
      {activeSubTab === 'scorecard' && (<div className="space-y-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {(lc?.versions || []).map((v, idx) => {
                const isLatest = idx === lc.versions.length - 1;
                return (<tr key={v.version} className={isLatest ? 'bg-cyan-500/5 font-bold' : ''}>
                        <td className="py-3 px-3 text-white flex items-center gap-2">
                          <lucide_react_1.GitBranch className={`w-3.5 h-3.5 ${isLatest ? 'text-cyan-400' : 'text-slate-500'}`}/>
                          {v.version}
                        </td>
                        ...
                        <td className="py-3 px-3 text-center">
                          <span className={`px-2 py-0.5 rounded text-[10px] ${isLatest
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-slate-800/80">
              <div className="bg-slate-900/40 p-3 rounded-xl border border-slate-800/80">
                ...
                <span className="text-sm font-bold text-emerald-400 flex items-center gap-1 mt-1">
                  <lucide_react_1.ArrowUpRight className="w-4 h-4"/>+{lc?.expectancyTrend?.deltaR || 0.16}R
                  Improvement
                </span>
              </div>
              ...
              <div className="bg-slate-900/40 p-3 rounded-xl border border-slate-800/80">
                ...
                <span className="text-sm font-bold text-indigo-400 flex items-center gap-1 mt-1">
                  <lucide_react_1.ArrowUpRight className="w-4 h-4"/>+{lc?.profitFactorTrend?.deltaPF || 0.37} PF
                  Boost
                </span>
              </div>
      ...
      {activeSubTab === 'experiments' && (<div className="bg-[#0B111E] border border-slate-800 rounded-2xl p-6 font-mono space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <lucide_react_1.FlaskConical className="w-4 h-4 text-cyan-400"/>
            ...
            <table className="w-full text-xs text-left">
              ...
              <tbody className="divide-y divide-slate-800/60">
                ...
                return (<tr key={exp.id} className="hover:bg-slate-900/40 transition-colors">
                        ...
                        <td className="py-3 px-3 text-slate-300 max-w-xs truncate" title={exp.hypothesis}>
                          {exp.hypothesis}
                        </td>
                        ...
                        <td className="py-3 px-3 text-right text-amber-300">
                          {((exp.robustnessMetrics?.monteCarloRuinProbability || 0) * 100).toFixed(1)}
                          %
                        </td>
                        <td className="py-3 px-3 text-right text-teal-300">
                        ...
                        <td className="py-3 px-3 text-center">
                          {exp.passedOutOfSample ? (<span className="px-2 py-0.5 rounded text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-bold">
                              PASS
                            </span>) : (<span className="px-2 py-0.5 rounded text-[10px] bg-rose-500/10 text-rose-400 border border-rose-500/20 font-bold">
                              FAIL
                            </span>)}
                        </td>
                        <td className="py-3 px-3 text-center">
                          <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${isPassed
      ...
      {activeSubTab === 'hypotheses' && (<div className="bg-[#0B111E] border border-slate-800 rounded-2xl p-6 font-mono space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {hypotheses.map((hyp) => (<div key={hyp.id} className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${hyp.status === 'PROMOTED'
                    ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30'
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-800/60 text-[10px]">
                  ...
                  <button onClick={() => handleTriggerExperiment(hyp.id)} disabled={isRunningExperiment} className="px-2.5 py-1 rounded bg-slate-800 hover:bg-slate-700 text-cyan-300 font-bold border border-slate-700 transition-colors">
                    Test Hypothesis
                  </button>
                </div>
              </div>))}
      ...
      {activeSubTab === 'ablation' && (<div className="bg-[#0B111E] border border-slate-800 rounded-2xl p-6 font-mono space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
                <lucide_react_1.Layers className="w-4 h-4 text-cyan-400"/>
            ...
            <button onClick={handleRunAblation} className="px-3 py-1.5 bg-slate-900 border border-slate-700 hover:border-cyan-500 text-cyan-300 text-xs font-bold rounded-xl transition-all">
              Re-run Ablation
            </button>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {(ablationData?.results || []).map((row) => {
                const isBase = row.variant === 'FULL_SYSTEM_BASELINE';
                return (<tr key={row.variant} className={isBase ? 'bg-cyan-500/10 font-bold' : 'hover:bg-slate-900/30'}>
                      <td className="py-3 px-3 text-white">
                        {row.variant}
                        {isBase && (<span className="ml-2 text-[10px] text-cyan-400">(FULL SUITE)</span>)}
                      </td>
                      ...
                      <td className={`py-3 px-3 text-right font-bold ${row.deltaRFromBaseline < 0 ? 'text-rose-400' : 'text-slate-400'}`}>
          ...
          {ablationData && (<div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-slate-800/80 text-xs">
      ...
      {activeSubTab === 'counterfactuals' && (<div className="bg-[#0B111E] border border-slate-800 rounded-2xl p-6 font-mono space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
      ...
      {activeSubTab === 'knowledgeGraph' && (<div className="bg-[#0B111E] border border-slate-800 rounded-2xl p-6 font-mono space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <lucide_react_1.Network className="w-4 h-4 text-cyan-400"/>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 space-y-3">
              <h4 className="text-xs font-bold text-cyan-400 flex items-center gap-2">
                <lucide_react_1.Activity className="w-3.5 h-3.5"/> Market Regimes
              </h4>
              <div className="space-y-2">
                {(knowledgeGraph?.nodes || [])
                .filter((n) => n.type === 'REGIME')
                .map((n) => (<div key={n.id} className="p-2 rounded bg-black/40 border border-slate-800 flex items-center justify-between text-xs">
            ...
            <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 space-y-3">
              <h4 className="text-xs font-bold text-indigo-400 flex items-center gap-2">
                <lucide_react_1.Layers className="w-3.5 h-3.5"/> Setups & Entry Quality
              </h4>
              <div className="space-y-2">
                {(knowledgeGraph?.nodes || [])
                .filter((n) => n.type === 'SETUP' || n.type === 'ENTRY_QUALITY')
                .map((n) => (<div key={n.id} className="p-2 rounded bg-black/40 border border-slate-800 flex items-center justify-between text-xs">
            ...
            <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 space-y-3">
              <h4 className="text-xs font-bold text-teal-400 flex items-center gap-2">
                <lucide_react_1.ShieldCheck className="w-3.5 h-3.5"/> Empirical Outcomes
              </h4>
              <div className="space-y-2">
                {(knowledgeGraph?.nodes || [])
...
exports.ResearchStudio = ResearchStudio;
//# sourceMappingURL=ResearchStudio.js.map