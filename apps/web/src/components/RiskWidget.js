...
exports.RiskWidget = void 0;
const react_1 = __importStar(require("react"));
const lucide_react_1 = require("lucide-react");
const RiskWidget = ({ selectedSignal }) => {
    ...
    const [accountBalance, setAccountBalance] = (0, react_1.useState)(1000000);
    const [riskPercent, setRiskPercent] = (0, react_1.useState)(1.0);
    const [leverage, setLeverage] = (0, react_1.useState)(5);
    const [tradeMode, setTradeMode] = (0, react_1.useState)('FO');
    (0, react_1.useEffect)(() => {
        if (typeof window !== 'undefined') {
            const savedBal = localStorage.getItem('quant_account_balance');
            if (savedBal && !isNaN(Number(savedBal)))
                setAccountBalance(Number(savedBal));
            const savedRisk = localStorage.getItem('quant_risk_percent');
            if (savedRisk && !isNaN(Number(savedRisk)))
                setRiskPercent(Number(savedRisk));
            const savedLev = localStorage.getItem('quant_risk_leverage');
            if (savedLev && !isNaN(Number(savedLev)))
                setLeverage(Number(savedLev));
            const savedMode = localStorage.getItem('quant_sizing_mode');
            if (savedMode === 'FO' || savedMode === 'CASH')
                setTradeMode(savedMode);
        }
    }, []);
    const [entryPrice, setEntryPrice] = (0, react_1.useState)(24175.65);
    const [stopLoss, setStopLoss] = (0, react_1.useState)(24086.6);
    const [tp1Price, setTp1Price] = (0, react_1.useState)(24302.62);
    const [tp2Price, setTp2Price] = (0, react_1.useState)(24387.27);
    const [lotSize, setLotSize] = (0, react_1.useState)(65);
    const [savedNotification, setSavedNotification] = (0, react_1.useState)(null);
    // Save leverage changes immediately to localStorage
    const handleLeverageChange = (newLev) => {
    ...
    const handleRiskPercentChange = (newRisk) => {
    ...
    return (<div className="bg-[#0B0F19] border border-slate-800 rounded-xl p-4 sm:p-6 shadow-2xl font-mono space-y-5 relative overflow-hidden">
      {/* Background Glow */}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800/80 pb-4">
        ...
        <div className="flex items-center gap-2">
          {savedNotification && (<span className="text-[11px] text-emerald-400 bg-emerald-950/60 border border-emerald-500/40 px-2.5 py-1 rounded-lg font-bold flex items-center gap-1 animate-in fade-in">
              <lucide_react_1.Check className="w-3 h-3"/> {savedNotification}
            </span>)}
          ...
          <span className="text-xs text-emerald-400 flex items-center gap-1.5 bg-emerald-950/40 border border-emerald-800/40 px-3 py-1 rounded-lg font-bold">
            <lucide_react_1.ShieldCheck className="w-4 h-4"/> {riskPercent}% Capital Risk
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 bg-slate-900/60 border border-slate-800 p-2.5 rounded-xl text-xs">
        <span className="text-[11px] text-slate-400 font-bold flex items-center gap-1">
          <lucide_react_1.Coins className="w-3.5 h-3.5 text-cyan-400"/>
          ACCOUNT CAPITAL PRESETS (₹ INR):
        </span>
        <div className="flex flex-wrap gap-1.5">
          {currentPresets.map((bal) => (<button key={bal} onClick={() => handleBalanceChange(bal)} className={`px-2.5 py-1 rounded-lg font-bold text-[11px] transition-all ${accountBalance === bal
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 text-xs">
            ...
            <div className="bg-slate-900/90 border border-slate-800 p-3 rounded-xl space-y-1.5">
              ...
              <div className="relative">
                <input type="number" value={accountBalance} onChange={(e) => handleBalanceChange(Number(e.target.value))} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white font-black text-sm focus:border-cyan-500 focus:outline-none"/>
              </div>
            </div>
            ...
            <div className="bg-slate-900/90 border border-slate-800 p-3 rounded-xl space-y-1.5">
              ...
              <div className="flex gap-1">
                {[0.5, 1.0, 1.5, 2.0].map((pct) => (<button key={pct} onClick={() => handleRiskPercentChange(pct)} className={`flex-1 py-1.5 rounded-lg text-[11px] font-bold transition-all ${riskPercent === pct
            ...
            <div className="bg-slate-900/90 border border-slate-800 p-3 rounded-xl space-y-1.5">
              ...
              <input type="number" step="any" value={entryPrice} onChange={(e) => setEntryPrice(Number(e.target.value))} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-cyan-400 font-black text-sm focus:border-cyan-500 focus:outline-none"/>
            </div>
            ...
            <div className="bg-slate-900/90 border border-slate-800 p-3 rounded-xl space-y-1.5">
              ...
              <input type="number" step="any" value={stopLoss} onChange={(e) => setStopLoss(Number(e.target.value))} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-rose-400 font-black text-sm focus:border-rose-500 focus:outline-none"/>
            </div>
            ...
            <div className="bg-slate-900/90 border border-slate-800 p-3 rounded-xl space-y-1.5">
              ...
              <div className="flex gap-1">
                <button disabled={isCrypto} onClick={() => handleTradeModeChange('FO')} className={`flex-1 py-1.5 rounded-lg text-[11px] font-bold transition-all ${tradeMode === 'FO' && !isCrypto
            ? 'bg-cyan-500 text-slate-950 shadow-sm'
            : 'bg-slate-950 border border-slate-700 text-slate-400 hover:text-white'}`}>
                  F&O Lots ({lotSize})
                </button>
                <button onClick={() => handleTradeModeChange('CASH')} className={`flex-1 py-1.5 rounded-lg text-[11px] font-bold transition-all ${tradeMode === 'CASH' || isCrypto
            ? 'bg-cyan-500 text-slate-950 shadow-sm'
            : 'bg-slate-950 border border-slate-700 text-slate-400 hover:text-white'}`}>
                  Cash / Units
                </button>
            ...
            <div className="bg-slate-900/90 border border-cyan-500/30 p-3 rounded-xl space-y-1.5">
              <div className="flex justify-between items-center">
                <label className="text-[10px] uppercase text-cyan-400 font-bold flex items-center gap-1">
                  <lucide_react_1.Zap className="w-3 h-3 text-cyan-400"/>
              ...
              <div className="flex gap-1">
                {leveragePresets.map((lev) => (<button key={lev} onClick={() => handleLeverageChange(lev)} className={`flex-1 py-1.5 rounded-lg text-[11px] font-bold transition-all ${leverage === lev
                ? 'bg-cyan-500 text-slate-950 shadow-sm'
                : 'bg-slate-950 border border-slate-700 text-slate-300 hover:bg-slate-800'}`}>
                    {lev}x
                  </button>))}
        ...
        <div className="lg:col-span-5 flex flex-col justify-between space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="bg-slate-900/90 border border-slate-800 p-3 rounded-xl">
              <span className="text-[10px] text-cyan-400 font-bold block uppercase flex items-center gap-1">
                <lucide_react_1.Target className="w-3 h-3"/> Target 1 (1.5R)
              </span>
            ...
            <div className="bg-slate-900/90 border border-slate-800 p-3 rounded-xl">
              <span className="text-[10px] text-emerald-400 font-bold block uppercase flex items-center gap-1">
                <lucide_react_1.TrendingUp className="w-3 h-3"/> Target 2 (2.5R)
              </span>
          ...
          <div className="bg-slate-900/90 border border-slate-800 p-3 rounded-xl text-xs space-y-1.5">
            ...
            <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden flex mt-2">
              <div className={`h-full transition-all ${isMarginWarning
            ? 'bg-rose-500'
            : marginUsagePercent > 70
                ? 'bg-amber-500'
                : 'bg-cyan-500'}`} style={{ width: `${Math.min(100, marginUsagePercent)}%` }}/>
            </div>
      ...
      <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-3.5 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
        <div className="flex items-center gap-2">
          {actualRiskPercent <= 2.0 ? (<lucide_react_1.CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0"/>) : (<lucide_react_1.ShieldAlert className="w-4 h-4 text-rose-400 shrink-0"/>)}
          <div>
        ...
        <div className="flex items-center gap-2">
          {rewardToRiskRatio >= 1.5 ? (<lucide_react_1.CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0"/>) : (<lucide_react_1.AlertTriangle className="w-4 h-4 text-amber-400 shrink-0"/>)}
          <div>
        ...
        <div className="flex items-center gap-2">
          {!isMarginWarning ? (<lucide_react_1.CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0"/>) : (<lucide_react_1.ShieldAlert className="w-4 h-4 text-rose-400 shrink-0"/>)}
          <div>
        ...
        <div className="flex items-center gap-2">
          <lucide_react_1.CheckCircle2 className="w-4 h-4 text-cyan-400 shrink-0"/>
          <div>
...
exports.RiskWidget = RiskWidget;
//# sourceMappingURL=RiskWidget.js.map