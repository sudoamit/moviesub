...
exports.PaperTradingWidget = void 0;
const react_1 = __importStar(require("react"));
const lucide_react_1 = require("lucide-react");
const PaperTradingWidget = ({ currentSymbol, activeSignal, livePrice, }) => {
    ...
    const [activeTab, setActiveTab] = (0, react_1.useState)('positions');
    const [isLoading, setIsLoading] = (0, react_1.useState)(true);
    const [isSubmitting, setIsSubmitting] = (0, react_1.useState)(false);
    const [lots, setLots] = (0, react_1.useState)(1);
    const [customQty, setCustomQty] = (0, react_1.useState)(0);
    const [orderSide, setOrderSide] = (0, react_1.useState)(activeSignal?.direction === 'BEARISH' ? 'SELL' : 'BUY');
    const [leverage, setLeverage] = (0, react_1.useState)(5);
    ...
    const handlePlaceOrder = async (overrideSide) => {
        ...
        try {
            ...
            if (typeof window !== 'undefined') {
                ...
                window.dispatchEvent(new CustomEvent('quant_trade_opened', { detail: { symbol: currentSymbol } }));
            }
            fetchPortfolio();
        }
        catch (err) {
    ...
    const handleClosePosition = async (posId) => {
        try {
            ...
            if (typeof window !== 'undefined') {
                ...
                window.dispatchEvent(new CustomEvent('quant_trade_closed', { detail: { symbol: currentSymbol } }));
            }
            fetchPortfolio();
        }
        catch (e) {
    ...
    const handleResetPortfolio = async () => {
        if (!confirm(`Are you sure you want to reset your virtual paper balance to ${isCrypto ? '$10,000' : '₹10,00,000'}?`))
            return;
        try {
    ...
    return (<div className="bg-[#0B0F19] border border-cyan-500/30 rounded-xl p-4 sm:p-6 shadow-2xl space-y-5 font-mono relative overflow-hidden">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={fetchPortfolio} className="p-1.5 rounded-lg bg-slate-900 border border-slate-800 hover:bg-slate-800 text-slate-400 hover:text-white transition-all text-xs flex items-center gap-1">
            <lucide_react_1.RefreshCw className="w-3.5 h-3.5"/>
          </button>
          <button onClick={handleResetPortfolio} className="text-xs text-slate-400 hover:text-cyan-300 flex items-center gap-1 bg-slate-900 border border-slate-800 hover:bg-slate-800 px-3 py-1.5 rounded-lg transition-all font-bold">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-slate-900/90 border border-slate-800 p-3.5 rounded-xl">
          ...
          <span className={`text-lg sm:text-xl font-black block mt-0.5 ${(portfolio?.realizedPnL || 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
        ...
        <div className="bg-slate-900/90 border border-slate-800 p-3.5 rounded-xl">
          ...
          <span className={`text-lg sm:text-xl font-black block mt-0.5 ${(portfolio?.unrealizedPnL || 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
      ...
      <div className="bg-slate-900/90 border border-slate-800 p-4 rounded-xl space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              {[1, 2, 5, 10].map((l) => (<button key={l} onClick={() => {
                setLots(l);
                setCustomQty(0);
            }} className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${lots === l && customQty === 0
          ...
          <div className="flex items-center gap-2 text-xs">
            <span className="text-slate-400 font-bold flex items-center gap-1">
              <lucide_react_1.Zap className="w-3 h-3 text-cyan-400"/> Leverage:
            </span>
            <div className="flex gap-1">
              {[1, 2, 5, 10, 20].map((lev) => (<button key={lev} onClick={() => setLeverage(lev)} className={`px-2 py-1 rounded-md text-[11px] font-bold border transition-all ${leverage === lev
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button onClick={() => handlePlaceOrder('BUY')} disabled={isSubmitting} className="bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white font-black py-3 rounded-xl flex items-center justify-center gap-2 text-sm transition-all shadow-lg shadow-emerald-950/50 active:scale-[0.99]">
            <lucide_react_1.TrendingUp className="w-4 h-4"/>
            1-CLICK BUY MARKET ({totalQuantity} {currentSymbol} @ {currencySymbol}
            {cmp.toFixed(2)})
          </button>

          <button onClick={() => handlePlaceOrder('SELL')} disabled={isSubmitting} className="bg-rose-600 hover:bg-rose-500 disabled:opacity-50 text-white font-black py-3 rounded-xl flex items-center justify-center gap-2 text-sm transition-all shadow-lg shadow-rose-950/50 active:scale-[0.99]">
        ...
        {statusMessage && (<div className="p-3 rounded-xl bg-slate-950 border border-cyan-500/40 text-cyan-300 text-xs text-center font-bold animate-in fade-in">
            {statusMessage}
          </div>)}
      </div>
      ...
      <div className="space-y-3">
        <div className="flex border-b border-slate-800 gap-4 text-xs font-bold">
          <button onClick={() => setActiveTab('positions')} className={`pb-2.5 flex items-center gap-1.5 transition-all ${activeTab === 'positions'
          ...
          <button onClick={() => setActiveTab('history')} className={`pb-2.5 flex items-center gap-1.5 transition-all ${activeTab === 'history'
            ? 'border-b-2 border-cyan-400 text-cyan-400'
            : 'text-slate-400 hover:text-slate-200'}`}>
          ...
          <button onClick={() => setActiveTab('analytics')} className={`pb-2.5 flex items-center gap-1.5 transition-all ${activeTab === 'analytics'
            ? 'border-b-2 border-cyan-400 text-cyan-400'
            : 'text-slate-400 hover:text-slate-200'}`}>
            <lucide_react_1.BarChart2 className="w-4 h-4"/>
            PERFORMANCE METRICS
          </button>
        </div>

        {/* TAB 1: ACTIVE POSITIONS */}
        {activeTab === 'positions' && (<div>
            {portfolio?.openPositions?.length === 0 ? (<div className="p-8 rounded-xl bg-slate-900/60 border border-slate-800 text-center text-xs text-slate-500 space-y-1">
              ...
              </div>) : (<div className="overflow-x-auto rounded-xl border border-slate-800">
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-800/60 bg-slate-950/60">
                    ...
                    return (<tr key={pos.id} className="hover:bg-slate-900/50 transition-colors">
                          ...
                          <td className="p-3">
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${pos.direction === 'BUY'
                          ...
                          <td className="p-3 text-right">
                            <button onClick={() => handleClosePosition(pos.id)} className="bg-rose-500/20 hover:bg-rose-500 text-rose-300 hover:text-white border border-rose-500/40 px-3 py-1 rounded-lg text-xs font-bold transition-all shadow-sm">
                              Close @ Market
                            </button>
                          </td>
                        </tr>);
        ...
        {activeTab === 'history' && (<div>
            {portfolio?.tradeHistory?.length === 0 ? (<div className="p-8 rounded-xl bg-slate-900/60 border border-slate-800 text-center text-xs text-slate-500">
                ...
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-800/60 bg-slate-950/60">
                    ...
                    const isCryptoTrade = trade.symbol === 'BTCUSDT' || trade.symbol?.includes('BTC');
                    const tradeCurr = isCryptoTrade ? '$' : '₹';
                    return (<tr key={trade.id} className="hover:bg-slate-900/50 transition-colors">
                          <td className="p-3 font-black text-white">
                            <span className="text-cyan-300">
                          ...
                          <td className="p-3">
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${trade.direction === 'BUY'
                            ? 'bg-emerald-950 text-emerald-400 border border-emerald-800'
                            : 'bg-rose-950 text-rose-400 border border-rose-800'}`}>
                              {trade.direction}
                            </span>
        ...
        {activeTab === 'analytics' && (<div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-slate-900/90 border border-slate-800 p-3.5 rounded-xl">
...
exports.PaperTradingWidget = PaperTradingWidget;
//# sourceMappingURL=PaperTradingWidget.js.map