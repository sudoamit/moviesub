...
const TradingChart = ({ symbol, timeframe, candles, signal, livePrice, liveChangePercent = 0, isTradeActive = true, onTimeframeChange, onSymbolChange, }) => {
    const chartContainerRef = (0, react_1.useRef)(null);
    const overlayCanvasRef = (0, react_1.useRef)(null);
    const chartApiRef = (0, react_1.useRef)(null);
    const mainSeriesRef = (0, react_1.useRef)(null);
    const volumeSeriesRef = (0, react_1.useRef)(null);
    // Indicator Series Refs
    const ema20SeriesRef = (0, react_1.useRef)(null);
    const ema50SeriesRef = (0, react_1.useRef)(null);
    const ema200SeriesRef = (0, react_1.useRef)(null);
    const vwapSeriesRef = (0, react_1.useRef)(null);
    const sma20SeriesRef = (0, react_1.useRef)(null);
    const priceLinesRef = (0, react_1.useRef)([]);
    // Chart UI States
    const [chartType, setChartType] = (0, react_1.useState)('Candles');
    ...
    const [showEMA20, setShowEMA20] = (0, react_1.useState)(true);
    const [showEMA50, setShowEMA50] = (0, react_1.useState)(true);
    const [showEMA200, setShowEMA200] = (0, react_1.useState)(false);
    const [showVWAP, setShowVWAP] = (0, react_1.useState)(true);
    const [showSMA20, setShowSMA20] = (0, react_1.useState)(false);
    const [showVolume, setShowVolume] = (0, react_1.useState)(true);
    const [showVolumeProfile, setShowVolumeProfile] = (0, react_1.useState)(true);
    const [showKillZones, setShowKillZones] = (0, react_1.useState)(true);
    const [showLiquidityHeatmap, setShowLiquidityHeatmap] = (0, react_1.useState)(false);
    const [showIndicatorsModal, setShowIndicatorsModal] = (0, react_1.useState)(false);
    // SMC Overlays Active Toggles
    const [showOB, setShowOB] = (0, react_1.useState)(true);
    const [showFVG, setShowFVG] = (0, react_1.useState)(true);
    const [showBOS, setShowBOS] = (0, react_1.useState)(true);
    const [showCHoCH, setShowCHoCH] = (0, react_1.useState)(true);
    const [showLiq, setShowLiq] = (0, react_1.useState)(true);
    const [showLevels, setShowLevels] = (0, react_1.useState)(true);
    const [showRange, setShowRange] = (0, react_1.useState)(true);
    const [showSMCDash, setShowSMCDash] = (0, react_1.useState)(true);
    const [showSMCModal, setShowSMCModal] = (0, react_1.useState)(false);
    // Drawing Tools State
    const [activeDrawingTool, setActiveDrawingTool] = (0, react_1.useState)(null);
    const [drawings, setDrawings] = (0, react_1.useState)([]);
    // OHLC Crosshair Display
    const [ohlcData, setOhlcData] = (0, react_1.useState)(null);
    // Live Paper Trading Position Sync
    const [paperPosition, setPaperPosition] = (0, react_1.useState)(null);
    const [isLocallyCut, setIsLocallyCut] = (0, react_1.useState)(false);
    (0, react_1.useEffect)(() => {
    ...
    const [aiPredictionInfo, setAiPredictionInfo] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
        let isMounted = true;
        ...
        const fetchActivePos = async () => {
            try {
                ...
                if (isMounted && data && Array.isArray(data.openPositions)) {
                    const found = data.openPositions.find((p) => p.symbol === symbol);
    ...
    const clientSMC = (0, react_1.useMemo)(() => {
        ...
        try {
            ...
            return trading_engine_1.SMCAnalyzer.analyze(cleanCandles);
        }
        catch (e) {
    ...
    const clientVP = (0, react_1.useMemo)(() => {
        if (!candles || candles.length < 5)
            return null;
        try {
            const cleanCandles = candles.map((c) => ({
            ...
            return trading_engine_1.VolumeProfileAnalyzer.compute(cleanCandles, 28, 0.7);
        }
        catch (e) {
    ...
    const clientHeatmap = (0, react_1.useMemo)(() => {
        ...
        try {
            ...
            return trading_engine_1.LiquidityHeatmapEngine.compute(cleanCandles);
        }
        catch (e) {
            return null;
        }
    ...
    const clientMTF = (0, react_1.useMemo)(() => {
        if (!candles || candles.length < 5)
            return null;
        try {
            const cleanCandles = candles.map((c) => ({
            ...
            return trading_engine_1.MTFFlowRadarEngine.analyze(symbol, [], [], cleanCandles, []);
        }
        catch (e) {
            return null;
        }
    ...
    (0, react_1.useEffect)(() => {
        ...
        const chart = (0, lightweight_charts_1.createChart)(chartContainerRef.current, {
            width: initialWidth,
            height: initialHeight,
            layout: {
                background: { type: lightweight_charts_1.ColorType.Solid, color: bg },
                textColor: textColor,
            ...
            crosshair: {
                ...
                vertLine: {
                    color: '#758696',
                    width: 1,
                    style: lightweight_charts_1.LineStyle.Dotted,
        ...
        if (chartType === 'Candles') {
            mainSeries = chart.addSeries(lightweight_charts_1.CandlestickSeries, {
                upColor: '#089981',
                downColor: '#F23645',
                borderVisible: true,
            ...
            mainSeries = chart.addSeries(lightweight_charts_1.BarSeries, {
                upColor: '#089981',
                downColor: '#F23645',
            });
        }
        else if (chartType === 'Area') {
            mainSeries = chart.addSeries(lightweight_charts_1.AreaSeries, {
            ...
            mainSeries = chart.addSeries(lightweight_charts_1.LineSeries, {
                color: '#089981',
                lineWidth: 2,
            });
        }
        ...
        const volumeSeries = chart.addSeries(lightweight_charts_1.HistogramSeries, {
        ...
        ema20SeriesRef.current = chart.addSeries(lightweight_charts_1.LineSeries, {
            color: '#FBBF24',
            lineWidth: 2,
            title: 'EMA 20',
            priceScaleId: 'right',
        });
        ema50SeriesRef.current = chart.addSeries(lightweight_charts_1.LineSeries, {
            color: '#06B6D4',
            lineWidth: 2,
            title: 'EMA 50',
            priceScaleId: 'right',
        });
        ema200SeriesRef.current = chart.addSeries(lightweight_charts_1.LineSeries, {
            color: '#8B5CF6',
            lineWidth: 2,
            title: 'EMA 200',
            priceScaleId: 'right',
        });
        vwapSeriesRef.current = chart.addSeries(lightweight_charts_1.LineSeries, {
            color: '#F97316',
            lineWidth: 2,
            title: 'VWAP',
            priceScaleId: 'right',
        });
        sma20SeriesRef.current = chart.addSeries(lightweight_charts_1.LineSeries, {
            color: '#3B82F6',
            lineWidth: 2,
            title: 'SMA 20',
            priceScaleId: 'right',
    ...
    (0, react_1.useEffect)(() => {
        if (!mainSeriesRef.current || !candles || candles.length === 0)
            return;
        ...
        if (sorted.length > 5) {
            ...
            if (ema20SeriesRef.current)
                ema20SeriesRef.current.setData(showEMA20 ? calcEMA(20) : []);
            if (ema50SeriesRef.current)
                ema50SeriesRef.current.setData(showEMA50 ? calcEMA(50) : []);
            if (ema200SeriesRef.current)
                ema200SeriesRef.current.setData(showEMA200 ? calcEMA(200) : []);
            if (vwapSeriesRef.current)
                vwapSeriesRef.current.setData(showVWAP ? calcVWAP() : []);
            if (sma20SeriesRef.current)
                sma20SeriesRef.current.setData(showSMA20 ? calcSMA(20) : []);
        }
    ...
    const prevSymbolTfRef = (0, react_1.useRef)('');
    (0, react_1.useEffect)(() => {
    ...
    (0, react_1.useEffect)(() => {
        if (!mainSeriesRef.current || !candles || candles.length === 0 || !currentPrice)
            return;
        const lastCandle = candles[candles.length - 1];
        const timeSec = Math.floor(new Date(lastCandle.timestamp).getTime() / 1000);
        if (chartType === 'Candles' || chartType === 'Bar') {
    ...
    const isActualTradeActive = isTradeActive !== false &&
        !isLocallyCut &&
        !!effSignal &&
    ...
    const entryPrice = Number((effSignal?.entryZone?.optimal || effSignal?.entryZone?.max || currentPrice || 0).toFixed(2));
    ...
    const drawSMCOverlays = (0, react_1.useCallback)(() => {
        ...
        if (candles && candles.length > 0) {
            ...
            if (entryTimestamp) {
                const timeSec = Math.floor(new Date(entryTimestamp).getTime() / 1000);
        ...
        if (clientSMC) {
                    ...
                    if (obTopY !== null && obBotY !== null) {
                        ...
                        if (boxW > 20) {
                            ...
                            ctx.fillText(isBull
                                ...
                                : `▼ Bearish OB [${ob.low.toFixed(2)} - ${ob.high.toFixed(2)}]`, startX + 6, boxY + Math.min(12, boxH - 2));
                        }
                    }
                });
            }
            ...
            if (showCHoCH && clientSMC.changesOfCharacter && clientSMC.changesOfCharacter.length > 0) {
                clientSMC.changesOfCharacter.slice(-3).forEach((ch) => {
                    ...
                    if (y !== null) {
                        ...
                        const timeSec = Math.floor(new Date(ch.timestamp).getTime() / 1000);
            ...
            if (showBOS && clientSMC.breaksOfStructure && clientSMC.breaksOfStructure.length > 0) {
                clientSMC.breaksOfStructure.slice(-4).forEach((bos) => {
                    ...
                    if (y !== null) {
                        ...
                        const timeSec = Math.floor(new Date(bos.timestamp).getTime() / 1000);
                        const candleX = chart.timeScale().timeToCoordinate(timeSec);
            ...
            if (showLiq && clientSMC.liquidityPools && clientSMC.liquidityPools.length > 0) {
                ...
                clientSMC.liquidityPools
                    ...
                    if (y !== null) {
                        ...
                        const timeSec = Math.floor(new Date(lp.firstTimestamp).getTime() / 1000);
            ...
            if (showLiq && clientSMC.liquiditySweeps && clientSMC.liquiditySweeps.length > 0) {
                clientSMC.liquiditySweeps.slice(-5).forEach((sweep) => {
                    ...
                    if (y !== null && sweepTime) {
                        const timeSec = Math.floor(new Date(sweepTime).getTime() / 1000);
                        const candleX = chart.timeScale().timeToCoordinate(timeSec);
    ...
    const handleCanvasClick = (e) => {
        if (!activeDrawingTool || !mainSeriesRef.current)
            return;
    ...
    return (<div className={`bg-[#0B0F19] border border-slate-800 rounded-xl shadow-2xl flex flex-col transition-all duration-200 ${isFullscreen
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-800/80 mb-2">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5">
            {onSymbolChange ? (<div className="flex items-center bg-slate-950 border border-slate-800 rounded-lg p-0.5 text-xs font-mono">
                {[
                ...
                { sym: 'INFY', label: 'INFY' },
            ].map((item) => (<button key={item.sym} onClick={() => onSymbolChange(item.sym)} className={`px-2 py-1 rounded transition-all font-bold ${symbol === item.sym
                    ? 'bg-cyan-500 text-slate-950 shadow-sm font-black'
                    : 'text-slate-400 hover:text-white'}`}>
                    {item.label}
                  </button>))}
              </div>) : (<span className="text-xl font-black text-white font-mono tracking-tight">
                {symbol}
              </span>)}
          ...
          <div className="flex items-center gap-2 pl-3 border-l border-slate-800">
            ...
            <span className={`text-xs font-bold px-2 py-0.5 rounded flex items-center gap-0.5 ${liveChangePercent >= 0
            ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'
            : 'bg-rose-500/20 text-rose-400 border border-rose-500/30'}`}>
              {liveChangePercent >= 0 ? (<lucide_react_1.TrendingUp className="w-3.5 h-3.5"/>) : (<lucide_react_1.TrendingDown className="w-3.5 h-3.5"/>)}
              {liveChangePercent >= 0 ? '+' : ''}
              {liveChangePercent}%
            </span>
        ...
        {onTimeframeChange && (<div className="flex items-center bg-slate-900 border border-slate-800 rounded-lg p-0.5 text-xs font-mono">
            {timeframes.map((tf) => (<button key={tf} onClick={() => onTimeframeChange(tf)} className={`px-2.5 py-1 rounded transition-all font-bold ${timeframe.toLowerCase() === tf.toLowerCase()
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center bg-slate-900 border border-slate-800 rounded-lg p-0.5 text-xs">
            <button onClick={handleZoomIn} title="Zoom In (+)" className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded transition-colors">
              <lucide_react_1.ZoomIn className="w-3.5 h-3.5"/>
            </button>
            <button onClick={handleZoomOut} title="Zoom Out / Minimize Candles (-)" className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded transition-colors">
              <lucide_react_1.ZoomOut className="w-3.5 h-3.5"/>
            </button>
            <button onClick={handleResetView} title="Reset View / Fit Content" className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded transition-colors">
          ...
          <div className="flex items-center bg-slate-900 border border-slate-800 rounded-lg p-0.5 text-xs font-mono">
            {['Candles', 'Bar', 'Line', 'Area'].map((type) => (<button key={type} onClick={() => setChartType(type)} className={`px-2 py-1 rounded transition-all ${chartType === type
          ...
          <button onClick={() => setShowIndicatorsModal(!showIndicatorsModal)} className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-mono border transition-all ${showIndicatorsModal
            ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/40'
            : 'bg-slate-900 text-slate-300 border-slate-800 hover:bg-slate-800'}`}>
            <lucide_react_1.Sliders className="w-3.5 h-3.5"/>
            <span>Indicators</span>
          </button>

          {/* Volume Profile Toggle */}
          <button onClick={() => setShowVolumeProfile(!showVolumeProfile)} title="Toggle Institutional Volume Profile (POC, VAH, VAL)" className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-mono border transition-all ${showVolumeProfile
          ...
          {/* Kill Zones Toggle */}
          <button onClick={() => setShowKillZones(!showKillZones)} title="Toggle ICT Kill Zones & Session Watermark" className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-mono border transition-all ${showKillZones
            ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40'
            : 'bg-slate-900 text-slate-400 border-slate-800 hover:bg-slate-800'}`}>
            <lucide_react_1.Clock className="w-3.5 h-3.5"/>
            <span>Kill Zones</span>
          </button>

          {/* Liquidity Heatmap Toggle */}
          <button onClick={() => setShowLiquidityHeatmap(!showLiquidityHeatmap)} title="Toggle Resting Stop-Loss Liquidity Heatmap" className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-mono border transition-all ${showLiquidityHeatmap
            ? 'bg-cyan-500/20 text-cyan-300 border-cyan-500/40'
            : 'bg-slate-900 text-slate-400 border-slate-800 hover:bg-slate-800'}`}>
            <lucide_react_1.Droplets className="w-3.5 h-3.5"/>
            <span>Liq Heatmap</span>
          </button>

          {/* SMC Overlays Toggle */}
          <button onClick={() => setShowSMCModal(!showSMCModal)} className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-mono border transition-all ${showSMCModal
          ...
          <div className="flex items-center bg-slate-900 border border-slate-800 rounded-lg p-0.5 text-xs">
            <button onClick={() => setActiveDrawingTool(activeDrawingTool === 'hline' ? null : 'hline')} title="Horizontal Line" className={`p-1.5 rounded transition-colors ${activeDrawingTool === 'hline'
            ? 'bg-cyan-500/20 text-cyan-400'
            : 'text-slate-400 hover:text-white'}`}>
              <lucide_react_1.Pencil className="w-3.5 h-3.5"/>
            </button>
            {drawings.length > 0 && (<button onClick={clearAllDrawings} title="Clear All Drawings" className="p-1.5 text-rose-400 hover:bg-slate-800 rounded transition-colors">
                <lucide_react_1.Trash2 className="w-3.5 h-3.5"/>
              </button>)}
          </div>

          {/* Fullscreen Maximize / Minimize Button */}
          <button onClick={() => setIsFullscreen(!isFullscreen)} title={isFullscreen ? 'Minimize Window (ESC)' : 'Maximize Fullscreen'} className={`p-1.5 rounded-lg border transition-all ${isFullscreen
            ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/40'
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 py-1.5 px-3 bg-slate-900/90 border border-slate-800 rounded-lg mb-2 text-xs font-mono shadow-md">
        <div className="flex flex-wrap items-center gap-2.5">
          {paperPosition && !isLocallyCut ? (<div className="flex flex-wrap items-center gap-2.5">
              ...
              <span className={`px-2.5 py-0.5 rounded font-black border flex items-center gap-1 ${paperPosition.unrealizedPnL >= 0
            ...
            </div>) : isLocallyCut ? (<div className="flex items-center gap-2">
              <span className="relative flex h-2.5 w-2.5">
            ...
            </div>) : effSignal?.state === 'SL_HIT' ? (<div className="flex items-center gap-2">
              <span className="relative flex h-2.5 w-2.5">
            ...
            </div>) : effSignal?.state === 'TP2_HIT' || effSignal?.state === 'TP3_HIT' ? (<div className="flex items-center gap-2">
              <span className="relative flex h-2.5 w-2.5">
            ...
            </div>) : effSignal?.state === 'TP1_HIT' ? (<div className="flex items-center gap-2">
              <span className="relative flex h-2.5 w-2.5">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"/>
                <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-emerald-500"/>
              </span>
        ...
        <div className="flex items-center gap-2">
          <span className={`px-3 py-1 rounded-lg text-xs font-black transition-all flex items-center gap-1.5 shadow-md ${isBullish
      ...
      <div className="bg-[#0B101D]/90 border border-slate-800/90 rounded-xl p-3 flex flex-wrap items-center justify-between gap-3 text-xs font-mono shadow-2xl backdrop-blur-2xl relative overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-2 relative z-10">
          ...
          {tp3Price && (<div className="hidden sm:flex items-center gap-1.5 bg-purple-950/40 border border-purple-500/40 px-2 py-1 rounded-lg">
        ...
        <div className="flex flex-wrap items-center gap-2 relative z-10">
          ...
          <span className={`px-3 py-1 rounded-lg font-black border text-[10px] uppercase tracking-wider flex items-center gap-1 shadow-sm ${(aiPredictionInfo?.recommendation || 'HIGH_CONFIDENCE') === 'HIGH_CONFIDENCE'
      ...
      {ohlcData && (<div className="flex flex-wrap items-center gap-3 text-[11px] font-mono text-slate-400 pb-2">
          <span>
          ...
          <span>
            C:{' '}
            <strong className={ohlcData.close >= ohlcData.open ? 'text-[#089981]' : 'text-[#F23645]'}>
          ...
          {(() => {
                ...
                const isUp = diff >= 0;
                return (<span className={`font-bold ${isUp ? 'text-[#089981]' : 'text-[#F23645]'}`}>
                {isUp ? '+' : ''}
                {diff.toFixed(2)} ({isUp ? '+' : ''}
                {pct.toFixed(2)}%)
              </span>);
      ...
      {showIndicatorsModal && (<div className="bg-slate-900/95 border border-slate-800 rounded-lg p-3 mb-2 grid grid-cols-2 sm:grid-cols-6 gap-2 text-xs font-mono animate-in fade-in">
      ...
      {showSMCModal && (<div className="bg-slate-900/95 border border-slate-800 rounded-lg p-3 mb-2 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2 text-xs font-mono animate-in fade-in">
      ...
      <div className="relative flex-1 w-full min-h-[420px] h-full overflow-hidden">
        ...
        <canvas ref={overlayCanvasRef} onClick={handleCanvasClick} className={`absolute inset-0 w-full h-full z-10 ${activeDrawingTool
            ? 'cursor-crosshair pointer-events-auto'
            : 'cursor-default pointer-events-none'}`}/>

        {/* Floating MTF Trade Permission Badge */}
        {clientMTF && (<div className="absolute top-3 left-3 z-20 bg-slate-950/85 backdrop-blur-md border border-slate-800 rounded-lg px-2.5 py-1.5 shadow-xl text-[10px] font-mono flex items-center gap-2 pointer-events-auto">
            <span className={`h-2 w-2 rounded-full animate-ping ${clientMTF.overallBias === 'STRONG_BULLISH'
                ? 'bg-emerald-400'
                : clientMTF.overallBias === 'STRONG_BEARISH'
                    ? 'bg-rose-400'
                    : 'bg-amber-400'}`}/>
            <span className="text-slate-400 font-bold">RADAR:</span>
            <span className={`font-black ${clientMTF.overallBias === 'STRONG_BULLISH'
                ? 'text-emerald-400'
                : clientMTF.overallBias === 'STRONG_BEARISH'
                    ? 'text-rose-400'
                    : 'text-amber-400'}`}>
        ...
        {showSMCDash && (<div className="absolute top-3 right-16 bg-slate-950/85 backdrop-blur-md border border-slate-800 rounded-lg p-2.5 shadow-xl text-[11px] font-mono space-y-1.5 pointer-events-auto">
            <div className="flex items-center justify-between gap-4 border-b border-slate-800/80 pb-1">
              <span className="text-slate-400 font-bold flex items-center gap-1">
                <lucide_react_1.Shield className="w-3 h-3 text-cyan-400"/>
                SMC Metric
              </span>
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <span className={`font-bold flex items-center gap-1 ${isBullish ? 'text-emerald-400' : 'text-rose-400'}`}>
                {isBullish ? (<lucide_react_1.ArrowUpRight className="w-3 h-3"/>) : (<lucide_react_1.ArrowDownRight className="w-3 h-3"/>)}
            ...
            {effSignal?.score !== undefined && (<div className="flex items-center justify-between gap-4 pt-1 border-t border-slate-800/80">
        ...
        {activeDrawingTool && (<div className="absolute top-2 left-2 bg-cyan-500 text-slate-950 px-3 py-1 rounded-lg text-xs font-mono font-bold shadow-lg flex items-center gap-2">
            <lucide_react_1.Pencil className="w-3.5 h-3.5"/>
            <span>Click on the chart to place {activeDrawingTool.toUpperCase()}</span>
            <button onClick={() => setActiveDrawingTool(null)} className="ml-2 bg-slate-950 text-white px-1.5 py-0.5 rounded text-[10px]">
              Cancel
            </button>
          </div>)}
      </div>
