...
exports.ScannerTableSkeleton = ScannerTableSkeleton;
const react_1 = __importDefault(require("react"));
const lucide_react_1 = require("lucide-react");
const SAMPLE_SCANNER_DATA = [
    {
        symbol: 'NIFTY',
        name: 'NIFTY 50 Index',
        direction: 'LONG',
...
function ScannerTableSkeleton({ selectedSymbol, onSelectSymbol, }) {
    return (<div className="bg-surface rounded-xl border border-surface-border overflow-hidden flex flex-col h-full">
      <div className="p-4 border-b border-surface-border flex items-center justify-between">
        <div className="flex items-center gap-2">
          <lucide_react_1.Zap className="w-4 h-4 text-indigo-400"/>
      ...
      <div className="overflow-x-auto flex-1">
        <table className="w-full text-left text-xs font-mono">
          ...
          <tbody className="divide-y divide-surface-border">
            ...
            return (<tr key={row.symbol} onClick={() => onSelectSymbol(row.symbol)} className={`cursor-pointer transition-colors ${isSelected
                  ...
                  <td className="p-3">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded font-semibold text-[11px] ${row.direction === 'LONG'
                    ? 'bg-bullish/10 text-bullish border border-bullish/20'
                    : row.direction === 'SHORT'
                        ? 'bg-bearish/10 text-bearish border border-bearish/20'
                        : 'bg-slate-800 text-slate-400'}`}>
                      {row.direction === 'LONG' && <lucide_react_1.ArrowUpRight className="w-3 h-3"/>}
                      {row.direction === 'SHORT' && <lucide_react_1.ArrowDownRight className="w-3 h-3"/>}
                      {row.direction}
                    </span>
                  </td>
                  <td className="p-3">
                    ...
                    <div className="w-16 bg-slate-800 h-1.5 rounded-full overflow-hidden mt-1">
                      <div className={`h-full ${row.score >= 80
                    ? 'bg-indigo-500'
                    : row.score >= 70
                        ? 'bg-emerald-500'
                        : 'bg-amber-500'}`} style={{ width: `${row.score}%` }}></div>
                    </div>
                  </td>
                  <td className="p-3">
                    <span className={`px-2 py-0.5 rounded font-bold text-xs ${row.grade === 'A+'
...
//# sourceMappingURL=ScannerTableSkeleton.js.map