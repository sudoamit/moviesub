...
export const CorrelationMatrix: React.FC<CorrelationMatrixProps> = ({ tickers }) => {
  ...
  return (
    <div className="bg-[#111827]/95 backdrop-blur-md border border-cyan-500/30 rounded-xl p-5 shadow-2xl space-y-5 font-mono">
      {/* Header */}
          ...
          <tbody className="divide-y divide-slate-800/60 bg-slate-950/60">
            {correlationData.map((item) => (
              <tr key={item.symbol} className="hover:bg-slate-900/50 transition-colors">
                ...
                <td className="p-3">
                  {item.isDivergent ? (
                    <span className="px-2 py-1 rounded bg-amber-950/80 text-amber-300 border border-amber-700 font-bold text-[10px] flex items-center gap-1 w-fit">
                      <AlertTriangle className="w-3 h-3 text-amber-400" />
                      Divergence Detected
                    </span>
                  ) : item.isAligned ? (
                    <span className="px-2 py-1 rounded bg-emerald-950/80 text-emerald-300 border border-emerald-700 font-bold text-[10px] flex items-center gap-1 w-fit">
