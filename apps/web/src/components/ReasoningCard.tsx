...
import { ISignalReasoning, ISignalSetup } from '@quant/shared';
import {
  CheckCircle2,
  ShieldAlert,
  Target,
...
export const ReasoningCard: React.FC<ReasoningCardProps> = ({ signal }) => {
  if (!signal || !signal.reasoning) {
    return (
      <div className="bg-[#0B0F19] border border-slate-800 rounded-xl p-6 shadow-2xl space-y-3 font-mono">
  ...
  return (
    <div className="bg-[#0B0F19] border border-slate-800 rounded-xl p-4 sm:p-6 shadow-2xl space-y-5 font-mono relative overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span
                className={`text-xs font-black px-2.5 py-0.5 rounded-lg flex items-center gap-1 ${
                  isBull
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {reasoning.confirmedChecklist?.map((item, idx) => (
            <div
              key={idx}
              className="flex items-center gap-2.5 bg-slate-900/80 border border-slate-800/90 px-3.5 py-2.5 rounded-xl text-xs text-slate-200 hover:border-slate-700 transition-all"
            >
