...
export const LearningEngineDashboard: React.FC = () => {
  const [activeSubTab, setActiveSubTab] = useState<
    'overview' | 'errors' | 'patterns' | 'candidates' | 'experiences'
  >('overview');
    ...
    setCandidates([
      {
        ...
        type: 'FILTER',
        description:
          'Reject setups matching loss-inducing condition: [HTF_CONFLICT AND HIGH_VOLATILITY]',
        evidence: {
          sampleSize: 26,
      ...
      },
      {
        ...
        type: 'THRESHOLD',
        description:
          'Boost position sizing by 1.2x when [HTF_ALIGNED + LIQUIDITY_SWEPT + HIGH_RVOL]',
        evidence: {
          sampleSize: 42,
  ...
  const handleTriggerRun = () => {
    ...
    setTimeout(() => {
      ...
      setLastCycleMessage(
        'Self-Improvement cycle completed: Ingested 142 experiences, mined 3 patterns, validated 2 candidates (in shadow mode).',
      );
    }, 1500);
  };
  ...
  const handlePromoteCandidate = (candidateId: string) => {
    setCandidates((prev) =>
      prev.map((c) => (c.id === candidateId ? { ...c, status: 'PROMOTED' } : c)),
    );
    if (overview) {
      setOverview({
        ...overview,
  ...
  const handleRollback = () => {
    ...
    setLastCycleMessage(
      'Rollback successfully executed: Reverted to previous stable baseline v2.0-smc-quant.',
    );
  };

  return (
    <div className="bg-[#080C14] border border-cyan-500/30 rounded-2xl p-4 sm:p-6 shadow-2xl font-mono space-y-6 text-slate-200 relative overflow-hidden">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-5">
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="px-4 py-2 bg-gradient-to-r from-cyan-500 to-indigo-600 hover:from-cyan-400 hover:to-indigo-500 text-slate-950 font-black text-xs rounded-xl shadow-lg shadow-cyan-500/20 transition-all flex items-center gap-2 active:scale-95 disabled:opacity-50"
          >
          ...
          <button
            ...
            className="px-3.5 py-2 bg-slate-900 hover:bg-slate-800 text-rose-400 border border-rose-500/30 hover:border-rose-400 text-xs font-bold rounded-xl transition-all flex items-center gap-1.5 shadow"
      ...
      {lastCycleMessage && (
        <div className="p-3 rounded-xl bg-cyan-950/60 border border-cyan-700 text-xs text-cyan-300 flex items-center gap-2">
      ...
      <div className="flex items-center gap-2 border-b border-slate-800 pb-3">
        {(
          [
            ...
            { id: 'errors', label: 'Failure Mode Mining', icon: AlertOctagon },
            { id: 'patterns', label: 'Pattern Discovery', icon: Sparkles },
            { id: 'candidates', label: 'Candidate Pipeline & Shadow Mode', icon: Layers },
          ] as const
        ).map((tab) => {
          const Icon = tab.icon;
          return (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-xl text-xs font-bold uppercase transition-all flex items-center gap-2 ${
                activeSubTab === tab.id
      ...
      {activeSubTab === 'overview' && overview && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div
              className={`p-4 rounded-xl border ${overview.driftStatus.hasDrift ? 'bg-rose-950/60 border-rose-700' : 'bg-slate-900/90 border-slate-800'}`}
            >
              <span className="text-[10px] text-slate-400 uppercase font-bold block">
              ...
              <div className="flex items-center gap-2 mt-1">
                <Activity
                  className={`w-4 h-4 ${overview.driftStatus.hasDrift ? 'text-rose-400' : 'text-emerald-400'}`}
                />
                <span
                  className={`text-sm font-black ${overview.driftStatus.hasDrift ? 'text-rose-400' : 'text-emerald-400'}`}
                >
                ...
                <div className="space-y-1">
                  ...
                  <div className="space-y-0.5">
                    {pat.conditions.map((c, i) => (
                      <span key={i} className="block text-xs font-mono font-black text-white">
                        • {c}
                      </span>
                    ))}
                  </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {candidates.map((cand) => (
              <div
                key={cand.id}
                className="bg-slate-900/90 border border-slate-800 p-4 rounded-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4"
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {cand.status !== 'PROMOTED' ? (
                    <button
                      ...
                      className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-black text-xs rounded-lg transition-all shadow flex items-center gap-1.5"
                    >
                    ...
                    <span className="text-xs text-emerald-400 font-bold flex items-center gap-1">
                      <ShieldCheck className="w-4 h-4" /> ACTIVE IN PRODUCTION
                    </span>
                  )}
                </div>
              </div>
