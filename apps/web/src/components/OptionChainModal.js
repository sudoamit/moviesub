...
exports.OptionChainModal = void 0;
const react_1 = __importStar(require("react"));
const lucide_react_1 = require("lucide-react");
const OptionChainModal = ({ symbol, isOpen, onClose, spotPrice, }) => {
    const [data, setData] = (0, react_1.useState)(null);
    const [selectedExpiry, setSelectedExpiry] = (0, react_1.useState)('');
    const [loading, setLoading] = (0, react_1.useState)(true);
    const fetchOptionChain = async (exp, silent = false) => {
        try {
            if (!silent && !data)
                setLoading(true);
    ...
    if (!isOpen)
        return null;
    return (<div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6 font-mono animate-in fade-in">
      <div className="bg-[#0f172a] border border-cyan-500/50 rounded-2xl w-full max-w-5xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="p-4 border-b border-slate-800 flex flex-wrap items-center justify-between gap-3 bg-slate-900/90">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {data && (<span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-950 text-emerald-300 border border-emerald-700 flex items-center gap-1">
          ...
          <div className="flex items-center gap-2">
            {data?.availableExpiries && (<select value={selectedExpiry || data.selectedExpiry} onChange={(e) => setSelectedExpiry(e.target.value)} className="bg-slate-800 border border-slate-700 text-cyan-300 text-xs px-2.5 py-1.5 rounded-lg font-bold outline-none cursor-pointer">
                {data.availableExpiries.map((exp) => (<option key={exp.dateString} value={exp.dateString} className="bg-slate-900 text-white">
                    {exp.formattedLabel}
                  </option>))}
              </select>)}

            <button onClick={() => fetchOptionChain(selectedExpiry)} className="p-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 transition-all text-xs flex items-center gap-1">
              <lucide_react_1.RefreshCw className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`}/>
              Refresh
            </button>
            <button onClick={onClose} className="p-1.5 rounded-lg bg-slate-800 hover:bg-rose-900/50 text-slate-400 hover:text-rose-300 transition-all">
        ...
        {data && (<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 p-3 bg-slate-950/80 border-b border-slate-800 text-xs">
            ...
            <div className="bg-slate-900/90 p-2 rounded-lg border border-slate-800">
              <span className="text-[10px] text-amber-400 block uppercase font-bold flex items-center gap-1">
                <lucide_react_1.Target className="w-3 h-3 text-amber-400"/> MAX PAIN STRIKE
              </span>
            ...
            <div className="bg-slate-900/90 p-2 rounded-lg border border-slate-800">
              <span className="text-[10px] text-purple-400 block uppercase font-bold flex items-center gap-1">
                <lucide_react_1.Zap className="w-3 h-3 text-purple-400"/> GAMMA FLIP LEVEL
              </span>
            ...
            <div className="bg-slate-900/90 p-2 rounded-lg border border-slate-800">
              ...
              <span className={`text-sm font-black mt-0.5 block ${data.pcr > 1 ? 'text-emerald-400' : 'text-rose-400'}`}>
        ...
        <div className="flex-1 overflow-auto p-3">
          {loading ? (<div className="py-20 text-center text-slate-500 text-sm animate-pulse">
              Streaming real-time Option Chain strikes and OI analytics...
            </div>) : data ? (<table className="w-full text-xs border-collapse min-w-[920px]">
              <thead>
              ...
              <tbody className="divide-y divide-slate-800/60 font-mono">
                ...
                const maxOI = Math.max(...data.strikes.map((x) => Math.max(x.call.oi, x.put.oi))) ||
                    100000;
                const callBarPct = Math.min(100, Math.max(5, (s.call.oi / maxOI) * 100));
                const putBarPct = Math.min(100, Math.max(5, (s.put.oi / maxOI) * 100));
                return (<tr key={s.strikePrice} className={`hover:bg-slate-800/40 transition-colors ${isATM
                      ...
                      <td className={`py-2 px-2 text-[10px] w-24 ${isCallITM ? 'bg-emerald-950/20' : ''}`}>
                        <div className="w-full bg-slate-900 rounded h-2 overflow-hidden flex items-center justify-end">
                          <div className="bg-gradient-to-l from-emerald-400 to-teal-600 h-2 rounded" style={{ width: `${callBarPct}%` }} title={`Call OI: ${s.call.oi.toLocaleString()}`}/>
                        </div>
                      </td>
                      ...
                      <td className={`py-2 px-2 text-[10px] ${isCallITM ? 'bg-emerald-950/20' : ''}`}>
                        ...
                        {s.call.oiChange !== 0 && (<span className={`block text-[8px] ${s.call.oiChange > 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                            {s.call.oiChange > 0 ? '+' : ''}
                      ...
                      <td className={`py-2 px-3 text-center ${isATM
                        ...
                        <div className="flex flex-col items-center">
                          ...
                          {isATM && (<span className="text-[8px] bg-slate-950 text-cyan-300 px-1 rounded uppercase font-black">
                              ATM
                            </span>)}
                          {isMaxPain && !isATM && (<span className="text-[8px] bg-amber-500 text-slate-950 px-1 rounded uppercase font-black">
                              MAX PAIN
                            </span>)}
                          {isGammaFlip && !isATM && (<span className="text-[8px] bg-purple-500 text-white px-1 rounded uppercase font-black">
                              γ FLIP
                            </span>)}
                        </div>
                      </td>
                      ...
                      <td className={`py-2 px-2 text-[10px] text-right ${isPutITM ? 'bg-rose-950/20' : ''}`}>
                        ...
                        {s.put.oiChange !== 0 && (<span className={`block text-[8px] ${s.put.oiChange > 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                            {s.put.oiChange > 0 ? '+' : ''}
                      ...
                      <td className={`py-2 px-2 text-[10px] w-24 ${isPutITM ? 'bg-rose-950/20' : ''}`}>
                        <div className="w-full bg-slate-900 rounded h-2 overflow-hidden flex items-center justify-start">
                          <div className="bg-gradient-to-r from-rose-400 to-amber-600 h-2 rounded" style={{ width: `${putBarPct}%` }} title={`Put OI: ${s.put.oi.toLocaleString()}`}/>
                        </div>
                      </td>
                    </tr>);
            })}
...
exports.OptionChainModal = OptionChainModal;
//# sourceMappingURL=OptionChainModal.js.map