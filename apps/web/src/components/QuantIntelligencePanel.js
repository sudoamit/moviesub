...
exports.QuantIntelligencePanel = void 0;
const react_1 = __importStar(require("react"));
const lucide_react_1 = require("lucide-react");
const QuantIntelligencePanel = ({ currentSymbol, activeSignal, livePrice, }) => {
    ...
    const isCrypto = currentSymbol.includes('BTC') ||
        currentSymbol.includes('ETH') ||
        currentSymbol.includes('USDT');
    // Regime Color & Icon
    const getRegimeBadge = (regimeType) => {
        switch (regimeType) {
            case 'BULLISH_TREND':
                return {
                    label: 'BULLISH TREND',
    ...
    return (<div className="bg-[#0B0F19] border border-cyan-500/30 rounded-xl p-4 sm:p-6 shadow-2xl font-mono space-y-5 relative overflow-hidden">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800/80 pb-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-cyan-500/20 to-indigo-500/20 border border-cyan-500/40 flex items-center justify-center shrink-0 shadow-lg">
            <lucide_react_1.BrainCircuit className="w-5 h-5 text-cyan-400"/>
          </div>
          <div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center bg-slate-900 border border-slate-800 p-1 rounded-lg">
            {['overview', 'pillars', 'volatility', 'trace'].map((tab) => (<button key={tab} onClick={() => setActiveSubTab(tab)} className={`px-3 py-1 rounded text-xs font-bold uppercase transition-all ${activeSubTab === tab
                ? 'bg-cyan-600 text-white shadow'
                : 'text-slate-400 hover:text-white'}`}>
                {tab}
              </button>))}
          </div>
          <button onClick={() => setIsExpanded(!isExpanded)} className="p-1.5 bg-slate-900 hover:bg-slate-800 border border-slate-700 rounded-lg text-slate-300 transition-colors">
            {isExpanded ? <lucide_react_1.ChevronUp className="w-4 h-4"/> : <lucide_react_1.ChevronDown className="w-4 h-4"/>}
          </button>
        </div>
      </div>
      ...
      {isExpanded && (<div className="space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {/* 1. Market Regime */}
            <div className={`p-3.5 rounded-xl border ${currentRegimeBadge.bg} ${currentRegimeBadge.border}`}>
            ...
            <div className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800">
              ...
              <div className="flex items-center gap-1.5 mt-1">
                ...
                <span className={`text-xs font-black ${multiHorizon?.alignment === 'ALIGNED'
                ? 'text-emerald-400'
            ...
            <div className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800">
              ...
              <div className="flex items-center gap-1.5 mt-1">
                <lucide_react_1.Sparkles className="w-4 h-4 text-amber-400"/>
          ...
          {activeSubTab === 'overview' && (<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="lg:col-span-2 bg-slate-900/80 border border-slate-800 p-4 rounded-xl space-y-3">
                <h4 className="text-xs font-bold text-white uppercase flex items-center gap-1.5">
                  <lucide_react_1.Radio className="w-3.5 h-3.5 text-cyan-400"/>
                ...
                <div className="pt-2 border-t border-slate-800/80 flex flex-wrap items-center justify-between gap-3 text-xs">
                  <span className="text-slate-400 font-bold flex items-center gap-1.5">
                    <lucide_react_1.Cpu className="w-3.5 h-3.5 text-indigo-400"/>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {!isCrypto ? (<>
                        ...
                        <span className="bg-slate-950 text-slate-300 border border-slate-800 px-2 py-0.5 rounded text-[11px]">
                          ...
                          <strong className="text-emerald-400">
                            +
                            {((quant?.alternativeData?.cryptoFundingRate ?? 0.0001) * 100).toFixed(3)}
                            %
                          </strong>
                        </span>
                        <span className="bg-slate-950 text-slate-300 border border-slate-800 px-2 py-0.5 rounded text-[11px]">
              ...
              <div className="bg-slate-900/80 border border-slate-800 p-4 rounded-xl space-y-2.5">
                ...
                <div className="text-[11px] text-slate-400 space-y-1">
                  <div className="flex items-center gap-1.5 text-emerald-400">
                    <lucide_react_1.CheckCircle2 className="w-3 h-3 shrink-0"/>
          ...
          {activeSubTab === 'pillars' && (<div className="space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                {[
                ...
                ].map((pillar) => (<div key={pillar.name} className="bg-slate-900/90 border border-slate-800 p-3 rounded-lg text-center">
                    ...
                    <div className="w-full bg-slate-950 h-1.5 rounded-full mt-2 overflow-hidden border border-slate-800">
                      <div className="bg-gradient-to-r from-cyan-500 to-emerald-400 h-full rounded-full" style={{ width: `${(pillar.pts / pillar.max) * 100}%` }}/>
                    </div>
                  </div>))}
              </div>
          ...
          {activeSubTab === 'volatility' && (<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="bg-slate-900/80 border border-slate-800 p-4 rounded-xl space-y-2">
                ...
                <div className="space-y-1.5 text-xs text-slate-300">
                  <div className="flex justify-between">
                    ...
                    <span className={`font-bold ${(quant?.returns?.return1Bar ?? 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                      {((quant?.returns?.return1Bar ?? 0) * 100).toFixed(2)}%
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">5-Bar Return:</span>
                    <span className={`font-bold ${(quant?.returns?.return5Bar ?? 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
          ...
          {activeSubTab === 'trace' && (<div className="bg-slate-950 border border-slate-800 p-4 rounded-xl space-y-3">
              <h4 className="text-xs font-bold text-white uppercase flex items-center gap-1.5">
              ...
              <div className="space-y-2">
                ...
                ]).map((reason, idx) => (<div key={idx} className="flex items-start gap-2 text-xs text-slate-200">
                    <span className="text-cyan-400 font-bold">#{idx + 1}</span>
                    <span>{reason}</span>
                  </div>))}
              </div>

              {trace?.invalidationRisks && trace.invalidationRisks.length > 0 && (<div className="mt-3 pt-3 border-t border-slate-800 space-y-1">
                  <span className="text-[11px] text-amber-400 font-bold uppercase block flex items-center gap-1">
                    <lucide_react_1.AlertTriangle className="w-3.5 h-3.5"/> Invalidation Warnings / Key Risks:
                  </span>
                  {trace.invalidationRisks.map((risk, idx) => (<p key={idx} className="text-xs text-slate-400 pl-4">
                      • {risk}
                    </p>))}
                </div>)}
            </div>)}
...
exports.QuantIntelligencePanel = QuantIntelligencePanel;
//# sourceMappingURL=QuantIntelligencePanel.js.map