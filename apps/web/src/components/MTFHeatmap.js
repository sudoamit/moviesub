...
exports.MTFHeatmap = void 0;
const react_1 = __importDefault(require("react"));
const lucide_react_1 = require("lucide-react");
const MTFHeatmap = ({ selectedSymbol, onSelectSymbol, signals = [], }) => {
    const baseAssets = [
    ...
    const computedMatrix = react_1.default.useMemo(() => {
    ...
    const renderBadge = (dir) => {
        if (dir === 'BULLISH') {
            return (<span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-bold bg-emerald-500/15 text-emerald-400 border border-emerald-500/30">
          <lucide_react_1.ArrowUpRight className="w-2.5 h-2.5"/> Bull
        </span>);
        }
        if (dir === 'BEARISH') {
            return (<span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-bold bg-rose-500/15 text-rose-400 border border-rose-500/30">
          <lucide_react_1.ArrowDownRight className="w-2.5 h-2.5"/> Bear
        </span>);
        }
        return (<span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-medium bg-slate-800 text-slate-400 border border-slate-700">
        <lucide_react_1.Minus className="w-2.5 h-2.5"/> Range
      </span>);
    };
    return (<div className="bg-[#0B0F19] border border-slate-800 rounded-xl p-4 shadow-xl space-y-3 font-mono">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-slate-800/50">
            ...
            return (<tr key={item.symbol} onClick={() => onSelectSymbol(item.symbol)} className={`cursor-pointer transition-all duration-200 ${isSelected
                  ...
                  <td className="py-2 text-right">
                    {item.is100Aligned ? (<span className="text-[9px] text-emerald-400 font-bold px-1.5 py-0.5 bg-emerald-950/80 border border-emerald-800/80 rounded inline-flex items-center gap-0.5">
                        <lucide_react_1.CheckCircle2 className="w-2.5 h-2.5 text-emerald-400"/>
                        {item.alignmentLabel}
                      </span>) : item.is75Aligned ? (<span className="text-[9px] text-cyan-400 font-bold px-1.5 py-0.5 bg-cyan-950/80 border border-cyan-800/80 rounded">
                        {item.alignmentLabel}
                      </span>) : (<span className="text-[9px] text-amber-400 font-medium px-1.5 py-0.5 bg-amber-950/80 border border-amber-800/80 rounded inline-flex items-center gap-0.5">
                        <lucide_react_1.AlertCircle className="w-2.5 h-2.5 text-amber-400"/>
                        {item.alignmentLabel}
                      </span>)}
                  </td>
                </tr>);
...
exports.MTFHeatmap = MTFHeatmap;
//# sourceMappingURL=MTFHeatmap.js.map