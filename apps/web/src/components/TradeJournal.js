...
const formatDateTime = (dateStr, symbol) => {
    if (!dateStr)
        return 'N/A';
...
const formatQuantity = (symbol, qty) => {
    if (qty && qty > 0) {
        ...
        if (symbol === 'XAUUSD' || symbol === 'GOLD')
            return `${qty} oz Gold`;
        if (symbol === 'NIFTY')
            return `${qty} Qty (${Math.round(qty / 65)}L)`;
        if (symbol === 'BANKNIFTY')
            return `${qty} Qty (${Math.round(qty / 15)}L)`;
        if (symbol === 'FINNIFTY')
            return `${qty} Qty (${Math.round(qty / 40)}L)`;
        if (symbol === 'RELIANCE')
            return `${qty} Qty (${Math.round(qty / 250)}L)`;
        if (symbol === 'HDFCBANK')
            return `${qty} Qty (${Math.round(qty / 550)}L)`;
        if (symbol === 'INFY')
            return `${qty} Qty (${Math.round(qty / 400)}L)`;
        return `${qty} Qty`;
    }
    if (symbol === 'BTCUSDT')
        return '0.20 BTC';
...
const getContractLabel = (symbol, direction, entryPrice) => {
    ...
    if (!strike)
        return direction === 'BEARISH' ? 'PE' : 'CE';
    return `${strike} ${direction === 'BEARISH' ? 'PE' : 'CE'}`;
};
const TradeJournal = ({ currentSymbol = 'NIFTY', activeSignal, livePrice, onTradeClosedNotification, }) => {
    ...
    const [selectedTradeReason, setSelectedTradeReason] = (0, react_1.useState)(null);
    const fetchCompletedTrades = (0, react_1.useCallback)(async () => {
        setIsLoading(true);
        try {
    ...
    const handleClearJournal = async () => {
        if (typeof window !== 'undefined') {
            const confirmed = window.confirm('Are you sure you want to clear all closed trades from the Journal?');
            if (!confirmed)
                return;
        }
        try {
    ...
    const filteredTrades = trades.filter((t) => {
        ...
        if (selectedFilter === 'WINS')
            return t.state !== 'SL_HIT';
        if (selectedFilter === 'LOSSES')
            return t.state === 'SL_HIT';
        return t.symbol === selectedFilter;
    });
    const effectiveStats = react_1.default.useMemo(() => {
        let totalPnl = 0;
        let winningTrades = 0;
        let losingTrades = 0;
        let grossProfit = 0;
    ...
    return (<div className="bg-[#111827]/90 backdrop-blur border border-slate-800 rounded-xl p-5 shadow-xl space-y-4">
      {/* Header Bar */}
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800/80 pb-3">
        <div className="flex items-center gap-2">
          <lucide_react_1.BookOpen className="w-4 h-4 text-cyan-400"/>
          ...
          <span className="text-[10px] bg-emerald-950/60 text-emerald-400 border border-emerald-800/40 px-2 py-0.5 rounded font-mono font-bold flex items-center gap-1">
            <lucide_react_1.ShieldCheck className="w-3 h-3"/> Auto-Recorded in Postgres
          </span>
        </div>

        <div className="flex items-center gap-2">
          <button onClick={handleExportTaxReportCsv} disabled={isLoading || trades.length === 0} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-900 hover:bg-slate-800 border border-slate-700 hover:border-slate-600 text-slate-200 rounded-lg font-mono text-xs font-bold transition-all shadow-sm active:scale-95 disabled:opacity-50" title="Download Tax & Audit CSV Report with STT, GST, and SEBI fee breakdown">
            <lucide_react_1.Download className="w-3.5 h-3.5 text-cyan-400"/>
            <span>Export Tax Report</span>
          </button>

          <button onClick={handleSyncHistoricalTrades} disabled={isLoading} className="flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-cyan-600/30 to-emerald-600/30 border border-cyan-500/50 hover:border-cyan-400 text-cyan-300 rounded-lg font-mono text-xs font-black transition-all shadow-sm active:scale-95" title="Scan market history and populate verified strategy executions into Journal">
            <lucide_react_1.Zap className={`w-3.5 h-3.5 text-cyan-400 ${isLoading ? 'animate-spin' : ''}`}/>
            <span>Sync Strategy Executions</span>
          </button>

          <button onClick={handleClearJournal} disabled={isLoading} className="flex items-center gap-1 px-2.5 py-1.5 bg-rose-950/40 border border-rose-800/60 hover:bg-rose-900/60 text-rose-300 rounded-lg font-mono text-xs font-bold transition-colors" title="Clear All Trades from Journal">
            <lucide_react_1.Trash2 className="w-3.5 h-3.5"/>
            <span>Clear</span>
          </button>

          <button onClick={fetchCompletedTrades} disabled={isLoading} className="p-1.5 bg-slate-900 border border-slate-700 hover:bg-slate-800 rounded-lg text-slate-300 transition-colors" title="Refresh Journal">
            <lucide_react_1.RefreshCw className={`w-3.5 h-3.5 ${isLoading ? 'animate-spin text-cyan-400' : ''}`}/>
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-1.5 pt-1 text-xs font-mono">
        <span className="text-[10px] text-slate-400 flex items-center gap-1 mr-1">
          <lucide_react_1.Filter className="w-3 h-3"/> Filter:
        </span>
        {[
            'ALL',
            'NIFTY',
            ...
            'WINS',
            'LOSSES',
        ].map((f) => (<button key={f} onClick={() => setSelectedFilter(f)} className={`px-2.5 py-1 rounded text-[11px] font-bold transition-all ${selectedFilter === f
      ...
      <div className="overflow-x-auto border border-slate-800 rounded-lg shadow-inner">
        <table className="w-full text-left border-collapse text-xs font-mono min-w-[1100px]">
            ...
            <tr className="bg-slate-900/95 text-slate-400 border-b border-slate-800 text-[10px] uppercase tracking-wider">
              ...
              <th className="py-3 px-3 text-left">
                <span className="flex items-center gap-1">
                  <lucide_react_1.Calendar className="w-3 h-3 text-cyan-400"/>
                  Entry & Close Time
                </span>
              </th>
              <th className="py-3 px-3 text-center">
                <span className="flex items-center justify-center gap-1">
                  <lucide_react_1.Timer className="w-3 h-3 text-amber-400"/>
                  Duration
                </span>
              </th>
            </tr>
          ...
          <tbody className="divide-y divide-slate-800/60 bg-slate-950/50">
            ...
            const contractLabel = t.contractSymbol && t.contractSymbol !== t.symbol
                ? t.contractSymbol
                : isOption && t.strike
            ...
            return (<tr key={t.id} className="hover:bg-slate-900/70 transition-colors">
                  {/* Instrument */}
                  <td className="py-3 px-3 font-bold text-white">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {contractLabel && (<span className="text-[10px] bg-amber-950 text-amber-300 border border-amber-800/80 px-1.5 py-0.5 rounded font-mono font-bold tracking-tight shadow-sm">
                          {contractLabel}
                        </span>)}
                    </div>
                  ...
                  <td className="py-3 px-3">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold flex items-center gap-0.5 w-fit ${t.direction === 'BULLISH'
                    ? 'bg-emerald-950 text-emerald-400 border border-emerald-800/60'
                    : 'bg-rose-950 text-rose-400 border border-rose-800/60'}`}>
                      {t.direction === 'BULLISH' ? (<lucide_react_1.TrendingUp className="w-3 h-3"/>) : (<lucide_react_1.TrendingDown className="w-3 h-3"/>)}
                      {t.direction}
                    </span>
                  </td>
                  ...
                  <td className="py-3 px-3 max-w-sm">
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        {isSaiyan ? (<span className="bg-amber-950/80 text-amber-300 border border-amber-700/80 px-1.5 py-0.5 rounded text-[9px] font-bold flex items-center gap-1">
                            <lucide_react_1.Zap className="w-2.5 h-2.5 text-amber-400"/> SAIYAN OCC
                          </span>) : (<span className="bg-cyan-950/80 text-cyan-300 border border-cyan-700/80 px-1.5 py-0.5 rounded text-[9px] font-bold flex items-center gap-1">
                            <lucide_react_1.Sparkles className="w-2.5 h-2.5 text-cyan-400"/> INSTITUTIONAL SMC
                          </span>)}
                    ...
                    `Institutional ${t.direction} momentum trigger with multi-timeframe order flow alignment.`}
                      ...
                      <button onClick={() => setSelectedTradeReason(t)} className="text-[10px] text-cyan-400 hover:text-cyan-300 flex items-center gap-1 font-bold bg-cyan-950/40 hover:bg-cyan-950/70 border border-cyan-800/60 px-2 py-0.5 rounded transition-all">
                        <lucide_react_1.ListChecks className="w-3 h-3"/> View 5-Pt Confluence Checklist
                      </button>
                    </div>
                  </td>
                  ...
                  <td className="py-3 px-3">
                    ...
                    t.exitReason?.toLowerCase().includes('cut') ? (<span className="bg-amber-950/80 text-amber-300 border border-amber-800 px-2 py-0.5 rounded text-[10px] font-bold flex items-center gap-1 w-fit">
                            <lucide_react_1.Zap className="w-3 h-3 text-amber-400"/> MANUAL MARKET EXIT
                          </span>) : t.state === 'TP3_HIT' ? (<span className="bg-emerald-950 text-emerald-300 border border-emerald-700 px-2 py-0.5 rounded text-[10px] font-bold flex items-center gap-1 w-fit">
                            <lucide_react_1.Award className="w-3 h-3 text-yellow-400"/> TARGET 3 RUNNER (4.0R)
                          </span>) : t.state === 'TP2_HIT' ? (<span className="bg-teal-950/80 text-teal-300 border border-teal-800 px-2 py-0.5 rounded text-[10px] font-bold flex items-center gap-1 w-fit">
                            <lucide_react_1.Target className="w-3 h-3 text-teal-400"/> TARGET 2 COMPLETED (2.5R)
                          </span>) : t.state === 'TP1_HIT' ? (<span className="bg-emerald-950/80 text-emerald-400 border border-emerald-800 px-2 py-0.5 rounded text-[10px] font-bold flex items-center gap-1 w-fit">
                            <lucide_react_1.CheckCircle2 className="w-3 h-3"/> TARGET 1 COMPLETED (1.5R)
                          </span>) : (<span className="bg-rose-950/80 text-rose-400 border border-rose-800 px-2 py-0.5 rounded text-[10px] font-bold flex items-center gap-1 w-fit">
                            <lucide_react_1.XCircle className="w-3 h-3"/> STOP LOSS HIT (-1.0R)
                          </span>)}
                      </div>
      ...
      {selectedTradeReason && (<div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#0B111E] border border-slate-700 rounded-2xl max-w-xl w-full p-6 space-y-5 font-mono shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                <lucide_react_1.BookOpen className="w-5 h-5 text-cyan-400"/>
                <h3 className="text-sm font-black text-white uppercase tracking-wider">
                  Trade Reason & Confluence Breakdown
                </h3>
              </div>
              <button onClick={() => setSelectedTradeReason(null)} className="p-1 rounded-lg bg-slate-900 border border-slate-800 text-slate-400 hover:text-white">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-950 border border-slate-800 rounded-xl p-3.5 flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded font-bold ${selectedTradeReason.direction === 'BULLISH'
              ...
              <div className="space-y-1.5">
                <span className="text-[11px] text-slate-400 font-bold uppercase flex items-center gap-1.5">
                  <lucide_react_1.Info className="w-3.5 h-3.5 text-cyan-400"/> Core Setup Rationale:
                </span>
              ...
              <div className="space-y-2">
                <span className="text-[11px] text-slate-400 font-bold uppercase flex items-center gap-1.5">
                  <lucide_react_1.ListChecks className="w-3.5 h-3.5 text-emerald-400"/> Confirmed Algorithmic
                  Checklist:
                </span>
            ...
            ]).map((item, idx) => (<div key={idx} className="flex items-start gap-2 text-xs text-slate-300">
                      <lucide_react_1.CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0 mt-0.5"/>
                      <span>{item}</span>
                    </div>))}
                </div>
              </div>
            ...
            <button onClick={() => setSelectedTradeReason(null)} className="w-full py-2 bg-gradient-to-r from-cyan-500 to-teal-400 hover:from-cyan-400 hover:to-teal-300 text-slate-950 font-black rounded-xl text-xs uppercase tracking-wider transition-all shadow-lg shadow-cyan-500/20">
              Close Breakdown
            </button>
          </div>
        </div>)}
