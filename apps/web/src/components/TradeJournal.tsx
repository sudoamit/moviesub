...
interface IJournalStats {
  ...
  executionDataFilter?: 'VERIFIED' | 'LEGACY' | 'ALL';
...
export const TradeJournal: React.FC<TradeJournalProps> = ({
  ...
  return (
    <div className="bg-[#111827]/90 backdrop-blur border border-slate-800 rounded-xl p-5 shadow-xl space-y-4">
      ...
      {/* Execution Data Mode & Filter Tabs */}
      <div className="flex flex-wrap items-center justify-between gap-3 pt-1 border-t border-slate-800/60 text-xs font-mono">
        {/* Execution Data Mode Selector (Primary Verified vs Historical Legacy) */}
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] text-slate-400 flex items-center gap-1 mr-1 uppercase tracking-wider font-bold">
            <ShieldCheck className="w-3.5 h-3.5 text-cyan-400" /> Execution Data:
          </span>
          <button
            onClick={() => handleExecutionFilterChange('VERIFIED')}
          ...
          <button
            onClick={() => handleExecutionFilterChange('LEGACY')}
          ...
          <button
            onClick={() => handleExecutionFilterChange('ALL')}
            className={`px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all ${
              executionDataFilter === 'ALL'
