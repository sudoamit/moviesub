...
exports.MTFFlowRadarWidget = void 0;
const react_1 = __importStar(require("react"));
const lucide_react_1 = require("lucide-react");
const MTFFlowRadarWidget = ({ symbol = 'NIFTY' }) => {
    const [selectedSym, setSelectedSym] = (0, react_1.useState)(symbol);
    const [radarData, setRadarData] = (0, react_1.useState)(null);
    const [loading, setLoading] = (0, react_1.useState)(true);
    const fetchRadar = async (sym) => {
        try {
            setLoading(true);
    ...
    return (<div className="bg-[#0B0F19] border border-slate-800 rounded-xl p-5 font-mono space-y-5">
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800/80 pb-4">
        <div>
          <div className="flex items-center gap-2">
            <lucide_react_1.Radar className="w-5 h-5 text-cyan-400 animate-spin" style={{ animationDuration: '6s' }}/>
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center bg-slate-900 border border-slate-800 rounded-lg p-0.5 text-xs">
            {['NIFTY', 'BANKNIFTY', 'XAUUSD', 'BTCUSDT', 'RELIANCE', 'HDFCBANK', 'INFY'].map((s) => (<button key={s} onClick={() => setSelectedSym(s)} className={`px-2.5 py-1 rounded transition-all font-bold ${selectedSym === s
          ...
          <button onClick={() => fetchRadar(selectedSym)} disabled={loading} className="p-2 bg-slate-900 hover:bg-slate-800 text-slate-300 rounded-lg border border-slate-800 transition-colors">
      ...
      {radarData && (<div className={`p-5 rounded-xl border transition-all ${isBuy
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center border ${isBuy
                ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/50'
                : isSell
                    ? 'bg-rose-500/20 text-rose-400 border-rose-500/50'
                    : 'bg-amber-500/20 text-amber-400 border-amber-500/50'}`}>
                {isBuy ? (<lucide_react_1.ShieldCheck className="w-7 h-7"/>) : isSell ? (<lucide_react_1.ShieldAlert className="w-7 h-7"/>) : (<lucide_react_1.AlertTriangle className="w-7 h-7"/>)}
              </div>
              ...
              <div>
                ...
                <h3 className={`text-lg font-black tracking-tight ${isBuy ? 'text-emerald-400' : isSell ? 'text-rose-400' : 'text-amber-400'}`}>
      ...
      {radarData && (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {[radarData.tiers.h4, radarData.tiers.h1, radarData.tiers.m15, radarData.tiers.m5].map((t, idx) => {
                const isTierBull = t.direction === 'BULLISH';
                return (<div key={idx} className={`p-4 rounded-xl border transition-all ${isTierBull
                        ? 'bg-emerald-950/10 border-emerald-500/30'
                        : 'bg-rose-950/10 border-rose-500/30'}`}>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-xs font-bold text-slate-300 uppercase">{t.name}</span>
                    <span className={`text-[10px] font-black px-2 py-0.5 rounded flex items-center gap-1 ${isTierBull
                        ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/40'
                        : 'bg-rose-500/20 text-rose-400 border border-rose-500/40'}`}>
                      {isTierBull ? (<lucide_react_1.ArrowUpRight className="w-3 h-3"/>) : (<lucide_react_1.ArrowDownRight className="w-3 h-3"/>)}
                      {t.direction}
                    </span>
                  </div>
...
exports.MTFFlowRadarWidget = MTFFlowRadarWidget;
//# sourceMappingURL=MTFFlowRadarWidget.js.map