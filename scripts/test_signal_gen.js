...
const { SignalGenerator } = require(
  path.join(__dirname, '../packages/trading-engine/dist/index.js'),
);
...
async function testSignals() {
  ...
  for (const inst of instruments) {
    ...
    console.log(`\n======================================================`);
    console.log(
      `Testing Symbol: ${inst.symbol} (M15: ${execCandles.length}, H1: ${htf1Candles.length}, H4: ${htf2Candles.length})`,
    );
    if (execCandles.length > 0) {
      ...
      console.log(
        `Direction: ${signal.direction} | Grade: ${signal.grade} | Score: ${signal.score}`,
      );
      console.log(
        `Entry Optimal: ₹${signal.entryZone?.optimal} (Min: ₹${signal.entryZone?.min}, Max: ₹${signal.entryZone?.max})`,
      );
      console.log(`Stop Loss: ₹${signal.stopLoss}`);
      console.log(
        `TP1: ₹${signal.takeProfits?.tp1} | TP2: ₹${signal.takeProfits?.tp2} | TP3: ₹${signal.takeProfits?.tp3}`,
      );
      console.log(
        `RR1: ${signal.riskRewardRatios?.rr1} | RR2: ${signal.riskRewardRatios?.rr2} | RR3: ${signal.riskRewardRatios?.rr3}`,
      );
